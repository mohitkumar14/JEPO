...
public class StripChart extends BaseStep {
      ...
      if (getStepManager().numIncomingConnectionsOfType(
        ...
        if (m_reset) {
          ...
          for (PlotNotificationListener l : m_plotListeners) {
            l.setLegend(legendEntries, 0.0f, 1.0f);
          }
        }

        float[] dataPoint = new float[m_instanceWidth];
        ...
        if (m_reset) {
          m_reset = false;
          float min =
            data.getPayloadElement(StepManager.CON_AUX_DATA_CHART_MIN, 0.0f);
          float max =
            data.getPayloadElement(StepManager.CON_AUX_DATA_CHART_MAX, 1.0f);
          List<String> legend =
            (List<String>) data
  ...
  public interface PlotNotificationListener {
    ...
    void setLegend(List<String> legendEntries, float min, float max);

    void acceptDataPoint(float[] dataPoint);
  }
}
