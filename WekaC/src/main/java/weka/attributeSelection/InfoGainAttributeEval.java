...
public class InfoGainAttributeEval extends ASEvaluation implements
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    float[][][] counts = new float[data.numAttributes()][][];
    for (int k = 0; k < data.numAttributes(); k++) {
      if (k != classIndex) {
        int numValues = data.attribute(k).numValues();
        counts[k] = new float[numValues + 1][numClasses + 1];
      }
    }

    // Initialize counters
    float[] temp = new float[numClasses + 1];
    ...
    if (m_missing_merge) {
      ...
      for (int k = 0; k < data.numAttributes(); k++) {
        if (k != classIndex) {
          ...
          // Compute marginals
          float[] rowSums = new float[numValues];
          float[] columnSums = new float[numClasses];
          float sum = 0;
          ...
          if (Utils.gr(sum, 0)) {
            float[][] additions = new float[numValues][numClasses];
            ...
            float[][] newTable = new float[numValues][numClasses];
            for (int i = 0; i < numValues; i++) {
              for (int j = 0; j < numClasses; j++) {
    ...
    // Compute info gains
    m_InfoGains = new float[data.numAttributes()];
    for (int i = 0; i < data.numAttributes(); i++) {
      if (i != classIndex) {
        m_InfoGains[i] = (float) (ContingencyTables.entropyOverColumns(counts[i]) - ContingencyTables
