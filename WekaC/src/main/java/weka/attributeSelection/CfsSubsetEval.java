...
public class CfsSubsetEval extends ASEvaluation implements SubsetEvaluator,
  ...
  private float[] m_std_devs;
  /** Threshold for admitting locally predictive features */
  private float m_c_Threshold;
  ...
  public String globalInfo() {
    return new StringBuilder("CfsSubsetEval :\n\nEvaluates the worth of a subset of attributes ")
      .append("by considering the individual predictive ability of each feature ")
    	.append("along with the degree of redundancy between them.\n\n")
    	.append("Subsets of features that are highly correlated with the class ")
    	.append("while having low intercorrelation are preferred.\n\n")
  ...
  public String preComputeCorrelationMatrixTipText() {
    return new StringBuilder("Precompute the full correlation matrix at the outset, ")
    	.append("rather than compute correlations lazily (as needed) ")
    	.append("during the search. Use this in conjuction with ")
    	.append("parallel processing in order to speed up a backward search.").toString();
  }
  ...
  public String locallyPredictiveTipText() {
    return new StringBuilder("Identify locally predictive attributes. Iteratively adds ")
      .append("attributes with the highest correlation with the class as long ")
      .append("as there is not already an attribute in the subset that has a ")
      .append("higher correlation with the attribute in question").toString();
  }
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    m_std_devs = new float[m_numAttribs];
    ...
    if (m_preComputeCorrelationMatrix && m_poolSize > 1) {
      ...
      if(numEntriesPerThread < 1)
    	  numEntriesPerThread =  1;
                ...
                if (m_debug) {
                  System.err
                    .println("Percentage of correlation matrix computed: "
                      + Utils.doubleToString(((float) m_numFilled.get()
                        / m_numEntries * 100.0f), 2) + "%");
                }
   ...
   */
  @Override
  public float evaluateSubset(BitSet subset) throws Exception {
    float num = 0.0f;
    float denom = 0.0f;
    float corr;
    int larger, smaller;
    ...
    float merit = (float) (num / Math.sqrt(denom));
  ...
  private float symmUncertCorr(int att1, int att2) {
    int i, j, ii, jj;
    int ni, nj;
    float sum = 0.0f;
    float sumi[], sumj[];
    ...
    if (!m_missingSeparate && (sumi[ni - 1] < m_numInstances)
      && (sumj[nj - 1] < m_numInstances)) {
    	float[] i_copy = new float[sumi.length];
    	float[] j_copy = new float[sumj.length];
    	float[][] counts_copy = new float[sumi.length][sumj.length];
  ...
  private float num_num(int att1, int att2) {
    int i;
    Instance inst;
    float r, diff1, diff2, num = 0.0f, sx = 0.0f, sy = 0.0f;
    float mx = m_trainInstances.meanOrMode(m_trainInstances.attribute(att1));
    float my = m_trainInstances.meanOrMode(m_trainInstances.attribute(att2));
    ...
    for (i = 0; i < m_numInstances; i++) {
      ...
      if((inst.isMissing(att1)))
    	  ...
    	  diff1 = (float) (inst.value(att1) - mx);
      if(inst.isMissing(att2))
    	  diff2 =  0.0f;
      else
    	  diff2 =  (float) (inst.value(att2) - my);
      num += (diff1 * diff2);
      sx += (diff1 * diff1);
      sy += (diff2 * diff2);
    }
    ...
    if (sx != 0.0) {
      if (m_std_devs[att1] == 1.0) {
        m_std_devs[att1] = (float) Math.sqrt((sx / m_numInstances));
      }
    }

    if (sy != 0.0) {
      if (m_std_devs[att2] == 1.0) {
        m_std_devs[att2] = (float) Math.sqrt((sy / m_numInstances));
      }
    }

    if ((sx * sy) > 0.0) {
      r = (float) (num / (Math.sqrt(sx * sy)));
      return ((r < 0.0) ? -r : r);
    } else {
      if (att1 != m_classIndex && att2 != m_classIndex) {
        return 1.0f;
  ...
  private float num_nom2(int att1, int att2) {
    int i, ii, k;
    float temp;
    Instance inst;
    int mx =
    ...
    int nx =
      ...
      float[] prior_nom = new float[nx];
      float[] stdvs_nom = new float[nx];
      float[] covs = new float[nx];

    for (i = 0; i < nx; i++) {
      stdvs_nom[i] = covs[i] = prior_nom[i] = 0.0f;
    }
    ...
    for (i = 0, temp = 0.0f; i < nx; i++) {
      ...
      if ((stdvs_nom[i] * stdv_num) > 0.0) {
        ...
        rr = (float) (covs[i] / (Math.sqrt(stdvs_nom[i] * stdv_num)));
    ...
    if (temp != 0.0) {
      if (m_std_devs[att1] == 1.0) {
        m_std_devs[att1] = (float) Math.sqrt(temp);
      }
    }

    if (stdv_num != 0.0) {
      if (m_std_devs[att2] == 1.0) {
        m_std_devs[att2] = (float) Math.sqrt((stdv_num / m_numInstances));
      }
    }
  ...
  private float nom_nom(int att1, int att2) {
    int i, j, ii, jj, z;
    float temp1, temp2;
    Instance inst;
    int mx =
    ...
    int ny =
      ...
      float[][] prior_nom = new float[nx][ny];
      float[] sumx = new float[nx];
      float[] sumy = new float[ny];
      float[] stdvsx = new float[nx];
      float[] stdvsy = new float[ny];
      float[][] covs = new float[nx][ny];

    for (i = 0; i < nx; i++) {
      sumx[i] = stdvsx[i] = 0.0f;
    }

    for (j = 0; j < ny; j++) {
      sumy[j] = stdvsy[j] = 0.0f;
    }

    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        covs[i][j] = prior_nom[i][j] = 0.0f;
      }
    }
    ...
    for (i = 0; i < nx; i++) {
      for (j = 0; j < ny; j++) {
        if ((stdvsx[i] * stdvsy[j]) > 0.0) {
          // System.out.println("Stdv :"+stdvs_nom[i]);
          rr = (float) (covs[i][j] / (Math.sqrt(stdvsx[i] * stdvsy[j])));
    ...
    // (if necessary)
    for (i = 0, temp1 = 0.0f; i < nx; i++) {
      temp1 += ((sumx[i] / m_numInstances) * (stdvsx[i] / m_numInstances));
    }

    if (temp1 != 0.0) {
      if (m_std_devs[att1] == 1.0) {
        m_std_devs[att1] = (float) Math.sqrt(temp1);
      }
    }

    for (j = 0, temp2 = 0.0f; j < ny; j++) {
      temp2 += ((sumy[j] / m_numInstances) * (stdvsy[j] / m_numInstances));
    }

    if (temp2 != 0.0) {
      if (m_std_devs[att2] == 1.0) {
        m_std_devs[att2] = (float) Math.sqrt(temp2);
      }
    }
    ...
    if (m_debug) {
      System.err.println("Percentage of correlation matrix computed "
        ...
        + Utils.doubleToString(
          ((float) m_numFilled.get() / m_numEntries * 100.0f), 2) + "%");
    }
