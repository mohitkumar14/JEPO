...
public class PrincipalComponents extends UnsupervisedAttributeEvaluator
  ...
  private float[] m_means;
  private float[] m_stdDevs;
   ...
   * data
   */
  private float[][] m_eigenvectors;

  /** Eigenvalues for the corresponding eigenvectors */
  private float[] m_eigenvalues = null;

  /** Sorted eigenvalues */
  private int[] m_sortedEigens;

  /** sum of the eigenvalues */
  private float m_sumOfEigenValues = 0.0f;
   ...
   * best n PC's
   */
  private float m_coverVariance = 0.95f;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      Float temp;
      temp = Float.valueOf(optionString);
      setVarianceCovered(temp.floatValue());
    }
  ...
  private void resetOptions() {
    m_coverVariance = 0.95f;
    m_sumOfEigenValues = 0.0f;
   ...
   */
  public void setVarianceCovered(float vc) {
    m_coverVariance = vc;
  }
   ...
   */
  public float getVarianceCovered() {
  ...
  public String[] getOptions() {
    ...
    options.add("-R");
    options.add(String.valueOf(getVarianceCovered()));

    options.add("-A");
    options.add(String.valueOf(getMaximumAttributeNames()));
  ...
  private void buildAttributeConstructor(Instances data) throws Exception {
    ...
    if (m_transBackToOriginal) {
      ...
      float[][] orderedVectors =
        new float[m_eigenvectors.length][numVectors + 1];
   ...
   */
  public float[][] getCorrelationMatrix() {
   ...
   * @return the unsorted eigenvectors
   */
  public float[][] getUnsortedEigenVectors() {
   ...
   */
  @Override
  public float evaluateAttribute(int att) throws Exception {
    if (m_eigenvalues == null) {
      throw new Exception("Principal components hasn't been built yet!");
    }

    if (m_transBackToOriginal) {
      return 1.0f; // can't evaluate back in the original space!
    }
    ...
    return 1.0f - cumulative / m_sumOfEigenValues;
  }

  private void fillCovariance() throws Exception {
    // first store the means
    m_means = new float[m_trainInstances.numAttributes()];
    m_stdDevs = new float[m_trainInstances.numAttributes()];
    for (int i = 0; i < m_trainInstances.numAttributes(); i++) {
  ...
  private String principalComponentsSummary() {
    ...
    if(m_center)
    	...
    	corrCov ="Correlation ";
    result
      .append(corrCov).append("matrix\n").append(matrixToString(Matrices.getArray(m_correlation))).append( "\n\n");
    result.append("eigenvalue\tproportion\tcumulative\n");
    for (int i = m_numAttribs - 1; i > (m_numAttribs - numVectors - 1); i--) {
      cumulative += m_eigenvalues[m_sortedEigens[i]];
      result.append(Utils
        .doubleToString(m_eigenvalues[m_sortedEigens[i]], 9, 5)).append("\t")
      .append(Utils.doubleToString(
          (m_eigenvalues[m_sortedEigens[i]] / m_sumOfEigenValues), 9, 5))
      .append("\t")
      .append(Utils.doubleToString((cumulative / m_sumOfEigenValues), 9, 5))
      .append( "\t")
      .append( output.attribute(m_numAttribs - i - 1).name()).append("\n");
    }

    result.append("\nEigenvectors\n");
    for (int j = 1; j <= numVectors; j++) {
      result.append(" V").append(j).append( '\t');
    }
    result.append("\n");
    for (int j = 0; j < m_numAttribs; j++) {
      ...
      for (int i = m_numAttribs - 1; i > (m_numAttribs - numVectors - 1); i--) {
        result.append(Utils.doubleToString(
          m_eigenvectors[j][m_sortedEigens[i]], 7, 4)).append("\t");
      }
      result.append(m_trainInstances.attribute(j).name()).append('\n');
    }
  ...
  public static String matrixToString(float[][] matrix) {
    ...
    for (int i = 0; i <= last; i++) {
      for (int j = 0; j <= last; j++) {
        result.append(Utils.doubleToString(matrix[i][j], 6, 2)).append(" ");
        if (j == last) {
          result.append('\n');
        }
      }
  ...
  private Instance convertInstanceToOriginal(Instance inst) throws Exception {
    ...
    if (m_hasClass) {
      newVals = new float[m_numAttribs + 1];
    } else {
      newVals = new float[m_numAttribs];
    }
  ...
  public Instance convertInstance(Instance instance) throws Exception {
    ...
    float[] newVals = new float[m_outputNumAtts];
  ...
  private Instances setOutputFormat() throws Exception {
    ...
    for (int i = m_numAttribs - 1; i >= 0; i--) {
      ...
      float[] coeff_mags = new float[m_numAttribs];
      for (int j = 0; j < m_numAttribs; j++) {
      ...
      for (int j = 0; j < num_attrs; j++) {
        float coeff_value = m_eigenvectors[coeff_inds[j]][m_sortedEigens[i]];
