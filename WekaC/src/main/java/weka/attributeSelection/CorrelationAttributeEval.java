...
public class CorrelationAttributeEval extends ASEvaluation implements
  ...
  protected float m_correlations[];
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    m_correlations = new float[data.numAttributes()];
    /*
    ...
    float[][][] nomAtts = new float[data.numAttributes()][][];
    for (int i = 0; i < data.numAttributes(); i++) {
      if (data.attribute(i).isNominal() && i != classIndex) {
        nomAtts[i] = new float[data.attribute(i).numValues()][data
          .numInstances()];
        Arrays.fill(nomAtts[i][0], 1.0f); // set zero index for this att to all
                                         // 1's
    ...
    if (data.classAttribute().isNumeric()) {
    	float[] classVals = data.attributeToDoubleArray(classIndex);

      // do the numeric attributes
      for (Integer i : numericIndexes) {
    	  float[] numAttVals = data.attributeToDoubleArray(i);
      ...
      float[][] binarizedClasses = new float[data.classAttribute()
        .numValues()][data.numInstances()];

      // this is equal to the number of instances for all inst weights = 1
      float[] classValCounts = new float[data.classAttribute().numValues()];
      ...
      float sumClass = Utils.sum(classValCounts);
      ...
      if (nominalIndexes.size() > 0) {
        for (Integer i : nominalIndexes) {
          ...
          float corrForAtt = 0;
          for (int j = 0; j < data.attribute(i).numValues(); j++) {
        	  float sumForValue = Utils.sum(nomAtts[i][j]);
        	  float corr = 0;
        	  float sumCorr = 0;
        	  float avgCorrForValue = 0;
          ...
          if(sumForAtt > 0)
        	  m_correlations[i] = corrForAtt / sumForAtt;
          else
        	  m_correlations[i] = 0;
        }
      }
