...
public class AttributeSelection implements Serializable, RevisionHandler {
  ...
  private float[][] m_attributeRanking;
   ...
   * validation
   */
  private float[][] m_rankResults = null;
  private float[] m_subsetResults = null;
   ...
   */
  public float[][] rankedAttributes() throws Exception {
  ...
  public static String SelectAttributes(ASEvaluation ASEvaluator,
    ...
    String[] optionsTmp = new String[options.length];
    System.arraycopy(options, 0, optionsTmp,  
            0, options.length);
  ...
  public String CVResultsString() throws Exception {
    ...
    CvString.append("\n\n=== Attribute selection ").append(m_numFolds)
    .append(" fold cross-validation ");

    if (!(m_ASEvaluator instanceof UnsupervisedSubsetEvaluator)
      && !(m_ASEvaluator instanceof UnsupervisedAttributeEvaluator)
      && (m_trainInstances.classAttribute().isNominal())) {
      CvString.append("(stratified), seed: ");
      CvString.append(m_seed).append(" ===\n\n");
    } else {
      CvString.append("seed: ").append(m_seed).append(" ===\n\n");
    }
    ...
    if ((m_searchMethod instanceof RankedOutputSearch) && (m_doRank == true)) {
      ...
      for (int i = 0; i < m_rankResults[0].length; i++) {
        ...
        float var = m_rankResults[0][i] * m_rankResults[0][i] * m_numFolds;
        ...
        if (var <= 0.0) {
          ...
          m_rankResults[2][i] = (float) Math.sqrt(var);
        }
        ...
        if (var <= 0.0) {
          ...
          m_rankResults[3][i] = (float) Math.sqrt(var);
        }
      }
      ...
      for (int element : s) {
        if (m_rankResults[1][element] > 0) {
          ...
          .append(" +-")
          .append( Utils.doubleToString(m_rankResults[2][element], 6, 3))
          .append( "   ")
          .append( Utils
              .doubleToString(m_rankResults[1][element], fieldWidth + 2, 1))
          .append( " +-").append(Utils.doubleToString(m_rankResults[3][element], 5, 2))
          .append( "  ").append(Utils.doubleToString((element + 1), fieldWidth, 0)).append( " ")
          .append( m_trainInstances.attribute(element).name()) .append("\n");
        }
      }
    } else {
      ...
      for (int i = 0; i < m_subsetResults.length; i++) {
        if ((m_ASEvaluator instanceof UnsupervisedSubsetEvaluator)
          ...
          .append("(")
          .append( Utils.doubleToString((m_subsetResults[i] / m_numFolds * 100.0f),
              3, 0)).append(" %)  ").append(Utils.doubleToString((i + 1), fieldWidth, 0))
          .append(" ").append(m_trainInstances.attribute(i).name()).append("\n");
        }
      }
    }
  ...
  public void updateStatsForModelCVSplit(Instances split,
    ...
    if (m_rankResults == null && m_subsetResults == null) {
      m_subsetResults = new float[split.numAttributes()];
      m_rankResults = new float[4][split.numAttributes()];
    }
  ...
  public void SelectAttributes(Instances data) throws Exception {
    ...
    if ((m_searchMethod instanceof RankedOutputSearch) && m_doRank == true) {
      ...
      for (int i = 0; i < m_numToSelect; i++) {
        ...
        float intPart = (int) (Math.abs(m_attributeRanking[i][1]));

        if (precision > 0) {
          precision =
            (float) (Math.abs((Math.log(Math.abs(precision)) / Math.log(10))) + 3);
        }
        if (precision > f_p) {
          f_p = (int) precision;
        }
      ...
      for (int i = 0; i < m_numToSelect; i++) {
        ...
        .append( Utils.doubleToString((m_attributeRanking[i][0] + 1),
            fieldWidth + 1, 0))
        .append(" ")
        .append( m_trainInstances.attribute((int) m_attributeRanking[i][0]).name())
        .append( "\n");
      }
      ...
      for (int i = 0; i < attributeSet.length; i++) {
        if (i == (attributeSet.length - 1)) {
          m_selectionResults.append((attributeSet[i] + 1)).append(" : ")
          .append(attributeSet.length).append("\n");
        } else {
          m_selectionResults.append(attributeSet[i] + 1).append( ",");
        }
      }
  ...
  private String printSelectionResults() {
    ...
    if (!(m_ASEvaluator instanceof UnsupervisedSubsetEvaluator)
      ...
      text.append(m_trainInstances.classIndex() + 1).append(" ")
      .append(m_trainInstances.attribute(m_trainInstances.classIndex()).name())
      .append("):\n");
    } else {
      text.append("unsupervised):\n");
    }

    text.append(m_ASEvaluator.toString()).append("\n");
  ...
  private static String makeOptionString(ASEvaluation ASEvaluator,
    ...
    if (ASEvaluator instanceof OptionHandler) {
      optionsText.append("\nOptions specific to ")
      .append(ASEvaluator.getClass().getName()).append(":\n\n");
    ...
    if (searchMethod != null) {
      if (searchMethod instanceof OptionHandler) {
        optionsText.append("\nOptions specific to ")
        .append(searchMethod.getClass().getName()).append(":\n\n");
