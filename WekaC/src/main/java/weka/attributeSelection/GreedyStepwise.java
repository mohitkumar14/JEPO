...
public class GreedyStepwise extends ASSearch implements RankedOutputSearch,
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(new StringBuilder("\t").append( numExecutionSlotsTipText())
    		.append(" (default 1)\n").toString(), "-num-slots", 1, "-num-slots <int>"));
  ...
  public String[] getOptions() {
    ...
    options.add("-T");
    options.add(String.valueOf(getThreshold()));

    options.add("-N");
    options.add(String.valueOf(getNumToSelect()));

    options.add("-num-slots");
    options.add(String.valueOf(getNumExecutionSlots()));
  ...
  public String toString() {
    StringBuffer FString = new StringBuffer();
    if(m_backward)
    	FString.append("\tGreedy Stepwise (backwards).\n\tStart set: ");
    else
    	FString.append("\tGreedy Stepwise (forwards).\n\tStart set: ");
    ...
    if (m_starting == null) {
      ...
      FString.append(startSetToString()).append("\n");
    }
    if (!m_doneRanking) {
      FString.append("\tMerit of best subset found: ")
      .append( Utils.doubleToString(Math.abs(m_bestMerit), 8, 3)).append("\n");
    } else {
      if (m_backward) {
        FString
    ...
    if ((m_threshold != -Float.MAX_VALUE) && (m_doneRanking)) {
      FString.append("\tThreshold for discarding attributes: ")
      .append(Utils.doubleToString(m_threshold, 8, 4)).append("\n");
    }
  ...
  public int[] search(ASEvaluation ASEval, Instances data) throws Exception {
    ...
    int i;
    float best_merit = -Float.MAX_VALUE;
    float temp_best, temp_merit;
    int temp_index = 0;
    BitSet temp_group;
    ...
    if (m_rankedAtts == null) {
      m_rankedAtts = new float[m_numAttribs][2];
      m_rankedSoFar = 0;
    }
    ...
    while (!done) {
      ...
      for (i = 0; i < m_numAttribs; i++) {
        ...
        if (z) {
          ...
          if (parallel) {
            ...
            Future<Float[]> future = m_pool.submit(new Callable<Float[]>() {
              @Override
              public Float[] call() throws Exception {
                Float[] r = new Float[2];
                float e = theEvaluator.evaluateSubset(tempCopy);
                r[0] = new Float(attBeingEvaluated);
                r[1] = e;
                return r;
              }
            });
      ...
      if (parallel) {
        for (int j = 0; j < results.size(); j++) {
          Future<Float[]> f = results.get(j);

          int index = f.get()[0].intValue();
          temp_merit = f.get()[1].floatValue();
  ...
  public float[][] rankedAttributes() throws Exception {
    ...
    float[][] final_rank = new float[m_rankedSoFar][2];
  ...
  private void determineNumToSelectFromThreshold(float[][] final_rank) {
    int count = 0;
    for (float[] element : final_rank) {
