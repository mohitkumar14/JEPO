...
public class ClassifierSubsetEval extends HoldOutSubsetEvaluator implements
   ...
   */
  public float evaluateSubset(BitSet subset, Instances holdOut)
    throws Exception {
    int i, j;
    float evalMetric = 0;
    int numAttributes = 0;
  ...
  public float
    ...
    if (m_trainingInstances.classAttribute().isNominal()) {
      ...
      error = (float) (testCopy.classValue() - pred);
    }
  ...
  public String toString() {
    ...
    if (m_trainingInstances == null) {
      ...
      text.append("\tLearning scheme: ").append(getClassifier().getClass().getName()).append("\n");
      ...
      if (!m_useTraining) {
        ...
        if (m_usePercentageSplit) {
          text.append("Percentage split: ").append(m_splitPercent).append("\n");
        } else {
      ...
      if (m_IRClassVal >= 0) {
        IRClassL =
        		new StringBuilder("(class value: ")
        		.append(m_trainingInstances.classAttribute().value(m_IRClassVal)).append(")").toString();
      }
      switch (m_evaluationMeasure.getID()) {
      case EVAL_DEFAULT:
      case EVAL_ACCURACY:
      ...
      case EVAL_AUPRC:
    	  if(m_IRClassVal >= 0)
    		  text.append("\tSubset evalation: area under the precision-recal curve ").append(IRClassL).append("\n");
    	  else
      ...
      default:
        ...
        if (((WrapperSubsetEval.PluginTag) m_evaluationMeasure).getMetric() instanceof InformationRetrievalEvaluationMetric) {
          text.append(" ").append(m_IRClassVal > 0 ? IRClassL : "");
        }
        text.append("\n");
        break;
      }
