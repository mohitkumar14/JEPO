...
public class WrapperSubsetEval extends ASEvaluation
  ...
  public Enumeration<Option> listOptions() {
    ...
    if ((m_BaseClassifier != null)
      ...
      newVector.addElement(new Option("", "", 0, new StringBuilder("\nOptions specific to scheme ")
        .append(m_BaseClassifier.getClass().getName()).append(":").toString()));
      newVector.addAll(
  ...
  public String[] getOptions() {
    ...
    options[current++] = "-F";
    options[current++] = String.valueOf(getFolds());
    options[current++] = "-T";
    options[current++] = String.valueOf(getThreshold());
    options[current++] = "-R";
    options[current++] = String.valueOf( getSeed());
  ...
  public String toString() {
    ...
    if (m_trainInstances == null) {
      ...
      text.append(
        "\tLearning scheme: ").append(getClassifier().getClass().getName()).append("\n");
      ...
      if (m_BaseClassifier instanceof OptionHandler) {
        ...
        for (String classifierOption : classifierOptions) {
          text.append(classifierOption).append(" ");
        }
      }
      ...
      String IRClassL = "";
      if (m_IRClassVal >= 0) {
        IRClassL = new StringBuilder("(class value: ")
          .append(m_trainInstances.classAttribute().value(m_IRClassVal)).append( ")").toString();
      }
      switch (m_evaluationMeasure.getID()) {
      case EVAL_DEFAULT:
      case EVAL_ACCURACY:
      ...
      case EVAL_FMEASURE:
    	  if(m_IRClassVal >= 0)
    		  text.append("\tSubset evaluation: F-measure " ).append(IRClassL ).append("\n");
    	  else
    		  text.append("\tSubset evaluation: F-measure \n");
        break;
      case EVAL_AUC:
    	  if(m_IRClassVal >= 0)
    		  text.append("\tSubset evaluation: area under the ROC curve " ).append(IRClassL).append("\n");
    	  else
    		  text.append("\tSubset evaluation: area under the ROC curve \n");
          break;
      case EVAL_AUPRC:
    	  if(m_IRClassVal >= 0)
    		  text.append("\tSubset evaluation: area under the precision-recall curve ").append(IRClassL).append("\n");
    	  else
    		  text.append("\tSubset evaluation: area under the precision-recall curve \n");
        break;
      case EVAL_CORRELATION:
      ...
      default:
        text
          .append("\tSubset evaluation: ").append( m_evaluationMeasure.getReadable());
        if (((PluginTag) m_evaluationMeasure)
          .getMetric() instanceof InformationRetrievalEvaluationMetric) {
        	if(m_IRClassVal > 0)
        		text.append(" ").append(IRClassL);
        	else
        		text.append(" ");
        }
        text.append("\n");
      ...
      text
        .append("\tNumber of folds for accuracy estimation: ").append(m_folds).append("\n");
    }
   ...
   */
  private boolean repeat(float[] repError, int entries) {
    int i;
    float mean = 0;
    float variance = 0;
