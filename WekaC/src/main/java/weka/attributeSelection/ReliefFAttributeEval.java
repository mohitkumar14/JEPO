...
public class ReliefFAttributeEval extends ASEvaluation implements
  ...
  private float[][][] m_karray;

  /** Upper bound for numeric attributes */
  private float[] m_maxArray;

  /** Lower bound for numeric attributes */
  private float[] m_minArray;
  ...
  public String[] getOptions() {
    ...
    options.add("-M");
    options.add(String.valueOf(getSampleSize()));
    options.add("-D");
    options.add(String.valueOf( getSeed()));
    options.add("-K");
    options.add(String.valueOf(getNumNeighbours()));

    if (getWeightByDistance()) {
      options.add("-A");
      options.add(String.valueOf(getSigma()));
    }
  ...
  public String toString() {
    ...
    if (m_trainInstances == null) {
      ...
      if (m_sampleM == -1) {
        ...
        text.append(m_sampleM).append("\n");
      }

      text.append("\tNumber of nearest neighbours (k): ").append(m_Knn).append("\n");

      if (m_weightByDistance) {
        text.append("\tExponentially decreasing (with distance) influence for\n\tnearest neighbours. Sigma:")
        .append( m_sigma).append("\n");
      } else {
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    if (!m_numericClass) {
      ...
      m_ndc = 0;
      m_numClasses = 1;
      m_nda = new float[m_numAttribs];
      m_ndcda = new float[m_numAttribs];
    }

    if (m_weightByDistance) // set up the rank based weights
    {
      m_weightsByRank = new float[m_Knn];

      for (int i = 0; i < m_Knn; i++) {
        m_weightsByRank[i] = (float) Math
          .exp(-((i /  m_sigma) * (i /  m_sigma)));
      }
    }

    // the final attribute weights
    m_weights = new float[m_numAttribs];
    // num classes (1 for numeric class) knn neighbours,
    // and 0 = distance, 1 = instance index
    m_karray = new float[m_numClasses][m_Knn][2];

    if (!m_numericClass) {
      m_classProbs = new float[m_numClasses];
    ...
    m_worst = new float[m_numClasses];
    m_index = new int[m_numClasses];
    m_stored = new int[m_numClasses];
    m_minArray = new float[m_numAttribs];
    m_maxArray = new float[m_numAttribs];

    for (int i = 0; i < m_numAttribs; i++) {
      m_minArray[i] = m_maxArray[i] = Float.NaN;
    }
   ...
   */
  @Override
  public float evaluateAttribute(int attribute) throws Exception {
   ...
   */
  private float norm(float x, int i) {
    if (Float.isNaN(m_minArray[i]) || Utils.eq(m_maxArray[i], m_minArray[i])) {
      return 0;
    } else {
   ...
   */
  private float difference(int index, float d, float e) {

    switch (m_trainInstances.attribute(index).type()) {
    case Attribute.NOMINAL:

      // If attribute is nominal
      if (Utils.isMissingValue(d) || Utils.isMissingValue(e)) {
        return (1.0f - (1.0f / (m_trainInstances.attribute(index).numValues())));
      } else if ((int) d != (int) e) {
        return 1;
      } else {
        return 0;
    ...
    case Attribute.NUMERIC:
      ...
      if (Utils.isMissingValue(d) || Utils.isMissingValue(e)) {
        ...
        return Math.abs(norm(d, index) - norm(e, index));
      }
    default:
      return 0;
    }
   ...
   */
  private float distance(Instance first, Instance second) {
  ...
  private void updateWeightsNumericClass(int instNum) {
    ...
    for (i = 0; i < m_stored[0]; i++) {
      ...
      if (m_weightByDistance) {
        temp = difference(
          m_classIndex,
          (float)inst.value(m_classIndex),
          (float)m_trainInstances.instance((int) m_karray[0][tempSorted[i]][1]).value(
            m_classIndex));
        temp *= (m_weightsByRank[i] / distNorm);
      } else {
        temp = difference(m_classIndex, (float)inst.value(m_classIndex),
        		(float)m_trainInstances.instance((int) m_karray[0][i][1])
            .value(m_classIndex));
      ...
      float temp_diffP_diffA_givNearest = difference(m_classIndex,
        (float)inst.value(m_classIndex), (float)cmp.value(m_classIndex));
      // now the attributes
  ...
  private void updateWeightsDiscreteClass(int instNum) {
    int i, j, k;
    int cl;
    float temp_diff, w_norm = 1.0f;
    float[] tempDistClass;
    int[] tempSortedClass = null;
    float distNormClass = 1.0f;
    float[] tempDistAtt;
    int[][] tempSortedAtt = null;
    float[] distNormAtt = null;
    int firstI, secondI;
    ...
    if (m_weightByDistance) {
      // do class (hits) first
      // sort the distances
      tempDistClass = new float[m_stored[cl]];
      ...
      distNormAtt = new float[m_numClasses];

      for (k = 0; k < m_numClasses; k++) {
        if (k != cl) // already done cl
        {
          // sort the distances
          tempDistAtt = new float[m_stored[k]];
    ...
    if (m_numClasses > 2) {
      ...
      w_norm = (1.0f - m_classProbs[cl]);
    }

    // do the k nearest hits of the same class
    for (j = 0, temp_diff = 0.0f; j < m_stored[cl]; j++) {
      Instance cmp;
      cmp = (m_weightByDistance) ? m_trainInstances
    ...
    for (i = 0; i < m_numInstances; i++) {
      if (i != instNum) {
        ...
        if (m_stored[cl] < m_Knn) {
          ...
          for (j = 0, ww = -1.0f; j < m_stored[cl]; j++) {
