...
public class ClassifierAttributeEval extends ASEvaluation implements
  ...
  public String[] getOptions() {
    ...
    result.add(String.valueOf(m_executionSlots));
  ...
  public void buildEvaluator(final Instances data) throws Exception {
    ...
    float baseMerit = 0;
    m_merit = new float[data.numAttributes()];
    ...
    for (int i = -1; i < data.numAttributes(); i++) {
      if (i != data.classIndex()) {
        ...
        Future<float[]> futureEval = m_pool.submit(new Callable<float[]>() {
          @Override
  ...
  public String toString() {
    ...
    if (m_trainInstances == null) {
      text.append("\tClassifier feature evaluator has not been built yet");
    	...
    	if(m_leaveOneOut)
    		text.append("\tClassifier feature evaluator (leave one out)\n\n");
    	else
