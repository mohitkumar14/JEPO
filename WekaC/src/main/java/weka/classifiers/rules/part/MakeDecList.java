...
public class MakeDecList implements Serializable, RevisionHandler {
   ...
   */
  public MakeDecList(ModelSelection toSelectLocModel, float cf, int minNum) {
  ...
  public String toString() {
    ...
    for (int i = 0; i < theRules.size(); i++) {
      text.append(theRules.elementAt(i)).append("\n");
    }
    text.append("Number of Rules  : \t").append(theRules.size()).append("\n");
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] currentProbs = null;
    float[] sumProbs;
    float currentWeight, weight = 1;
    int i, j;

    // Get probabilities.
    sumProbs = new float[instance.numClasses()];
    i = 0;
