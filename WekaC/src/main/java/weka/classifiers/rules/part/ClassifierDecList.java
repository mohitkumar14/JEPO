...
public class ClassifierDecList implements Serializable, RevisionHandler {
   ...
   * @exception Exception if something goes wrong
   */
  public float weight(Instance instance) throws Exception {
  ...
  public String toString() {
    ...
    try {
      ...
      if (m_isLeaf) {
        text.append(": ");
        text.append(m_localModel.dumpLabel(0, m_train)).append("\n");
      } else {
        dumpDecList(text);
        // dumpTree(0,text);
      }
   ...
   */
  protected float getSizeOfBranch() {
  ...
  private void dumpDecList(StringBuffer text) throws Exception {
    ...
    if (m_sons[indeX].m_isLeaf) {
      text.append(": ");
      text.append(m_localModel.dumpLabel(indeX, m_train)).append("\n");
    } else {
      text.append(" AND\n");
  ...
  private float getProbs(int classIndex, Instance instance, float weight)
    ...
    if (m_isLeaf) {
      return localModel().classProb(classIndex, instance, -1);
    } else {
