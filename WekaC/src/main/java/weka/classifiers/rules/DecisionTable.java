...
public class DecisionTable extends AbstractClassifier implements OptionHandler,
  ...
  protected Hashtable<DecisionTableHashKey, float[]> m_entries;

  /** The class priors to use when there is no match in the table */
  protected float[] m_classPriorCounts;
   ...
   */
  private void insertIntoTable(Instance inst, float[] instA) throws Exception {

    float[] tempClassDist2;
    float[] newDist;
    ...
    if (tempClassDist2 == null) {
      if (m_classIsNominal) {
        newDist = new float[m_theInstances.classAttribute().numValues()];
   ...
   */
  protected float evaluateInstanceLeaveOneOut(Instance instance, float[] instA)
    throws Exception {
    ...
    if (m_classIsNominal) {
      ...
      if ((tempDist = m_entries.get(thekey)) == null) {
        ...
        normDist = new float[tempDist.length];
        ...
        boolean ok = false;
        for (float element : normDist) {
          if (Utils.gr(element, 1.0f)) {
            ok = true;
            break;
          }
        }
        ...
        float[] classPriors = new float[m_classPriorCounts.length];
        System.arraycopy(m_classPriorCounts, 0, classPriors, 0, m_classPriorCounts.length);
   ...
   * @throws Exception if something goes wrong
   */
  protected float evaluateFoldCV(Instances fold, int[] fs) throws Exception {

    int i;
    int numFold = fold.numInstances();
    int numCl = m_theInstances.classAttribute().numValues();
    float[][] class_distribs = new float[numFold][numCl];
    float[] instA = new float[fs.length];
    float[] normDist;
    DecisionTableHashKey thekey;
    float acc = 0.0f;
    ...
    if (m_classIsNominal) {
      normDist = new float[numCl];
    } else {
      normDist = new float[2];
    }
    ...
    for (i = 0; i < numFold; i++) {
      ...
      for (int j = 0; j < fs.length; j++) {
        if (fs[j] == classI) {
          instA[j] = Float.MAX_VALUE; // missing for the class
        } else if (inst.isMissing(fs[j])) {
          instA[j] = Float.MAX_VALUE;
        } else {
    ...
    for (i = 0; i < numFold; i++) {
      ...
      if (m_classIsNominal) {
        ...
        if (!ok) { // majority class
          normDist = new float[classPriors.length];
          System.arraycopy(classPriors, 0, normDist, 0, classPriors.length);
        }
   ...
   */
  protected float estimatePerformance(BitSet feature_set, int num_atts)
    throws Exception {
    ...
    float[] instA = new float[num_atts];
    int classI = m_theInstances.classIndex();
    ...
    m_entries = new Hashtable<DecisionTableHashKey, float[]>(
    ...
    case EVAL_AUC:
      float[] classPriors = m_evaluation.getClassPriors();
      Utils.normalize(classPriors);
      float weightedAUC = 0;
      for (i = 0; i < m_theInstances.classAttribute().numValues(); i++) {
        float tempAUC = m_evaluation.areaUnderROC(i);
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("", "", 0,
    		new StringBuilder("\nOptions specific to search method ").append(m_search.getClass().getName())
    		.append(":").toString()));
    newVector
  ...
  public String[] getOptions() {
    ...
    options.add("-X");
    options.add(String.valueOf(m_CVFolds));
    ...
    options.add("-S");
    options.add(String.valueOf(getSearchSpec()));
  ...
  protected String getSearchSpec() {
    ...
    if (s instanceof OptionHandler) {
      return new StringBuilder(s.getClass().getName()).append(" ")
    		  .append(Utils.joinOptions(((OptionHandler) s).getOptions())).toString();
    }
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_theInstances.classAttribute().isNominal()) {// Set up class priors
      m_classPriorCounts = new float[data.classAttribute().numValues()];
      Arrays.fill(m_classPriorCounts, 1.0f);
      for (int i = 0; i < data.numInstances(); i++) {
        Instance curr = data.instance(i);
        m_classPriorCounts[(int) curr.classValue()] += curr.weight();
      }
      m_classPriors = new float[m_classPriorCounts.length];
      System.arraycopy(m_classPriorCounts, 0, m_classPriors, 0, m_classPriorCounts.length);
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    if ((tempDist = m_entries.get(thekey)) == null) {
      if (m_useIBk) {
        ...
        if (!m_classIsNominal) {
          tempDist = new float[1];
          tempDist[0] = m_majority;
        } else {
          tempDist = new float[m_classPriors.length];
          System.arraycopy(m_classPriors, 0, tempDist, 0, m_classPriors.length);
          /*
           * tempDist = new double
  ...
  public String printFeatures() {
    ...
    for (i = 0; i < m_decisionFeatures.length; i++) {
      if (i == 0) {
        s = String.valueOf(m_decisionFeatures[i] + 1);
      } else {
  ...
  public String toString() {
    ...
    if (m_entries == null) {
      ...
      .append(m_numInstances).append("\nNumber of Rules : ").append(m_entries.size()).append("\n");
      ...
      if (m_CVFolds > 1) {
        text.append("(").append(m_CVFolds).append(" fold) ");
      } else {
        text.append("(leave one out) ");
      }
      text.append("\nFeature set: ").append(printFeatures());
      ...
      if (m_displayRules) {
        ...
        tm.append(m_dtInstances.attribute(m_dtInstances.classIndex()).name()).append("  ");
        ...
        while (e.hasMoreElements()) {
          ...
          float[] ClassDist = m_entries.get(tt);

          if (m_classIsNominal) {
            int m = Utils.maxIndex(ClassDist);
            try {
              text.append(m_dtInstances.classAttribute().value(m)).append("\n");
            } catch (Exception ee) {
              System.out.println(ee.getMessage());
            }
          } else {
            text.append((ClassDist[0] / ClassDist[1])).append("\n");
          }
        }
