...
public class RuleStats implements Serializable, RevisionHandler {
  ...
  private ArrayList<float[]> m_SimpleStats;
  ...
  private static float REDUNDANCY_FACTOR = 0.5f;

  /** The theory weight in the MDL calculation */
  private float MDL_THEORY_WEIGHT = 1.0f;

  /** The class distributions predicted by each rule */
  private ArrayList<float[]> m_Distributions;
   ...
   */
  public void setNumAllConds(float total) {
    if (total < 0) {
   ...
   * @return the stats
   */
  public float[] getSimpleStats(int index) {
   ...
   */
  public float[] getDistributions(int index) {
   ...
   */
  public void setMDLTheoryWeight(float weight) {
   ...
   */
  public static float numAllConditions(Instances data) {
    float total = 0;
  ...
  public void countData() {
    ...
    m_SimpleStats = new ArrayList<float[]>(size);
    m_Distributions = new ArrayList<float[]>(size);
    Instances data = new Instances(m_Data);

    for (int i = 0; i < size; i++) {
      float[] stats = new float[6]; // 6 statistics parameters
      float[] classCounts = new float[m_Data.classAttribute().numValues()];
   ...
   */
  public void countData(int index, Instances uncovered, float[][] prevRuleStats) {
  ...
  public void addAndUpdate(Rule lastRule) {
    ...
    if (m_SimpleStats == null) {
      m_SimpleStats = new ArrayList<float[]>();
    }
    m_SimpleStats.add(stats);

    if (m_Distributions == null) {
      m_Distributions = new ArrayList<float[]>();
    }
    m_Distributions.add(classCounts);
  }
   ...
   */
  public static float subsetDL(float t, float k, float p) {
    float rt = Utils.gr(p, 0.0f) ? (-k * Utils.log2(p)) : 0.0f;
   ...
   */
  public float theoryDL(int index) {

    float k = m_Ruleset.get(index).size();

    if (k == 0) {
      return 0.0f;
    }

    float tdl = Utils.log2(k);
    if (k > 1) {
   ...
   */
  public static float dataDL(float expFPOverErr, float cover,
    float uncover, float fp, float fn) {
    float totalBits = Utils.log2(cover + uncover + 1.0f); // how many data?
    float coverBits, uncoverBits; // What's the error?
    float expErr; // Expected FP or FN

    if (Utils.gr(cover, uncover)) {
      expErr = expFPOverErr * (fp + fn);
      coverBits = subsetDL(cover, fp, expErr / cover);
      uncoverBits = Utils.gr(uncover, 0.0f) ? subsetDL(uncover, fn, fn / uncover)
        : 0.0f;
    } else {
      expErr = (1.0f - expFPOverErr) * (fp + fn);
      if(Utils.gr(cover, 0.0f))
    	  coverBits = subsetDL(cover, fp, fp / cover);
      else
    	  coverBits =  0.0f;  
   ...
   */
  public float potential(int index, float expFPOverErr, float[] rulesetStat,
    float[] ruleStat, boolean checkErr) {
    // System.out.println("!!!inside potential: ");
    // Restore the stats if deleted
    float pcov = rulesetStat[0] - ruleStat[0];
    float puncov = rulesetStat[1] + ruleStat[0];
    float pfp = rulesetStat[4] - ruleStat[4];
    float pfn = rulesetStat[5] + ruleStat[2];

    float dataDLWith = dataDL(expFPOverErr, rulesetStat[0], rulesetStat[1],
      rulesetStat[4], rulesetStat[5]);
    float theoryDLWith = theoryDL(index);
    float dataDLWithout = dataDL(expFPOverErr, pcov, puncov, pfp, pfn);

    float potential = dataDLWith + theoryDLWith - dataDLWithout;
    float err = ruleStat[4] / ruleStat[0];
    /*
     * System.out.println("!!!"+dataDLWith +" | "+ theoryDLWith + " | "
     * +dataDLWithout+"|"+ruleStat[4] + " / " + ruleStat[0]);
     */
    boolean overErr = Utils.grOrEq(err, 0.5f);
    if (!checkErr) {
      overErr = false;
    }

    if (Utils.grOrEq(potential, 0.0f) || overErr) {
   ...
   */
  public float minDataDLIfDeleted(int index, float expFPRate, boolean checkErr) {
    // System.out.println("!!!Enter without: ");
    float[] rulesetStat = new float[6]; // Stats of ruleset if deleted
    int more = m_Ruleset.size() - 1 - index; // How many rules after?
    ArrayList<float[]> indexPlus = new ArrayList<float[]>(more); // Their
                                                                   // stats
    ...
    for (int k = index + 1; k < m_Ruleset.size(); k++) {
      float[] ruleStat = indexPlus.get(k - index - 1);
      float ifDeleted = potential(k, expFPRate, rulesetStat, ruleStat,
        checkErr);
      if (!Float.isNaN(ifDeleted)) {
        potential += ifDeleted;
      }
    }
    ...
    float dataDLWithout = dataDL(expFPRate, rulesetStat[0], rulesetStat[1],
      rulesetStat[4], rulesetStat[5]);
   ...
   * @return the minDataDL
   */
  public float minDataDLIfExists(int index, float expFPRate, boolean checkErr) {
    // System.out.println("!!!Enter with: ");
    float[] rulesetStat = new float[6]; // Stats of ruleset if rule exists
    ...
    for (int k = index + 1; k < m_SimpleStats.size(); k++) {
      float[] ruleStat = getSimpleStats(k);
      float ifDeleted = potential(k, expFPRate, rulesetStat, ruleStat,
        checkErr);
      if (!Float.isNaN(ifDeleted)) {
        potential += ifDeleted;
    ...
    // if any potential
    float dataDLWith = dataDL(expFPRate, rulesetStat[0], rulesetStat[1],
      rulesetStat[4], rulesetStat[5]);
   ...
   */
  public float relativeDL(int index, float expFPRate, boolean checkErr) {
   ...
   * @param checkErr whether check if error rate >= 0.5
   */
  public void reduceDL(float expFPRate, boolean checkErr) {

    boolean needUpdate = false;
    float[] rulesetStat = new float[6];
    for (int j = 0; j < m_SimpleStats.size(); j++) {
      // Covered stats are cumulative
      rulesetStat[0] += m_SimpleStats.get(j)[0];
      rulesetStat[2] += m_SimpleStats.get(j)[2];
    ...
    for (int k = m_SimpleStats.size() - 1; k >= 0; k--) {
      ...
      float[] ruleStat = m_SimpleStats.get(k);
   ...
   */
  public float combinedDL(float expFPRate, float predicted) {
    float rt = 0;

    if (getRulesetSize() > 0) {
      float[] stats = m_SimpleStats.get(m_SimpleStats.size() - 1);
      ...
      rt += dataDL(expFPRate, 0.0f, m_Data.sumOfWeights(), 0.0f, fn);
    }
