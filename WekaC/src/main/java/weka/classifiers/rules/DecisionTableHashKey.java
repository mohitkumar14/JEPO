...
public class DecisionTableHashKey 
   ...
   * Constructor for a hashKey
   *
   * @param instA an array of feature values
   */
  public DecisionTableHashKey(float[] instA) {

    int i;
    int l = instA.length;

    key = -999;
    attributes = new float [l];
    missing = new boolean [l];
    for (i=0;i<l;i++) {
      if (instA[i] == Double.MAX_VALUE) {
        missing[i] = true;
      } else {
        missing[i] = false;
        attributes[i] = instA[i];
      }
    }
  }
