...
public class OneR extends AbstractClassifier implements
  ...
  private class OneRRule implements Serializable, RevisionHandler {
    ...
    public OneRRule(Instances data, Attribute attribute, int nBreaks) throws Exception {
      ...
      m_breakpoints = new float[nBreaks - 1]; // last breakpoint is infinity
    }
    ...
    public String toString() {
      ...
      try {
        ...
        text.append(m_attr.name()).append(":\n");
        for (int v = 0; v < m_classifications.length; v++) {
          text.append("\t");
          if (m_attr.isNominal()) {
            text.append(m_attr.value(v));
          } else if (v < m_breakpoints.length) {
            text.append("< ").append( m_breakpoints[v]);
          } else if (v > 0) {
            text.append(">= ").append(m_breakpoints[v - 1]);
          } else {
            text.append("not ?");
          }
          text.append("\t-> ").append(m_class.value(m_classifications[v])).append("\n");
        }
        if (m_missingValueClass != -1) {
          text.append("\t?\t-> ").append(m_class.value(m_missingValueClass)).append("\n");
        }
        text
          .append("(").append(m_correct).append("/").append(m_numInst).append(" instances correct)\n");
  ...
  public OneRRule newNumericRule(Attribute attr, Instances data,
    ...
    for (int i = 0; i < lastInstance; i++) {
      ...
      if ((i == 0) || (data.instance(i).value(attr) > lastValue)) {
        if (i != 0) {
          values.add((lastValue + data.instance(i).value(attr)) / 2.0f);
        }
    ...
    ListIterator<Float> itVals = values.listIterator();
    int[] oldDist = null;
    while (it.hasNext()) {
  ...
  public String[] getOptions() {
    ...
    options.add("-B");
    options.add(String.valueOf(m_minBucketSize));
  ...
  public String toSource(String className) throws Exception {
    ...
    if (m_ZeroR != null) {
      ...
      result.append("    // chosen attribute: ").append(m_rule.m_attr.name()).append(" ("
    		  ).append(m_rule.m_attr.index()).append(")\n");
      result.append("\n");
      // missing values
      result.append("    // missing value?\n");
      result.append("    if (i[").append(m_rule.m_attr.index()).append("] == null)\n");
      if (m_rule.m_missingValueClass != -1) {
      ...
      .append(Utils.arrayToString(m_rule.m_classifications)).append("};");
      result.append(" // ");
      ...
      if (m_rule.m_attr.isNominal()) {
        for (i = 0; i < m_rule.m_attr.numValues(); i++) {
          ...
          result.append("if (((String) i[").append(m_rule.m_attr.index()
        		  ).append("]).equals(\"").append(m_rule.m_attr.value(i)).append("\"))\n");
          result.append("      v = ").append(i).append("; // "
        		  ).append(m_rule.m_class.value(m_rule.m_classifications[i])).append("\n");
        }
      } else {
        result.append("    double[] breakpoints = new double[]{")
        .append(Utils.arrayToString(m_rule.m_breakpoints)).append("};\n");
        result.append("    while (v < breakpoints.length && \n");
        result.append("           ((Double) i[").append(m_rule.m_attr.index()).append("]) >= breakpoints[(int) v]) {\n");
        result.append("      v++;\n");
        result.append("    }\n");
      }
