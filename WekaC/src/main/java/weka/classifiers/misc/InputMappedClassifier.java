...
public class InputMappedClassifier extends SingleClassifierEnhancer implements
  ...
  private StringBuffer createMappingReport() {
    ...
    for (int i = 0; i < m_modelHeader.numAttributes(); i++) {
      ...
      String attName;
      if(temp.isNumeric())
    	  attName= "(numeric) " + temp.name();
      else
    	  attName= "(nominal) " + temp.name();
      ...
      if (m_attributeStatus[i] == NO_MATCH) {
        inAttNum += "- ";
        result.append(inAttNum).append("missing (no match)\n");
      } else if (m_attributeStatus[i] == TYPE_MISMATCH) {
        inAttNum += (m_attributeMap[i] + 1) + " ";
        result.append(inAttNum).append("missing (type mis-match)\n");
      } else {
        Attribute inAtt = m_inputHeader.attribute(m_attributeMap[i]);
        String inName;
        if(inAtt.isNumeric())
        	inName= new StringBuilder(m_attributeMap[i] + 1).append(" (numeric) ").append(inAtt.name()).toString();
        else
        	inName= new StringBuilder(m_attributeMap[i] + 1).append(" (nominal) ").append(inAtt.name()).toString();
        result.append(inName).append("\n");
      }
    }
      ...
      for (int j = 0; j < m_inputHeader.numAttributes(); j++) {
        ...
        if (stringMatch(modelAttName, incomingAttName)) {
          ...
          if (modelAtt.numValues() != incomingAtt.numValues()) {
            System.out
              .println(new StringBuilder("[InputMappedClassifier] Warning: incoming nominal attribute ")
              .append(incomingAttName).append(" does not have the same number of values as model attribute ")
              .append(modelAttName));
  ...
  public synchronized Instance constructMappedInstance(Instance incoming) throws Exception {
    ...
    m_vals = new float[m_modelHeader.numAttributes()];
  ...
  public String toString() {
    ...
    if (m_modelPath != null && m_modelPath.length() > 0) {
      buff.append("Model sourced from: ").append(m_modelPath).append("\n\n");
    }
    ...
    if (!m_suppressMappingReport && m_inputHeader != null) {
      ...
      if (m_attributeMap != null) {
        buff.append("\n").append(createMappingReport().toString());
      }
    }
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 13069 $");
  }
