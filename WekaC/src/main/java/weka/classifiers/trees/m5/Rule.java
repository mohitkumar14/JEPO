...
public class Rule implements Serializable, RevisionHandler {
   ...
   */
  private float m_minNumInstances;
  ...
  private String treeToString() {
    ...
    text.append("M5 ").append(((m_useUnpruned) ? "unpruned " : "pruned "))
    .append(((m_regressionTree) ? "regression " : "model ")).append("tree:\n");
    ...
    text.append("\nNumber of Rules : ").append(m_topOfTree.numberOfLinearModels());
  ...
  private String ruleToString() {
    ...
    if (m_splitAtts.length > 0) {
      ...
      for (int i = m_splitAtts.length - 1; i >= 0; i--) {
        text.append("\t").append(m_covered.attribute(m_splitAtts[i]).name()).append(" ");
        ...
        text.append(Utils.doubleToString(m_splitVals[i], 1, 3)).append("\n");
      }
    ...
    if (m_ruleModel != null) {
      try {
        ...
        text.append(" [").append(m_numCovered/* m_covered.numInstances() */);

        if (m_globalAbsDev > 0.0) {
          text
            .append("/").append(Utils.doubleToString(
   ...
   */
  protected static final float stdDev(int attr, Instances inst) {
    int i, count = 0;
    float sd, va, sum = 0.0f, sqrSum = 0.0f, value;
   ...
   */
  protected static final float absDev(int attr, Instances inst) {
    int i;
    float average = 0.0f, absdiff = 0.0f, absDev;
