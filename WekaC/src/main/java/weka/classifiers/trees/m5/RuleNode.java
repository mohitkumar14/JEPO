...
public class RuleNode extends AbstractClassifier {
   ...
   */
  private float m_rootMeanSquaredError;
   ...
   */
  private final float m_devFraction = 0.05f;
  private final float m_pruningMultiplier = 2;
   ...
   */
  private final float m_globalDeviation;

  /**
   * the absolute deviation of the global class
   */
  private final float m_globalAbsDeviation;
   ...
   */
  private static final float SMOOTHING_CONSTANT = 15.0f;
   ...
   */
  public RuleNode(float globalDev, float globalAbsDev, RuleNode parent) {
    m_nodeModel = null;
    m_right = null;
    m_left = null;
    m_parent = parent;
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    m_rootMeanSquaredError = Float.MAX_VALUE;
   ...
   */
  @Override
  public float classifyInstance(Instance inst) throws Exception {
    if (m_isLeaf) {
      if (m_nodeModel == null) {
   ...
   * @throws Exception if an error occurs
   */
  protected static float smoothingOriginal(float n, float pred,
    float supportPred) throws Exception {
    float smoothed;
  ...
  private void buildLinearModel(int[] indices) throws Exception {
    ...
    float[] lmCoeffs = temp.coefficients();
    float[] coeffs = new float[m_instances.numAttributes()];
  ...
  public String nodeToString() {
    ...
    text.append("Node:\n\tnum inst: ").append(m_numInstances);
    ...
    text.append("\n\tSplit att: ").append(m_instances.attribute(m_splitAtt).name());
    text.append("\n\tSplit val: ").append(Utils.doubleToString(m_splitValue, 1, 3));
    text.append("\n\tLM num: ").append(m_leafModelNum);
    text.append("\n\tLinear model\n").append(m_nodeModel.toString());
    text.append("\n\n");
  ...
  public String treeToString(int level) {
    ...
    if (!m_isLeaf) {
      ...
      if (m_instances.attribute(m_splitAtt).name().charAt(0) != '[') {
        text.append(m_instances.attribute(m_splitAtt).name()).append(" <= ")
        .append(Utils.doubleToString(m_splitValue, 1, 3)).append(" : ");
      } else {
        text.append(m_instances.attribute(m_splitAtt).name()).append(" false : ");
      }
      ...
      if (m_instances.attribute(m_splitAtt).name().charAt(0) != '[') {
        ...
        text.append(m_instances.attribute(m_splitAtt).name()).append(" true : ");
      }
      ...
      text.append("LM").append(m_leafModelNum);

      if (m_globalDeviation > 0.0) {
        text.append(" (").append(m_numInstances).append("/")
        .append(Utils.doubleToString(
            (100.0f * m_rootMeanSquaredError / m_globalDeviation), 1, 3)).append("%)\n");
      } else {
        text.append(" (").append(m_numInstances).append(")\n");
      }
    }
  ...
  public void installSmoothedModels() throws Exception {
    ...
    if (m_isLeaf) {
      float[] coefficients = new float[m_numAttributes];
      float intercept;
      float[] coeffsUsedByLinearModel = m_nodeModel.coefficients();
      ...
      do {
        if (current.m_parent != null) {
          float n = current.m_numInstances;
   ...
   */
  private float pruningFactor(int num_instances, int num_params) {
    if (num_instances <= num_params) {
      return 10.0f; // Caution says Yong in his code
    }
   ...
   * 
   * @param best_cov the greatest coverage found so far
   * @param bestLeaf the leaf with the greatest coverage
   */
  public void findBestLeaf(float[] best_cov, RuleNode[] bestLeaf) {
    if (!m_isLeaf) {
      if (m_left != null) {
        m_left.findBestLeaf(best_cov, bestLeaf);
      }

      if (m_right != null) {
        m_right.findBestLeaf(best_cov, bestLeaf);
      }
    } else {
      if (m_numInstances > best_cov[0]) {
        best_cov[0] = m_numInstances;
        bestLeaf[0] = this;
      }
    }
  }
   ...
   */
  protected float rootMeanSquaredError() {
   ...
   */
  public void setMinNumInstances(float minNum) {
    m_splitNum = minNum;
  }
   ...
   */
  public float getMinNumInstances() {
    return m_splitNum;
  }
      ...
      + (m_isLeaf ? " ("
        + ((m_globalDeviation > 0.0) ? m_numInstances
          ...
          + Utils.doubleToString(
            (100.0f * m_rootMeanSquaredError / m_globalDeviation), 1, 3) + "%)"
          : m_numInstances + ")") + "\" shape=box style=filled " : "\"")
      + (m_saveInstances ? "data=\n" + m_instances + "\n,\n" : "") + "]\n");

    if (m_left != null) {
      text.append("N").append(m_id).append("->N").append(m_left.m_id).append(" [label=\"<=")
      .append(Utils.doubleToString(m_splitValue, 1, 3)).append("\"]\n");
      m_left.graphTree(text);
    }

    if (m_right != null) {
      text.append("N").append(m_id).append("->N").append(m_right.m_id).append(" [label=\">")
      .append(Utils.doubleToString(m_splitValue, 1, 3)).append("\"]\n");
