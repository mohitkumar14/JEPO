...
public class PreConstructedLinearModel 
   ...
   */
  public PreConstructedLinearModel(float [] coeffs, float intercept) {
  ...
  public String toString() {
    ...
    b.append("\n").append(m_instancesHeader.classAttribute().name()).append(" = ");
    boolean first = true;
    for (int i = 0; i < m_coefficients.length; i++) {
      if (m_coefficients[i] != 0.0) {
	float c = m_coefficients[i];
	if (first) {
	  b.append("\n\t").append(Utils.doubleToString(c, 12, 4).trim()).append(" * ")
	  .append(m_instancesHeader.attribute(i).name()).append(" ");
	  first = false;
	} else {
		if(m_coefficients[i] < 0)
			b.append("\n\t- ").append(Utils.doubleToString(Math.abs(c), 12, 4).trim()).append(" * ")
			.append(m_instancesHeader.attribute(i).name()).append(" ");
		else
			b.append("\n\t+ ").append(Utils.doubleToString(Math.abs(c), 12, 4).trim()).append(" * ")
			.append(m_instancesHeader.attribute(i).name()).append(" ");
