...
public final class CorrelationSplitInfo implements Cloneable, Serializable,
  ...
  public final void attrSplit(int attr, Instances inst) throws Exception {
    ...
    PairedStats full = new PairedStats(0.01f);
    PairedStats leftSubset = new PairedStats(0.01f);
    PairedStats rightSubset = new PairedStats(0.01f);
    int classIndex = inst.classIndex();
    float leftCorr, rightCorr;
    float leftVar, rightVar, allVar;
    float order = 2.0f;
    ...
    allVar = (float) Math.pow(allVar, (1.0f / order));
    ...
    for (i = low + len; i < high - len - 1; i++) {
      ...
      if (!Utils.eq(inst.instance(i + 1).value(attr),
        ...
        leftCorr = (float) Math.abs(leftSubset.correlation);
        rightCorr = (float) Math.abs(rightSubset.correlation);
        leftVar = (leftSubset.yStats.stdDev * leftSubset.yStats.stdDev);
        leftVar = Math.abs(leftVar);
        leftVar = (float) Math.pow(leftVar, (1.0 / order));
        rightVar = (rightSubset.yStats.stdDev * rightSubset.yStats.stdDev);
        rightVar = Math.abs(rightVar);
        rightVar = (float) Math.pow(rightVar, (1.0 / order));

        float score = allVar - ((leftSubset.count / full.count) * leftVar)
