...
public class DecisionStump 
  ...
  protected float[][] m_Distribution;
  ...
  public void buildClassifier(Instances instances) throws Exception {
    ...
    float bestVal = Float.MAX_VALUE, currVal;
    float bestPoint = -Float.MAX_VALUE;
    ...
    float[][] bestDist = new float[3][instances.numClasses()];
	...
	m_Distribution = new float[3][numClasses];
	...
	float sumCounts = Utils.sum(m_Distribution[i]);
  ...
  public String toSource(String className) throws Exception {
    ...
    text.append("    /* ").append(m_Instances.attribute(m_AttIndex).name()).append(" */\n");
   ...
   * @return the value
   */
  protected String sourceClass(Attribute c, float []dist) {

    if (c.isNominal()) {
      return Integer.toString(Utils.maxIndex(dist));
    } else {
      return Float.toString(dist[0]);
    }
  }
  ...
  public String toString(){
    ...
    try {
      ...
      if (att.isNominal()) {
	text.append(att.name()).append(" = ").append(att.value((int)m_SplitPoint)).append(" : ");
	text.append(printClass(m_Distribution[0]));
	text.append(att.name()).append(" != ").append(att.value((int)m_SplitPoint)).append(" : ");
	text.append(printClass(m_Distribution[1]));
      } else {
	text.append(att.name()).append(" <= ").append(m_SplitPoint).append(" : ");
	text.append(printClass(m_Distribution[0]));
	text.append(att.name()).append(" > ").append(m_SplitPoint).append(" : ");
	text.append(printClass(m_Distribution[1]));
      }
      text.append(att.name()).append(" is missing : ");
   ...
   */
  protected String printDist(float[] dist) throws Exception {

    StringBuffer text = new StringBuffer();
    
    if (m_Instances.classAttribute().isNominal()) {
      for (int i = 0; i < m_Instances.numClasses(); i++) {
	text.append(m_Instances.classAttribute().value(i)).append("\t");
      }
      text.append("\n");
      for (int i = 0; i < m_Instances.numClasses(); i++) {
	text.append(dist[i]).append("\t");
      }
      text.append("\n");
    }
   ...
   */
  protected String printClass(float[] dist) throws Exception {
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNominal(int index) throws Exception {
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNominalNominal(int index) throws Exception {

    float bestVal = Float.MAX_VALUE, currVal;
    float[][] counts = new float[m_Instances.attribute(index).numValues() 
				  + 1][m_Instances.numClasses()];
    float[] sumCounts = new float[m_Instances.numClasses()];
    float[][] bestDist = new float[3][m_Instances.numClasses()];
    int numMissing = 0;
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNominalNumeric(int index) throws Exception {

    float bestVal = Float.MAX_VALUE, currVal;
    float[] sumsSquaresPerValue = 
      new float[m_Instances.attribute(index).numValues()], 
      sumsPerValue = new float[m_Instances.attribute(index).numValues()], 
      weightsPerValue = new float[m_Instances.attribute(index).numValues()];
    float totalSumSquaresW = 0, totalSumW = 0, totalSumOfWeightsW = 0,
      totalSumOfWeights = 0, totalSum = 0;
    float[] sumsSquares = new float[3], sumOfWeights = new float[3];
    float[][] bestDist = new float[3][1];
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNumeric(int index) throws Exception {
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNumericNominal(int index) throws Exception {

    float bestVal = Float.MAX_VALUE, currVal, currCutPoint;
    int numMissing = 0;
    float[] sum = new float[m_Instances.numClasses()];
    float[][] bestDist = new float[3][m_Instances.numClasses()];
	...
	currCutPoint = (inst.value(index) + instPlusOne.value(index)) / 2.0f;
	currVal = ContingencyTables.entropyConditionedOnRows(m_Distribution);
	if (currVal < bestVal) {
   ...
   * @throws Exception if something goes wrong
   */
  protected float findSplitNumericNumeric(int index) throws Exception {

    float bestVal = Float.MAX_VALUE, currVal, currCutPoint;
    int numMissing = 0;
    float[] sumsSquares = new float[3], sumOfWeights = new float[3];
    float[][] bestDist = new float[3][1];
    float totalSum = 0, totalSumOfWeights = 0;
  ...
  protected float variance(float[][] s,float[] sS,float[] sumOfWeights) {
    ...
    for (int i = 0; i < s.length; i++) {
      if (sumOfWeights[i] > 0) {
	var += sS[i] - ((s[i][0] * s[i][0]) / (float) sumOfWeights[i]);
      }
    }
