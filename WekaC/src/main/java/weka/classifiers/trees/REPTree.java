...
public class REPTree extends AbstractClassifier implements OptionHandler,
  ...
  protected class Tree implements Serializable, RevisionHandler {
     ...
     */
    protected float[] m_HoldOutDist = null;
     ...
     */
    protected float m_HoldOutError = 0;
     ...
     */
    protected float[] distributionForInstance(Instance instance)
      throws Exception {
    ...
    public final String sourceExpression(int index) {
      ...
      if (index < 0) {
        return new StringBuilder("i[").append(m_Attribute).append("] == null").toString();
      }
      ...
      if (m_Attribute == -1) {
        ...
        for (int i = 0; i < m_Successors.length; i++) {
          ...
          if (m_Successors[i].m_Attribute == -1) {
            float[] successorProbs = m_Successors[i].m_ClassProbs;
            ...
            text.append(String.valueOf(sub[0]));
            atEnd.append(String.valueOf(sub[1]));
          }
          text.append("    } ");
        ...
        result[0] = new StringBuffer("    p = ").append(className).append(".N");
        result[0].append(Integer.toHexString(this.hashCode()) + printID)
          .append("(i);\n");
        result[1] = text.append(String.valueOf(atEnd));
      }
      return result;
    }
    ...
    protected int toGraph(StringBuffer text, int num, Tree parent)
      ...
      num++;
      if (m_Attribute == -1) {
        text.append("N").append(Integer.toHexString(Tree.this.hashCode())
        		).append(" [label=\"").append(num).append(Utils.backQuoteChars(leafString(parent)))
        .append("\"shape=box]\n");
      } else {
        text.append("N").append(Integer.toHexString(Tree.this.hashCode())
        		).append(" [label=\"").append(num).append(": ")
        .append(Utils.backQuoteChars(m_Info.attribute(m_Attribute).name())).append("\"]\n");
        for (int i = 0; i < m_Successors.length; i++) {
          text.append("N").append(Integer.toHexString(Tree.this.hashCode())).append("->N")
          .append(Integer.toHexString(m_Successors[i].hashCode())).append(" [label=\"");
          if (m_Info.attribute(m_Attribute).isNumeric()) {
            if (i == 0) {
              text.append(" < ").append(Utils.doubleToString(m_SplitPoint, getNumDecimalPlaces()));
            } else {
              text.append(" >= ").append(Utils.doubleToString(m_SplitPoint, getNumDecimalPlaces()));
            }
          } else {
    ...
    protected String leafString(Tree parent) throws Exception {
      ...
      if (m_Info.classAttribute().isNumeric()) {
        ...
        buffer.append(" : ").append(Utils.doubleToString(classMean, getNumDecimalPlaces()));
        float avgError = 0;
        if (m_Distribution[1] > 0) {
          avgError = m_Distribution[0] / m_Distribution[1];
        }
        buffer.append(" (").append(Utils.doubleToString(m_Distribution[1], getNumDecimalPlaces())).append("/"
        		).append(Utils.doubleToString(avgError, getNumDecimalPlaces())).append(")");
        avgError = 0;
        if (m_HoldOutDist[0] > 0) {
          avgError = m_HoldOutError / m_HoldOutDist[0];
        }
        buffer.append(" [").append(Utils.doubleToString(m_HoldOutDist[0], getNumDecimalPlaces())).append("/"
        		).append(Utils.doubleToString(avgError, getNumDecimalPlaces())).append("]");
        ...
        return new StringBuilder(" : ").append(m_Info.classAttribute().value(maxIndex)).append(" (")
        		.append(Utils.doubleToString(Utils.sum(m_Distribution), getNumDecimalPlaces())).append("/")
        		.append(Utils.doubleToString(
            (Utils.sum(m_Distribution) - m_Distribution[maxIndex]), getNumDecimalPlaces())).append(") [")
        		.append(Utils.doubleToString(Utils.sum(m_HoldOutDist), getNumDecimalPlaces())).append("/")
        		.append(Utils.doubleToString(
            (Utils.sum(m_HoldOutDist) - m_HoldOutDist[maxIndex]), getNumDecimalPlaces())).append("]").toString();
      }
    }
      ...
      try {
        ...
        if (m_Attribute == -1) {
          ...
          for (int i = 0; i < m_Successors.length; i++) {
            ...
            text.append(m_Info.attribute(m_Attribute).name()).append(" = ")
            .append(m_Info.attribute(m_Attribute).value(i));
          ...
          text.append(m_Info.attribute(m_Attribute).name()).append(" < ")
          .append(Utils.doubleToString(m_SplitPoint, getNumDecimalPlaces()));
     ...
     * @throws Exception if generation fails
     */
    protected void buildTree(int[][][] sortedIndices, float[][][] weights,
      Instances data, float totalWeight, float[] classProbs,
      Instances header, float minNum, float minVariance, int depth,
      ...
      if (data.classAttribute().isNumeric()) {
        m_HoldOutDist = new float[2];
      } else {
        m_HoldOutDist = new float[data.numClasses()];
      }
      ...
      if (sortedIndices[0][helpIndex].length == 0) {
        if (data.classAttribute().isNumeric()) {
          ...
          m_Distribution = new float[data.numClasses()];
        }
        m_ClassProbs = null;
      ...
      m_ClassProbs = new float[classProbs.length];
      ...
      if ((totalWeight < (2 * minNum))
        ...
        if (data.classAttribute().isNominal()) {
          ...
          // Nominal case
          m_Distribution = new float[m_ClassProbs.length];
      ...
      float[] vals = new float[data.numAttributes()];
      float[][][] dists = new float[data.numAttributes()][0][0];
      float[][] props = new float[data.numAttributes()][0];
      float[][] totalSubsetWeights = new float[data.numAttributes()][0];
      float[] splits = new float[data.numAttributes()];
      if (data.classAttribute().isNominal()) {
      ...
      if (Utils.gr(vals[m_Attribute], 0) && (count > 1)) {
        ...
        float[][] attSubsetDists = dists[m_Attribute];
        float[] attTotalSubsetWeights = totalSubsetWeights[m_Attribute];
        ...
        float[][][][] subsetWeights = new float[numAttVals][1][data
          .numAttributes()][0];
    ...
    protected void splitData(int[][][][] subsetIndices,
      float[][][][] subsetWeights, int att, float splitPoint,
      int[][] sortedIndices, float[][] weights, Instances data)
      throws Exception {
        ...
        if (i != data.classIndex()) {
          if (data.attribute(att).isNominal()) {
            ...
            for (int k = 0; k < num.length; k++) {
              ...
              subsetWeights[k][0][i] = new float[sortedIndices[i].length];
            }
            ...
            for (int k = 0; k < 2; k++) {
              subsetIndices[k][0][i] = new int[sortedIndices[i].length];
              subsetWeights[k][0][i] = new float[weights[i].length];
            }
            for (j = 0; j < sortedIndices[i].length; j++) {
              Instance inst = data.instance(sortedIndices[i][j]);
              if (inst.isMissing(att)) {
          ...
          for (int k = 0; k < num.length; k++) {
            ...
            float[] copyWeights = new float[num[k]];
    ...
    protected float distribution(float[][] props, float[][][] dists,
      int att, int[] sortedIndices, float[] weights, float[][] subsetWeights,
      ...
      if (attribute.isNominal()) {
        ...
        float currSplit = data.instance(sortedIndices[0]).value(att);
      ...
      // Compute weights
      props[att] = new float[dist.length];
      for (int k = 0; k < props[att].length; k++) {
        props[att][k] = Utils.sum(dist[k]);
      }
      if (!(Utils.sum(props[att]) > 0)) {
        for (int k = 0; k < props[att].length; k++) {
          props[att][k] = 1.0f / props[att].length;
        }
      } else {
      ...
      subsetWeights[att] = new float[dist.length];
      for (int j = 0; j < dist.length; j++) {
    ...
    protected float numericDistribution(float[][] props, float[][][] dists,
      int att, int[] sortedIndices, float[] weights, float[][] subsetWeights,
      Instances data, float[] vals) throws Exception {
      ...
      if (attribute.isNominal()) {
        ...
        for (i = 0; i < sortedIndices.length; i++) {
          ...
          float classVal = inst.classValue() * weights[i];
      ...
      // Compute weights
      props[att] = new float[sums.length];
      for (int k = 0; k < props[att].length; k++) {
      ...
      float priorVar = singleVariance(totalSum, totalSumSquared,
        totalSumOfWeights);
     ...
     * @return the variance
     */
    protected float variance(float[] s, float[] sS, float[] sumOfWeights) {
     ...
     * @return the variance
     */
    protected float singleVariance(float s, float sS, float weight) {
     ...
     */
    protected float reducedErrorPrune() throws Exception {
     ...
     */
    protected void insertHoldOutInstance(Instance inst, float weight,
    ...
    protected void backfitHoldOutSet() throws Exception {
      ...
      if (m_Info.classAttribute().isNominal()) {
        ...
        // Nominal case
        if (m_ClassProbs == null) {
          m_ClassProbs = new float[m_Info.numClasses()];
        }
        ...
        // Numeric case
        float sumOfWeightsTrainAndHoldout = m_Distribution[1]
          + m_HoldOutDist[0];
        if (sumOfWeightsTrainAndHoldout <= 0) {
          return;
        }
        if (m_ClassProbs == null) {
          m_ClassProbs = new float[1];
        } else {
  ...
  protected float m_InitialCount = 0;
   ...
   */
  public void setInitialCount(float newInitialCount) {
  ...
  public String[] getOptions() {
    ...
    options.add("-M");
    options.add(String.valueOf((int) getMinNum()));
    options.add("-V");
    options.add(String.valueOf(getMinVarianceProp()));
    options.add("-N");
    options.add(String.valueOf(getNumFolds()));
    options.add("-S");
    options.add(String.valueOf(getSeed()));
    options.add("-L");
    options.add(String.valueOf(getMaxDepth()));
    if (getNoPruning()) {
      options.add("-P");
    }
    options.add("-I");
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (initialCountString.length() != 0) {
      m_InitialCount = Float.parseFloat(initialCountString);
    } else {
      m_InitialCount = 0;
    }
   ...
   */
  @Override
  public float getMeasure(String additionalMeasureName) {
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    float[][][] weights = new float[1][train.numAttributes()][0];
    float[] vals = new float[train.numInstances()];
    for (int j = 0; j < train.numAttributes(); j++) {
      if (j != train.classIndex()) {
        weights[0][j] = new float[train.numInstances()];
    ...
    float totalWeight = 0, totalSumSquared = 0;
    for (int i = 0; i < train.numInstances(); i++) {
      Instance inst = train.instance(i);
      if (data.classAttribute().isNominal()) {
  ...
  public String toSource(String className) throws Exception {
    ...
    return new StringBuilder("class ").append(className).append(" {\n\n  public static double classify(Object [] i)\n    throws Exception {\n\n    double p = Double.NaN;\n"
    		).append(source[0] // Assignment code
    				).append("    return p;\n  }\n").append(source[1] // Support code
    						).append("}\n").toString();
  }
  ...
  public String graph() throws Exception {
    ...
    String result = new StringBuilder("digraph Tree {\nedge [style=bold]\n")
  ...
  public String toString() {
    ...
    return new StringBuilder("\nREPTree\n============\n").append(m_Tree.toString(0, null)).append("\n"
    		).append("\nSize of the tree : ").append(numNodes()).toString();
  }
