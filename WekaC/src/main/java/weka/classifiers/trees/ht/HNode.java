...
public abstract class HNode implements Serializable {
   ...
   */
  public float[] getDistribution(Instance inst, Attribute classAtt)
      throws Exception {
    float[] dist = new float[classAtt.numValues()];
  ...
  protected int dumpTree(int depth, int leafCount, StringBuffer buff) {
    ...
    buff.append(classVal).append(" (").append(String.format("%-9.3f", max).trim()).append(")");
    leafCount++;
    m_leafNum = leafCount;
  ...
  public void graphTree(StringBuffer text) {
    ...
    text.append("N").append(m_nodeNum).append(" [label=\"").append(classVal).append(" ("
    		).append(String.format("%-9.3f", max).trim()).append(")\" shape=box style=filled]\n");
  }
