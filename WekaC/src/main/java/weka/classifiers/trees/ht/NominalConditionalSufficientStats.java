...
public class NominalConditionalSufficientStats extends
  ...
  protected class ValueDistribution implements Serializable {
    ...
    public void add(int val, float weight) {
    ...
    public void delete(int val, float weight) {
      WeightMass count = m_dist.get(val);
      if (count != null) {
        count.m_weight -= weight;
        m_sum -= weight;
    ...
    public float getWeight(int val) {
      WeightMass count = m_dist.get(val);
      if (count != null) {
  ...
  protected float m_missingWeight;
  ...
  public float probabilityOfAttValConditionedOnClass(float attVal,
    ...
    if (valDist != null) {
      float prob = valDist.getWeight((int) attVal) / valDist.sum();
      return prob;
    }
  ...
  public SplitCandidate bestSplit(SplitMetric splitMetric,
    ...
    float merit = splitMetric.evaluateSplit(preSplitDist, postSplitDists);
