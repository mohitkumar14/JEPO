...
public class LMTNode extends LogisticBase {
  ...
  protected float m_totalInstanceWeight;
  ...
  public LMTNode(ModelSelection modelSelection, int numBoostingIterations,
    ...
    float weightTrimBeta, boolean useAIC, NominalToBinary ntb, int numDecimalPlaces) {
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    float[][] alphas = new float[m_numFoldsPruning][];
    float[][] errors = new float[m_numFoldsPruning][];
    ...
    for (int i = 0; i < m_numFoldsPruning; i++) {
      ...
      alphas[i] = new float[numNodes + 2];
      errors[i] = new float[numNodes + 2];
    ...
    float[] treeAlphas = new float[numNodes + 2];

    // prune back and log alpha-values
    int iterations = prune(treeAlphas, null, null);

    float[] treeErrors = new float[numNodes + 2];

    for (int i = 0; i <= iterations; i++) {
      // compute midpoint alphas
      float alpha = (float) Math.sqrt(treeAlphas[i] * treeAlphas[i + 1]);
      float error = 0;
    ...
    float bestAlpha = (float) Math.sqrt(treeAlphas[best] * treeAlphas[best + 1]);
  ...
  public void buildTree(Instances data,
    SimpleLinearRegression[][] higherRegressions, float totalInstanceWeight,
    float higherNumParameters, Instances numericDataHeader) throws Exception {
    ...
    if (m_numInstances > m_minNumInstances) {
      ...
      if (m_modelSelection instanceof ResidualModelSelection) {
        ...
        float[][] probs = getProbs(getFs(m_numericData));
        float[][] trainYs = getYs(m_train);
        float[][] dataZs = getZs(probs, trainYs);
        float[][] dataWs = getWs(probs, trainYs);
   ...
   * @throws Exception if something goes wrong
   */
  public void prune(float alpha) throws Exception {
   ...
   * @throws Exception if something goes wrong
   */
  public int prune(float[] alphas, float[] errors, Instances test)
    throws Exception {
  ...
  public void calculateAlphas() throws Exception {
    ...
    if (!m_isLeaf) {
      float errorDiff = m_numIncorrectModel - m_numIncorrectTree;
   ...
   */
  public float[] modelDistributionForInstance(Instance instance)
    throws Exception {
  ...
  public String toString() {
    ...
    try {
      ...
      if (m_isLeaf) {
        text.append(": ");
        text.append("LM_").append(m_leafModelNum).append(":").append(getModelParameters());
      } else {
        dumpTree(0, text);
      }
  ...
  public String getModelParameters() {
    ...
    text.append(m_numRegressions).append("/").append(numModels).append(" (").append(m_numInstances).append(")");
    return text.toString();
  }
  ...
  protected void dumpTree(int depth, StringBuffer text) throws Exception {
    ...
    for (int i = 0; i < m_sons.length; i++) {
      ...
      if (m_sons[i].m_isLeaf) {
        text.append(": ");
        text.append("LM_").append(m_sons[i].m_leafModelNum).append(":").append(m_sons[i].getModelParameters());
      } else {
  ...
  public String modelsToString() {
    ...
    StringBuffer text = new StringBuffer();
    if (m_isLeaf) {
      text.append("LM_").append(m_leafModelNum).append(":").append(super.toString());
    } else {
      for (LMTNode m_son : m_sons) {
        text.append("\n").append(m_son.modelsToString());
      }
    }
    return text.toString();
  }
  ...
  public String graph() throws Exception {
    ...
    if (m_isLeaf) {
      text.append("N").append(m_id).append(" [label=\"LM_").append(m_leafModelNum).append(":")
      .append(getModelParameters()).append("\" shape=box style=filled");
      text.append("]\n");
    } else {
  ...
  private void graphTree(StringBuffer text) throws Exception {
    ...
    for (int i = 0; i < m_sons.length; i++) {
      text.append("N").append(m_id).append("->N").append(m_sons[i].m_id).append(" [label=\""
    		  ).append(Utils.backQuoteChars(m_localModel.rightSide(i, m_train).trim())).append("\"]\n");
      if (m_sons[i].m_isLeaf) {
        text.append("N").append(m_sons[i].m_id).append(" [label=\"LM_")
        .append(m_sons[i].m_leafModelNum).append(":").append(m_sons[i].getModelParameters())
