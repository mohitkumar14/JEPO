...
public class SimpleLinearRegression implements Serializable {
  ...
  /** The slope */
  private float m_slope = Float.NaN;

  /** The intercept */
  private float m_intercept = Float.NaN;
   ...
   */
  public SimpleLinearRegression(int attIndex, float slope, float intercept) {
   ...
   */
  public float classifyInstance(Instance inst) {
   ...
   */
  protected float[] computeMeans(Instances insts) {

    // We can assume that all the attributes are numeric and that
    // we don't have any missing attribute values (including the class)
    float[] means = new float[insts.numAttributes()];
    float[] counts = new float[insts.numAttributes()];
  ...
  public void buildClassifier(Instances insts) {
    ...
    float[] means = computeMeans(insts);
    ...
    for (int j = 0; j < insts.numInstances(); j++) {
      ...
      float yDiff = inst.value(classIndex) - means[classIndex];
      float weightedYDiff = inst.weight() * yDiff;

      // For all attributes
      for (int i = 0; i < insts.numAttributes(); i++) {
        float diff = inst.value(i) - means[i];
    ...
    for (int i = 0; i < insts.numAttributes(); i++) {
      ...
      float intercept = means[classIndex] - slopes[i] * means[i];

      // Compute sum of squared errors
      float sse = sumWeightedDiffsSquared[classIndex] - slopes[i] * numerator;
