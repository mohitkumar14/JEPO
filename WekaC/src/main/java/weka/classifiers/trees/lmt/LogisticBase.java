...
public class LogisticBase extends AbstractClassifier implements
  ...
  protected float m_numParameters = 0;
   ...
   **/
  protected float m_weightTrimBeta = 0;
  ...
  protected void performBoostingInfCriterion() throws Exception {
    ...
    float bestCriterion = Float.MAX_VALUE;
    int bestIteration = 0;
    int noMin = 0;

    // Variable to keep track of criterion values (AIC)
    float criterionValue = Float.MAX_VALUE;

    // initialize Ys/Fs/ps
    float[][] trainYs = getYs(m_train);
    float[][] trainFs = getFs(m_numericData);
    float[][] probs = getProbs(trainFs);
    ...
    while (iteration < m_maxIterations) {
      ...
      float numberOfAttributes = m_numParameters + iteration;

      // Fill criterion array values
      criterionValue = 2.0f * negativeLogLikelihood(trainYs, probs) + 2.0f
        * numberOfAttributes;
  ...
  protected int performBoosting(Instances train, Instances test,
    ...
    // initialize Ys/Fs/ps
    float[][] trainYs = getYs(train);
    float[][] trainFs = getFs(numericTrain);
    float[][] probs = getProbs(trainFs);
   ...
   * @throws Exception if something goes wrong
   */
  protected boolean performIteration(int iteration, float[][] trainYs,
    float[][] trainFs, float[][] probs, Instances trainNumeric)
    throws Exception {

    SimpleLinearRegression[] linearRegressionForEachClass = new SimpleLinearRegression[m_numClasses];

    // Store weights
    float[] oldWeights = new float[trainNumeric.numInstances()];
    ...
    for (int j = 0; j < m_numClasses; j++) {
      ...
      for (int i = 0; i < trainNumeric.numInstances(); i++) {
        ...
        float p = probs[i][j];
        float actual = trainYs[i][j];
        float z = getZ(actual, p);
        float w = (actual - p) / z;
      ...
      if (weightSum > 0) {
        ...
        if (m_weightTrimBeta > 0) {
          ...
          // Get weights
          float[] weights = new float[oldWeights.length];
          for (int i = 0; i < oldWeights.length; i++) {
        ...
        // Scale the weights
        float multiplier = instancesCopy.numInstances() / weightSum;
  ...
  private class UnsafeInstance extends DenseInstance {
     ...
     */
    @Override
    public void setValue(int attIndex, float value) {
   ...
   */
  protected float getZ(float actual, float p) {
    float z;
    if (actual == 1) {
      z = 1.0f / p;
   ...
   * 
   * @param trainYs the actual class probabilities
   * @param probs the estimated class probabilities
   * @return the LogitBoost response
   */
  protected float[][] getZs(float[][] probs, float[][] trainYs) {

    float[][] dataZs = new float[probs.length][m_numClasses];
    for (int j = 0; j < m_numClasses; j++) {
      for (int i = 0; i < probs.length; i++) {
        dataZs[i][j] = getZ(trainYs[i][j], probs[i][j]);
      }
    }
    return dataZs;
  }
   ...
   */
  protected float[][] getWs(float[][] probs, float[][] trainYs) {

    float[][] dataWs = new float[probs.length][m_numClasses];
    for (int j = 0; j < m_numClasses; j++) {
      for (int i = 0; i < probs.length; i++) {
        float z = getZ(trainYs[i][j], probs[i][j]);
        dataWs[i][j] = (trainYs[i][j] - probs[i][j]) / z;
      }
    }
    return dataWs;
  }
   ...
   * @return the p-values
   */
  protected float[] probs(float[] Fs) {
   ...
   * @return the Y-values
   */
  protected float[][] getYs(Instances data) {

    float[][] dataYs = new float[data.numInstances()][m_numClasses];
    for (int j = 0; j < m_numClasses; j++) {
      for (int k = 0; k < data.numInstances(); k++) {
    	  if(data.instance(k).classValue() == j)
    		  dataYs[k][j] = 1.0f;
    	  else
    		  dataYs[k][j] = 0.0f;
      }
   ...
   * @throws Exception if something goes wrong
   */
  protected float[] getFs(Instance instance) throws Exception {

    float[] pred = new float[m_numClasses];
    float[] instanceFs = new float[m_numClasses];
   ...
   * @throws Exception if something goes wrong
   */
  protected float[][] getFs(Instances data) throws Exception {

    float[][] dataFs = new float[data.numInstances()][];
   ...
   * @return the p-values
   */
  protected float[][] getProbs(float[][] dataFs) {
   ...
   */
  protected float negativeLogLikelihood(float[][] dataYs, float[][] probs) {
   ...
   */
  public float getWeightTrimBeta() {
   ...
   */
  public void setWeightTrimBeta(float w) {
    m_weightTrimBeta = w;
  }
  ...
  protected float[][] getCoefficients() {
    float[][] coefficients = new float[m_numClasses][m_numericDataHeader
      .numAttributes() + 1];
    for (int j = 0; j < m_numClasses; j++) {
      ...
      for (int i = 0; i < m_numericDataHeader.numAttributes(); i++) {
        if (i != m_numericDataHeader.classIndex()) {
          float slope = m_regressions[j][i].getSlope();
          float intercept = m_regressions[j][i].getIntercept();
    ...
    for (int j = 0; j < coefficients.length; j++) {
      for (int i = 0; i < coefficients[0].length; i++) {
        coefficients[j][i] *= (float) (m_numClasses - 1)
          / (float) m_numClasses;
      }
    }
  ...
  public float percentAttributesUsed() {
    ...
    return count / (m_numericDataHeader.numAttributes() - 1) * 100.0f;
  }
  ...
  public String toString() {
    ...
    for (int j = 0; j < m_numClasses; j++) {
      s.append("\nClass ").append(m_train.classAttribute().value(j)).append(" :\n");
      // constant term
      s.append(Utils.doubleToString(coefficients[j][0], 2 + m_numDecimalPlaces, m_numDecimalPlaces)).append(" + \n");
      for (int i = 0; i < attributes[j].length; i++) {
        // attribute/coefficient pairs
        s.append("[").append(m_numericDataHeader.attribute(attributes[j][i]).name()).append("]");
        s.append(" * "
