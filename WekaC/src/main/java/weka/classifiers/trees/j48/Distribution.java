...
public class Distribution implements Cloneable, Serializable, RevisionHandler {
  ...
  protected final float m_perClassPerBag[][];

  /** Weight of instances per bag. */
  protected final float m_perBag[];

  /** Weight of instances per class. */
  protected final float m_perClass[];
  ...
  public Distribution(int numBags, int numClasses) {
    ...
    m_perClassPerBag = new float[numBags][0];
    m_perBag = new float[numBags];
    m_perClass = new float[numClasses];
    for (i = 0; i < numBags; i++) {
      m_perClassPerBag[i] = new float[numClasses];
    }
    totaL = 0;
  }
   ...
   */
  public Distribution(float[][] table) {

    int i, j;

    m_perClassPerBag = table;
    m_perBag = new float[table.length];
    m_perClass = new float[table[0].length];
  ...
  public Distribution(Instances source) throws Exception {
    ...
    m_perClassPerBag = new float[1][0];
    m_perBag = new float[1];
    totaL = 0;
    m_perClass = new float[source.numClasses()];
    m_perClassPerBag[0] = new float[source.numClasses()];
  ...
  public Distribution(Instances source, ClassifierSplitModel modelToUse) throws Exception {
    ...
    m_perClassPerBag = new float[modelToUse.numSubsets()][0];
    m_perBag = new float[modelToUse.numSubsets()];
    totaL = 0;
    m_perClass = new float[source.numClasses()];
    for (int i = 0; i < modelToUse.numSubsets(); i++) {
      m_perClassPerBag[i] = new float[source.numClasses()];
    }
    Enumeration<Instance> enu = source.enumerateInstances();
    while (enu.hasMoreElements()) {
  ...
  public Distribution(Distribution toMerge) {
    ...
    totaL = toMerge.totaL;
    m_perClass = new float[toMerge.numClasses()];
    System
      .arraycopy(toMerge.m_perClass, 0, m_perClass, 0, toMerge.numClasses());
    m_perClassPerBag = new float[1][0];
    m_perClassPerBag[0] = new float[toMerge.numClasses()];
   ...
   */
  public final void add(int bagIndex, float[] counts) {

    float sum = Utils.sum(counts);
  ...
  public final void addInstWithUnknown(Instances source, int attIndex)
    ...
    probs = new float[m_perBag.length];
    for (j = 0; j < m_perBag.length; j++) {
      if (Utils.eq(totaL, 0)) {
        probs[j] = 1.0f / probs.length;
      } else {
   ...
   * @exception Exception if something goes wrong
   */
  public final void addWeights(Instance instance, float[] weights)
    throws Exception {
    ...
    classIndex = (int) instance.classValue();
    for (i = 0; i < m_perBag.length; i++) {
      float weight = instance.weight() * weights[i];
   ...
   */
  public final boolean check(float minNoObj) {
  ...
  public final String dumpDistribution() {
    ...
    for (i = 0; i < m_perBag.length; i++) {
      text.append("Bag num ").append(i).append("\n");
      for (j = 0; j < m_perClass.length; j++) {
        text.append("Class num ").append(j).append(" ").append(m_perClassPerBag[i][j]).append("\n");
      }
    }
   ...
   */
  public final float numCorrect(int index) {
   ...
   */
  public final float numIncorrect() {
   ...
   */
  public final float numIncorrect(int index) {
   ...
   * bag.
   */
  public final float perClassPerBag(int bagIndex, int classIndex) {
   ...
   */
  public final float perBag(int bagIndex) {
   ...
   */
  public final float perClass(int classIndex) {
   ...
   */
  public final float laplaceProb(int classIndex) {
   ...
   */
  public final float laplaceProb(int classIndex, int intIndex) {

    if (Utils.gr(m_perBag[intIndex], 0)) {
      return (m_perClassPerBag[intIndex][classIndex] + 1.0f)
   ...
   */
  public final float prob(int classIndex) {
   ...
   * Returns relative frequency of class for given bag.
   */
  public final float prob(int classIndex, int intIndex) {
