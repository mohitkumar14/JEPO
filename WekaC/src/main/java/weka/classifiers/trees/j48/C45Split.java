...
public class C45Split extends ClassifierSplitModel {
  ...
  /** InfoGain of split. */
  protected float m_infoGain;

  /** GainRatio of split. */
  protected float m_gainRatio;

  /** The sum of the weights of the instances. */
  protected final float m_sumOfWeights;
   ...
   */
  public C45Split(int attIndex, int minNoObj, float sumOfWeights,
   ...
   */
  @Override
  public final float classProb(int classIndex, Instance instance, int theSubset)
    throws Exception {

    if (theSubset <= -1) {
      float[] weights = weights(instance);
      if (weights == null) {
  ...
  private void handleNumericAttribute(Instances trainInstances)
    ...
    // subset.
    minSplit = 0.1f * (m_distribution.total()) / (trainInstances.numClasses());
  ...
  public final String sourceExpression(int index, Instances data) {
    ...
    if (index < 0) {
      return new StringBuilder("i[").append(m_attIndex).append("] == null").toString();
    }
    if (data.attribute(m_attIndex).isNominal()) {
  ...
  public final void setSplitPoint(Instances allInstances) {
    ...
    float newSplitPoint = -Float.MAX_VALUE;
    ...
    if ((allInstances.attribute(m_attIndex).isNumeric()) && (m_numSubsets > 1)) {
      for (int i = 0; i < allInstances.numInstances(); i++) {
        ...
        float tempValue = instance.value(m_attIndex);
   ...
   */
  public final float[][] minsAndMaxs(Instances data, float[][] minsAndMaxs,
    int index) {

    float[][] newMinsAndMaxs = new float[data.numAttributes()][2];
  ...
  public final float[] weights(Instance instance) {
    ...
    if (instance.isMissing(m_attIndex)) {
      weights = new float[m_numSubsets];
