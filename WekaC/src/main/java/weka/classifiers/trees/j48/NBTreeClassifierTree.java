...
public class NBTreeClassifierTree extends ClassifierTree {
  ...
  public String printLeafModels() {
    ...
    if (m_isLeaf) {
      text.append("\nLeaf number: ").append(m_id).append(" ");
  ...
  public String graph() throws Exception {
    ...
    if (m_isLeaf) {
      text.append("N").append(m_id).append(" [label=\"NB model\" shape=box style=filled ");
  ...
  private void graphTree(StringBuffer text) throws Exception {
    ...
    for (int i = 0; i < m_sons.length; i++) {
      ...
      if (m_sons[i].m_isLeaf) {
        text.append("N").append(m_sons[i].m_id).append(" [label=\"NB Model\" shape=box style=filled ");
        if (m_train != null && m_train.numInstances() > 0) {
