...
public class ClassifierTree implements Drawable, Serializable, RevisionHandler, CapabilitiesHandler {
   ...
   * @throws Exception if something goes wrong
   */
  public final float[] distributionForInstance(Instance instance,
    boolean useLaplace) throws Exception {

    float[] doubles = new float[instance.numClasses()];
  ...
  public String graph() throws Exception {
    ...
    if (m_isLeaf) {
      text.append("N").append(m_id).append(" [label=\""
    		  ).append(Utils.backQuoteChars(m_localModel.dumpLabel(0, m_train))).append("\" shape=box style=filled ");
  ...
  public StringBuffer[] toSource(String className) throws Exception {
    ...
    if (m_isLeaf) {
      result[0] = new StringBuffer("    p = ").append(m_localModel.distribution().maxClass(0)).append(";\n");
    ...
    for (int i = 0; i < m_sons.length; i++) {
      ...
      if (m_sons[i].m_isLeaf) {
        text.append("N").append(m_sons[i].m_id).append(" [label=\""
        		).append(Utils.backQuoteChars(m_localModel.dumpLabel(i, m_train)))
        ...
        text.append("N").append(m_sons[i].m_id).append(" [label=\"")
        .append(Utils.backQuoteChars(m_sons[i].m_localModel.leftSide(m_train)))
        .append("\" ");
        if (m_train != null && m_train.numInstances() > 0) {
          text.append("data =\n").append(m_sons[i].m_train).append("\n");
          text.append(",\n");
   ...
   * @throws Exception if something goes wrong
   */
  private float getProbsLaplace(int classIndex, Instance instance,
    ...
    if (m_isLeaf) {
      ...
      if (treeIndex == -1) {
        float[] weights = localModel().weights(instance);
        for (int i = 0; i < m_sons.length; i++) {
          if (!son(i).m_isEmpty) {
   ...
   * @throws Exception if something goes wrong
   */
  private float getProbs(int classIndex, Instance instance, float weight)
    throws Exception {
  ...
  public float[] getMembershipValues(Instance instance) throws Exception {
    ...
    while (!queueOfNodes.isEmpty()) {
      ...
      float[] weights = new float[node.m_sons.length];

      // Check for missing value
      if (treeIndex == -1) {
        weights = node.localModel().weights(instance);
      } else {
        weights[treeIndex] = 1.0f;
      }
