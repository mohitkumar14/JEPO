...
public class RandomForest extends Bagging {
  ...
  public String toString() {
    ...
    if (getComputeAttributeImportance()) {
      try {
        float[] nodeCounts = new float[m_data.numAttributes()];
        float[] impurityScores =
   ...
   */
  public float[] computeAverageImpurityDecreasePerAttribute(
    float[] nodeCounts) throws WekaException {
    ...
    float[] impurityDecreases = new float[m_data.numAttributes()];
    if (nodeCounts == null) {
      nodeCounts = new float[m_data.numAttributes()];
    }
    for (Classifier c : m_Classifiers) {
      float[][] forClassifier = ((RandomTree) c).getImpurityDecreases();
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
      "\tCompute and output attribute importance (mean impurity decrease method)", "attribute-importance", 0, "-attribute-importance"));
  ...
  public String[] getOptions() {
    ...
    result.add("-P");
    result.add(String.valueOf(getBagSizePercent()));
    ...
    result.add(String.valueOf(getNumExecutionSlots()));
