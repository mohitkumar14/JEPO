...
public class BVDecomposeSegCVSub
    ...
    protected float m_KWBias;

    /** The calculated Kohavi & Wolpert variance */
    protected float m_KWVariance;

    /** The calculated Kohavi & Wolpert sigma */
    protected float m_KWSigma;

    /** The calculated Webb bias */
    protected float m_WBias;

    /** The calculated Webb variance */
    protected float m_WVariance;
    ...
    public Enumeration<Option> listOptions() {
        ...
        (m_Classifier instanceof OptionHandler)) {
            ...
            .append(m_Classifier.getClass().getName())
            .append(":").toString()));
    ...
    public String [] getOptions() {
        ...
        options[current++] = "-c"; options[current++] = String.valueOf(getClassIndex());
        options[current++] = "-l"; options[current++] = String.valueOf(getClassifyIterations());
        options[current++] = "-p"; options[current++] = String.valueOf(getP());
        options[current++] = "-s"; options[current++] = String.valueOf(getSeed());
        if (getDataFileName() != null) {
            options[current++] = "-t"; options[current++] = String.valueOf(getDataFileName());
        }
        options[current++] = "-T"; options[current++] = String.valueOf(getTrainSize());
     ...
     *
     */
    public void setP(float proportion) {
    ...
    public void decompose() throws Exception {
        ...
        if ( m_P == -1 ){ // default value
            m_P =  m_TrainSize / ( data.numInstances() - 1 );
        ...
        float [][] instanceProbs = new float [data.numInstances()][numClasses];
        ...
        for (int i = 0; i < data.numInstances(); i++) {
            ...
            float [] predProbs = instanceProbs[ i ];
            float pActual, pPred;
            float bsum = 0, vsum = 0, ssum = 0;
            float wBSum = 0, wVSum = 0;
     ...
     */
    public Vector<Integer> findCentralTendencies(float[] predProbs) {
        ...
        try {
            ...
            try {
                ...
                while (enu.hasMoreElements()) {
                    ...
                    result += new StringBuilder(option.synopsis()).append("\n").append(option.description()).
                    		append("\n");
                }
