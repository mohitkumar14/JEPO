...
public class IBk 
  ...
  protected float m_NumAttributesUsed;
  ...
  public float [] distributionForInstance(Instance instance) throws Exception {
    ...
    float [] distances = m_NNSearch.getDistances();
    float [] distribution = makeDistribution( neighbours, distances );
  ...
  public String [] getOptions() {
    ...
    options.add("-K"); options.add(String.valueOf(getKNN()));
    options.add("-W"); options.add(String.valueOf(m_WindowSize));
    if (getCrossValidate()) {
        options.add("-X");
    }
    if (getMeanSquared()) {
  ...
  public String toString() {
    ...
    if (m_WindowSize != 0) {
      result += new StringBuilder("using a maximum of ").append(m_WindowSize)
    		  .append(" (windowed) training instances\n");
    }
    return result;
  }
   ...
   */
  protected float [] makeDistribution(Instances neighbours, float[] distances)
    throws Exception {

   float total = 0, weight;
    float [] distribution = new float [m_NumClasses];
    
    // Set up a correction to the estimator
    if (m_ClassType == Attribute.NOMINAL) {
      for(int i = 0; i < m_NumClasses; i++) {
	distribution[i] = 1.0f / Math.max(1,m_Train.numInstances());
      }
      total = (float)m_NumClasses / Math.max(1,m_Train.numInstances());
    }
    ...
    for(int i=0; i < neighbours.numInstances(); i++) {
      ...
      distances[i] = (float) Math.sqrt(distances[i]/m_NumAttributesUsed);
      switch (m_DistanceWeighting) {
        case WEIGHT_INVERSE:
          weight = 1.0f / (distances[i] + 0.001f); // to avoid div by zero
          break;
        case WEIGHT_SIMILARITY:
          weight = 1.0f - distances[i];
          break;
        default:                                 // WEIGHT_NONE:
          weight = 1.0f;
          break;
  ...
  protected void crossValidate() {
	...
	throw new Exception("CoverTree doesn't support hold-one-out "+
      ...
      float [] performanceStats = new float [m_kNNUpper];
      float [] performanceStatsSq = new float [m_kNNUpper];
	...
	performanceStatsSq[i] = 0;
      ...
      float[] origDistances, convertedDistances;
	...
	for(int j = m_kNNUpper - 1; j >= 0; j--) {
	  // Update the performance stats
          convertedDistances = new float[origDistances.length];
          System.arraycopy(origDistances, 0, 
                           convertedDistances, 0, origDistances.length);
	  float [] distribution = makeDistribution(neighbours, 
                                                    convertedDistances);
          float thisPrediction = Utils.maxIndex(distribution);
	  if (m_Train.classAttribute().isNumeric()) {
	    thisPrediction = distribution[0];
	    float err = thisPrediction - instance.classValue();
	...
	if (m_Train.classAttribute().isNumeric()) {
      ...
      float [] searchStats = performanceStats;
      if (m_Train.classAttribute().isNumeric() && m_MeanSquared) {
	searchStats = performanceStatsSq;
      }
      float bestPerformance = Float.NaN;
      int bestK = 1;
      for(int i = 0; i < m_kNNUpper; i++) {
	if (Float.isNaN(bestPerformance)
   ...
   */
  public Instances pruneToK(Instances neighbours, float[] distances, int k) {
