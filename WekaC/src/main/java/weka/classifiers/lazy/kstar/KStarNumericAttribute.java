...
public class KStarNumericAttribute implements KStarConstants, RevisionHandler {
  ...
  private float scaleFactorUsingBlend() {
    ...
    m_Distances = new float[m_NumInstances];
    ...
    if (nextlowest == -1 || lowest == -1) { // Data values are all the same
      scale = 1.0f;
      m_SmallestProb = m_AverageProb = 1.0f;
      return scale;
    } else {
      ...
      // aimfor = (N - n0) * b + n0
      aimfor = (m_ActualCount - lowestcount) * (float) m_BlendFactor / 100.0f
        + lowestcount;
      if (m_BlendFactor == 0) {
        aimfor += 1.0;
      }
      ...
      if (botvals.sphere < 0) { // Couldn't include that many
        ...
        for (;;) {
          ...
          if (Math.abs(vals.sphere) < min_val) {
            min_val = (float) Math.abs(vals.sphere);
            min_pos = root;
   ...
   */
  private void calculateSphereSize(float scale, KStarWrapper params) {
    int i;
    float sphereSize, minprob = 1.0f;
    float pstar; // P*(b|a)
    float pstarSum = 0.0f; // sum(P*)
    float pstarSquareSum = 0.0f; // sum(P*^2)
    float inc;
   ...
   * @return the scale factor value
   */
  private float scaleFactorUsingEntropy() {
    ...
    int j, itcount;
    float lowest = -1.0f, nextlowest = -1.0f;
    float root, up, bot, stepsize, delta;
    float randscale;
    float bestdiff, bestroot, currentdiff, lastdiff;
    float bestpsum, bestminprob, scale = 1.0f;
    ...
    if (nextlowest == -1 || lowest == -1) { // Data values are all the same
      ...
      root = 1.0f / (nextlowest - lowest);
      // root is bracketed in interval [bot,up]
      bot = 0.0f + ROOT_FINDER_ACCURACY / 2;
   ...
   */
  private void calculateEntropy(float scale, KStarWrapper params) {
    int i, j, k;
    float actent = 0.0f, randent = 0.0f;
    float pstar, tprob, avgprob = 0.0f, minprob = 1.0f;
    float actClassProb, randClassProb;
    float[][] pseudoClassProbs = new float[NUM_RAND_COLS + 1][m_NumClasses];
    // init
    for (j = 0; j <= NUM_RAND_COLS; j++) {
      for (i = 0; i < m_NumClasses; i++) {
        pseudoClassProbs[j][i] = 0.0f;
      }
    }
    for (i = 0; i < m_NumInstances; i++) {
      if (m_Distances[i] < 0) {
   ...
   */
  private float PStar(float x, float scale) {
    return (float) (scale * Math.exp(-2.0f * x * scale));
  }
