...
public class KStarCache implements Serializable, RevisionHandler {
   ...
   *          missing value.
   */
  public void store(float key, float value, float pmiss) {
   ...
   */
  public TableEntry getCacheValues(float key) {
    if (m_Cache.containsKey(key)) {
  ...
  public class CacheTable implements Serializable, RevisionHandler {
     ...
     */
    public void insert(float key, float value, float pmiss) {
     ...
     */
    public TableEntry getEntry(float key) {
      TableEntry[] table = m_Table;
      int hash = hashCode(key);
      int index = (hash & 0x7FFFFFFF) % table.length;
      for (TableEntry e = table[index]; e != null; e = e.next) {
     ...
     */
    private int hashCode(float key) {
  ...
  public class TableEntry implements Serializable, RevisionHandler {
    ...
    /** Constructor */
    public TableEntry(int hash, float key, float value, float pmiss,
      TableEntry next) {
      this.hash = hash;
      this.key = key;
      this.value = value;
