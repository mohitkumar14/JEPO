...
public class KStarNominalAttribute implements KStarConstants, RevisionHandler {
  ...
  public float transProb() {
    ...
    if (m_Train.isMissing(m_AttrIndex)) {
      ...
      try {
        transProb = (1.0f - m_Stop) / m_Test.attribute(m_AttrIndex).numValues();
   ...
   * 
   */
  private float stopProbUsingEntropy() {
    ...
    int itcount = 0;
    float stopProb;
    float lower, upper, pstop;
    float bestminprob = 0.0f, bestpsum = 0.0f;
    float bestdiff = 0.0f, bestpstop = 0.0f;
    float currentdiff, lastdiff, stepsize, delta;
    ...
    lower = 0.0f + ROOT_FINDER_ACCURACY / 2.0f;
    upper = 1.0f - ROOT_FINDER_ACCURACY / 2.0f;
   ...
   * 
   */
  private void calculateEntropy(float stop, KStarWrapper params) {
    int i, j, k;
    Instance train;
    float actent = 0.0f, randent = 0.0f;
    float pstar, tprob, psum = 0.0f, minprob = 1.0f;
    float actClassProb, randClassProb;
    float[][] pseudoClassProb = new float[NUM_RAND_COLS + 1][m_NumClasses];
    // init ...
    for (j = 0; j <= NUM_RAND_COLS; j++) {
      for (i = 0; i < m_NumClasses; i++) {
        pseudoClassProb[j][i] = 0.0f;
      }
    }
   ...
   * 
   */
  private float stopProbUsingBlend() {
    int itcount = 0;
    float stopProb, aimfor;
    float lower, upper, tstop;
    ...
    aimfor = (m_TotalCount - m_Distribution[testvalue])
      * (float) m_BlendFactor / 100.0f + m_Distribution[testvalue];

    // Initial values for root finder
    tstop = 1.0f - m_BlendFactor / 100.0f;
    lower = 0.0f + ROOT_FINDER_ACCURACY / 2.0f;
    upper = 1.0f - ROOT_FINDER_ACCURACY / 2.0f;
   ...
   * 
   */
  private void calculateSphereSize(int testvalue, float stop,
    KStarWrapper params) {
    int i, thiscount;
    float tprob, tval = 0.0f, t1 = 0.0f;
    float sphere, minprob = 1.0f, transprob = 0.0f;
   ...
   * 
   */
  private float PStar(Instance test, Instance train, int col, float stop) {
    float pstar;
    int numvalues = 0;
    try {
