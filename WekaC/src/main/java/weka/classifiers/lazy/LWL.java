...
public class LWL 
  ...
  public String [] getOptions() {
    ...
    options.add("-U"); options.add(String.valueOf(getWeightingKernel()));
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    if (m_Debug) {
      ...
      System.out.println(new StringBuilder("For ").append(k).append(" kept ").append(neighbours.numInstances()).
    		  append(" out of ").append(m_Train.numInstances()).append(" instances."));
    }
	...
	System.out.println(String.valueOf(distances[i]));
      }
    }

    // Determine the bandwidth
    float bandwidth = distances[k-1];
    ...
    for (int i = 0; i < distances.length; i++) {
      switch (m_WeightKernel) {
        case LINEAR:
          distances[i] = 1.0001f - distances[i];
          break;
        case EPANECHNIKOV:
          distances[i] = (float) (3/4D*(1.0001f - distances[i]*distances[i]));
          break;
        case TRICUBE:
          distances[i] = (float) Math.pow( (1.0001 - Math.pow(distances[i], 3)), 3 );
          break;
        case CONSTANT:
          ...
          break;
        case INVERSE:
          distances[i] = 1.0f / (1.0f + distances[i]);
          break;
        case GAUSS:
          distances[i] = (float) Math.exp(-distances[i] * distances[i]);
          break;
      }
    }
    ...
    float sumOfWeights = 0, newSumOfWeights = 0;
    for (int i = 0; i < distances.length; i++) {
  ...
  public String toString() {
    ...
    result += new StringBuilder("Using classifier: ").append(m_Classifier.getClass().getName() ).append("\n");
    ...
    if(m_UseAllK)
    	result += "Using all neighbours";
    else
    	result += new StringBuilder("Using ").append(m_kNN).append(" neighbours");
    return result;
  }
