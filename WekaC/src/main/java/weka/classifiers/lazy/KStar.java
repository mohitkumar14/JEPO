...
public class KStar 
  ...
  public float [] distributionForInstance(Instance instance) throws Exception {
    ...
    float transProb = 0.0f, temp = 0.0f;
    float [] classProbability = new float[m_NumClasses];
    float [] predictedValue = new float[1];

    // initialization ...
    for (int i=0; i<classProbability.length; i++) {
      classProbability[i] = 0.0f;
    }
    predictedValue[0] = 0.0f;
    if (m_InitFlag == ON) {
    ...
    if (m_ClassType == Attribute.NOMINAL) {
      float sum = Utils.sum(classProbability);
      if (sum <= 0.0)
	for (int i=0; i<classProbability.length; i++)
	  classProbability[i] = (float) 1/ (float) m_NumClasses;
      else Utils.normalize(classProbability, sum);
      return classProbability;
    }
    else {
    	if((temp != 0))
    		predictedValue[0] =predictedValue[0] / temp ;
    	else
    		predictedValue[0] =0.0f ;
   ...
   */
  private float instanceTransformationProbability(Instance first, 
						   Instance second) {
    float transProb = 1.0f;
    int numMissAttr = 0;
    for (int i = 0; i < m_NumAttributes; i++) {
	...
	transProb = (float) Math.pow(transProb, (float)m_NumAttributes / 
   ...
   */
  private float attrTransProb(Instance first, Instance second, int col) {
  ...
  public String [] getOptions() {
    ...
    options.add("-B");
    options.add(String.valueOf(m_GlobalBlend));
