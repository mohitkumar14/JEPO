...
public class TreeModel extends PMMLClassifier implements Drawable {
  ...
  static class ScoreDistribution implements Serializable {
    ...
    private final float m_recordCount;

    /** The optional confidence value */
    private float m_confidence = Utils.missingValue();
    ...
    protected ScoreDistribution(Element scoreE, MiningSchema miningSchema,
      float baseCount) throws Exception {
      // get the label
     ...
     */
    void deriveConfidenceValue(float baseCount) {
    ...
    public String toString() {
      return new StringBuilder(m_classLabel).append(": ").append(m_recordCount).append(" (")
    		  .append(Utils.doubleToString(m_confidence, 2)).append(") ").toString();
    }
  }
  ...
  static abstract class Predicate implements Serializable {
     ...
     */
    abstract Eval evaluate(float[] input);
  ...
  static class True extends Predicate {
    ...
    @Override
    public Predicate.Eval evaluate(float[] input) {
  ...
  static class SimplePredicate extends Predicate {
    ...
    enum Operator {
      EQUAL("equal") {
        @Override
        Predicate.Eval evaluate(float[] input, float value, int fieldIndex) {
      ...
      abstract Predicate.Eval evaluate(float[] input, float value,
        int fieldIndex);
    ...
    public SimplePredicate(Element simpleP, MiningSchema miningSchema) throws Exception {
      ...
      if (m_operator != Operator.ISMISSING
        ...
        if (att.isNumeric()) {
          m_value = Float.parseFloat(valueS);
        } else {
    ...
    public String toString() {
      ...
      temp.append(m_fieldName).append(" ").append(m_operator.shortName());
      if (m_operator != Operator.ISMISSING
        && m_operator != Operator.ISNOTMISSING) {
    	  if(m_isNominal)
    		  temp.append(" ").append( m_nominalValue).append(m_value);
    	  else
    		  temp.append(" ").append( m_value);
      }
  ...
  static class CompoundPredicate extends Predicate {
    ...
    public String toString(int level, boolean cr) {
      ...
      text.append("Compound [").append(m_booleanOperator.toString()).append("]");
      if (cr) {
        text.append("\\n");
      } else {
        text.append("\n");
  ...
  static class SimpleSetPredicate extends Predicate {
    ...
    enum BooleanOperator {
      IS_IN("isIn") {
        @Override
        Predicate.Eval evaluate(float[] input, int fieldIndex, Array set,
      ...
      abstract Predicate.Eval evaluate(float[] input, int fieldIndex,
    ...
    public String toString() {
      ...
      temp.append(m_fieldName).append(" ").append(m_operator.toString()).append(" ");
  ...
  class TreeNode implements Serializable {
    ...
    /** ID for this node */
    private String m_ID = String.valueOf(this.hashCode());
    ...
    private float m_scoreNumeric = Utils.missingValue();

    /** The record count at this node (if defined) */
    private float m_recordCount = Utils.missingValue();
    ...
    protected TreeNode(Element nodeE, MiningSchema miningSchema) throws Exception {
      ...
      if (scoreS != null && scoreS.length() > 0) {
        ...
        if (classAtt.isNumeric()) {
          try {
            m_scoreNumeric = Float.parseFloat(scoreS);
     ...
     */
    protected float getScoreNumeric() {
     ...
     */
    protected float getRecordCount() {
      return m_recordCount;
    }

    protected void dumpGraph(StringBuffer text) throws Exception {
      text.append("N").append(m_ID).append(" ");
      if (m_scoreString != null) {
        text.append("[label=\"score=").append(m_scoreString);
      }
      ...
      for (TreeNode c : m_childNodes) {
        text.append("N").append(m_ID).append("->").append("N").append(c.getID());
        text.append(" [label=\"").append(c.getPredicate().toString(0, true));
        text.append("\"]\n");
        c.dumpGraph(text);
      }
    }
    ...
    protected void dumpTree(int level, StringBuffer text) {
      if (m_childNodes.size() > 0) {
        ...
        if (!Utils.isMissingValue(m_scoreNumeric)) {
          ...
          text.append(m_scoreString).append(" ");
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] score(float[] instance, Attribute classAtt)
      throws Exception {
      float[] preds = null;
      ...
      if (classAtt.isNumeric()) {
        ...
        preds = new float[classAtt.numValues()];
      }
     ...
     * @throws Exception if something goes wrong.
     */
    protected void doLeaf(Attribute classAtt, float[] preds) throws Exception {
      if (classAtt.isNumeric()) {
        preds[0] = m_scoreNumeric;
      } else {
        if (m_scoreDistributions.size() == 0) {
          preds[m_scoreIndex] = 1.0f;
        } else {
     ...
     * @throws Exception if something goes wrong.
     */
    protected void doNoTrueChild(Attribute classAtt, float[] preds)
      throws Exception {
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyWeightedConfidence(
      float[] instance, Attribute classAtt) throws Exception {
      ...
      if (strategyInvoked) {
        ...
        float[][] dists = new float[nodeCount][];
    ...
    protected float[] freqCountsForAggNodesStrategy(float[] instance,
      Attribute classAtt) throws Exception {

      float[] counts = new float[classAtt.numValues()];
      ...
      if (m_childNodes.size() > 0) {
        ...
        for (TreeNode c : m_childNodes) {
          if (c.getPredicate().evaluate(instance) == Predicate.Eval.TRUE
            ...
            float[] temp = c.freqCountsForAggNodesStrategy(instance, classAtt);
            for (int i = 0; i < classAtt.numValues(); i++) {
              counts[i] += temp[i];
            }
          }
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyAggregateNodes(float[] instance,
      Attribute classAtt) throws Exception {
      ...
      if (strategyInvoked) {
        float[] aggregatedCounts = freqCountsForAggNodesStrategy(instance,
          classAtt);
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyDefaultChild(float[] instance,
      Attribute classAtt) throws Exception {
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyLastPrediction(float[] instance,
      Attribute classAtt) throws Exception {
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyNullPrediction(float[] instance,
      Attribute classAtt) throws Exception {
     ...
     * @throws Exception if something goes wrong.
     */
    protected float[] missingValueStrategyNone(float[] instance,
      Attribute classAtt) throws Exception {
   ...
   */
  protected float m_missingValuePenalty = Utils.missingValue();
  ...
  public TreeModel(Element model, Instances dataDictionary,
    ...
    if (missingP != null && missingP.length() > 0) {
      ...
      try {
        m_missingValuePenalty = Float.parseFloat(missingP);
      } catch (NumberFormatException ex) {
  ...
  public float[] distributionForInstance(Instance inst) throws Exception {
    ...
    if (m_miningSchema.getFieldsAsInstances().classAttribute().isNumeric()) {
      ...
      preds = new float[m_miningSchema.getFieldsAsInstances().classAttribute()
        .numValues()];
    }
  ...
  public String toString() {
    ...
    temp.append("Split-type: ").append(m_splitCharacteristic).append("\n");
    temp.append("No true child strategy: ").append(m_noTrueChildStrategy).append("\n");
    temp.append("Missing value strategy: ").append(m_missingValueStrategy).append("\n");
