...
public class Regression extends PMMLClassifier
  ...
  static class RegressionTable implements Serializable {
    ...
    abstract static class Predictor implements Serializable {
      ...
      protected float m_coefficient = 1.0f;
      ...
      protected Predictor(Element predictor, Instances miningSchema) throws Exception {
        ...
        if (coeff.length() > 0) {
          m_coefficient = Float.parseFloat(coeff);
        }
      }
       ...
       */
      public abstract void add(float[] preds, float[] input);
    }
    ...
    protected class NumericPredictor extends Predictor {
      ...
      /** The exponent*/
      protected float m_exponent = 1.0f;
      ...
      protected NumericPredictor(Element predictor, 
        ...
        if (exponent.length() > 0) {
          m_exponent = Float.parseFloat(exponent);
        }
      }
       ...
       * @param input the input instance's values
       */
      public void add(float[] preds, float[] input) {
    ...
    protected class CategoricalPredictor extends Predictor {
      ...
      public String toString() {
        String output = super.toString();
        output += new StringBuilder(m_name).append("=").append(m_valueName);
        return output;
      }
    ...
    protected class PredictorTerm implements Serializable {
      ...
      public String toString() {
        ...
        result.append("(").append(Utils.doubleToString(m_coefficient, 12, 4));
        for (int i = 0; i < m_fieldNames.length; i++) {
          result.append(" * ").append(m_fieldNames[i]);
        }
        result.append(")");
    ...
    /** The intercept */
    protected float m_intercept = 0.0f;
    ...
    public String toString() {
      ...
      if (m_functionType == CLASSIFICATION) {
        temp.append("=").append(miningSchema.
      ...
      for (int i = 0; i < m_predictors.size(); i++) {
        temp.append(m_predictors.get(i).toString()).append(" +\n");
      }
      
      // do the predictor terms
      for (int i = 0; i < m_predictorTerms.size(); i++) {
        temp.append(m_predictorTerms.get(i).toString()).append(" +\n");
      }
    ...
    public void predict(float[] preds, float[] input) {
      if (m_targetCategory == -1) {
        preds[0] = m_intercept;
      } else {
  ...
  public String toString() {
    ...
    if (m_algorithmName != null) {
      temp.append("\nPMML Model: ").append(m_algorithmName);
    }
    temp.append("\n\n");
    ...
    if (m_normalizationMethod != Normalization.NONE) {
      temp.append("Normalization: ").append(m_normalizationMethod);
    }
    temp.append("\n");
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
        ...
        if(m_miningSchema.getFieldsAsInstances().classAttribute().isNominal() ||
        	...
        	message= "[Regression] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made (zero probabilities output).";
        else
        	message= "[Regression] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made NaN output).";
      ...
      case LOGIT:
        for (int i = 0; i < preds.length; i++) {
          preds[i] = (float) (1.0 / (1.0 + Math.exp(-preds[i])));
        }
        Utils.normalize(preds);
        break;
      case PROBIT:
      ...
      case CLOGLOG:
        ...
        for (int i = 0; i < preds.length; i++) {
          preds[i] = (float) (1.0 - Math.exp(-Math.exp(-preds[i])));
        }
        Utils.normalize(preds);
        break;
      case EXP:
      ...
      case LOGLOG:
        // note this is supposed to be illegal for regression
        for (int i = 0; i < preds.length; i++) {
          preds[i] = (float) Math.exp(-Math.exp(-preds[i]));
        }
        Utils.normalize(preds);
        break;
      case CAUCHIT:
        for (int i = 0; i < preds.length; i++) {
          preds[i] = (float) (0.5 + (1.0 / Math.PI) * Math.atan(preds[i]));
        }
        Utils.normalize(preds);
        break;
      default:
