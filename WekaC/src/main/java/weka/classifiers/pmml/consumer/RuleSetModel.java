...
public class RuleSetModel extends PMMLClassifier {
  ...
  static abstract class Rule implements Serializable {
     ...
     */
    public abstract void fires(float[] input, ArrayList<SimpleRule> ruleCollection);
  ...
  static class SimpleRule extends Rule {
     ...
     * confidence (optional).
     */
    protected float m_weight = Utils.missingValue();
    ...
    public String toString(String prefix, int indent) {
      ...
      temp.append(prefix).append("Simple rule: ").append(m_predicate).append("\n");
      temp.append(prefix).append(" => ").append(m_scoreString).append("\n");
      if (!Utils.isMissingValue(m_recordCount)) {
        temp.append(prefix).append(" recordCount: ").append(m_recordCount).append("\n");
      }
      if (!Utils.isMissingValue(m_nbCorrect)) {
        temp.append(prefix).append("   nbCorrect: ").append(m_nbCorrect).append("\n");
      }
      if (!Utils.isMissingValue(m_confidence)) {
        temp.append(prefix).append("  confidence: ").append(m_confidence).append("\n");
      }
      if (!Utils.isMissingValue(m_weight)) {
        temp.append(prefix).append("      weight: ").append(m_weight).append("\n");
      }
    ...
    public SimpleRule(Element ruleE, MiningSchema miningSchema) throws Exception {
      ...
      if (classAtt.isNumeric()) {
        m_score = Float.parseFloat(m_scoreString);
      } else {
      ...
      if (weight != null && weight.length() > 0) {
        m_weight = Float.parseFloat(weight);
      }
     ...
     * @param ruleCollection the array list to add any firing rules into
     */
    public void fires(float[] input, ArrayList<SimpleRule> ruleCollection) {
     ...
     * @throws Exception if something goes wrong
     */
    public float[] score(float[] instance, Attribute classAtt) 
      throws Exception {
      ...
      if (classAtt.isNumeric()) {
        ...
        if (m_scoreDistributions.size() > 0) {
          ...
          preds[classAtt.indexOfValue(m_scoreString)] = 1.0f;
        }
      }      
  ...
  static class CompoundRule extends Rule {
    ...
    public String toString(String prefix, int indent) {
      ...
      temp.append(prefix).append("Compound rule: ").append(m_predicate).append("\n");
  ...
  static class RuleSet implements Serializable {
     ...
     */
    private float m_nbCorrect = Utils.missingValue();
     ...
     * index) 
     * */
    private float m_defaultPrediction = Utils.missingValue();
     ...
     */
    private float m_defaultConfidence = Utils.missingValue();
    ...
    public String toString() {
      ...
      temp.append("Rule selection method: ").append(m_currentMethod).append("\n");
      if (m_defaultScore != null) {
        temp.append("Default prediction: ").append(m_defaultScore).append("\n");
        
        if (!Utils.isMissingValue(m_recordCount)) {
          temp.append("       recordCount: ").append(m_recordCount).append("\n");
        }
        if (!Utils.isMissingValue(m_nbCorrect)) {
          temp.append("         nbCorrect: ").append(m_nbCorrect).append("\n");
        }
        if (!Utils.isMissingValue(m_defaultConfidence)) {
          temp.append(" defaultConfidence: ").append(m_defaultConfidence).append("\n");
        }
    ...
    public RuleSet(Element ruleSetNode, MiningSchema miningSchema) 
      ...
      if (defaultScore != null && defaultScore.length() > 0) {
        ...
        if (classAtt.isNumeric()) {
          m_defaultPrediction = Float.parseFloat(defaultScore);
        } else {
      ...
      if (defaultConfidence != null && defaultConfidence.length() > 0) {
        m_defaultConfidence = Float.parseFloat(defaultConfidence);
      }
      ...
      if (firingRules.size() > 0) {
        ...
        if (classAtt.isNumeric()) {
          ...
          if (m_scoreDistributions.size() > 0) {
            ...
            preds[(int)m_defaultPrediction] = 1.0f;
          }
        }
      }
  ...
  public String toString() {
    ...
    if (m_algorithmName != null) {
      temp.append("\nAlgorithm: ").append(m_algorithmName).append("\n");
    }
