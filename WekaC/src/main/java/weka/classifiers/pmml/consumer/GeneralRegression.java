...
public class GeneralRegression extends PMMLClassifier
  ...
  enum CumulativeLinkFunction {
    NONE ("none") {
      float eval(float value, float offset) {
        return Float.NaN; // no evaluation defined in this case!
      }
    },
    LOGIT ("logit") {
      float eval(float value, float offset) {
        return (float) (1.0 / (1.0 + Math.exp(-(value + offset))));
      }
    },
    PROBIT ("probit") {
      float eval(float value, float offset) {
    ...
    CLOGLOG ("cloglog") {
      float eval(float value, float offset) {
        return (float) (1.0f - Math.exp(-Math.exp(value + offset)));
      }
    },
    LOGLOG ("loglog") {
      float eval(float value, float offset) {
        return (float) Math.exp(-Math.exp(-(value + offset))); 
      }
    },
    CAUCHIT ("cauchit") {
      float eval(float value, float offset) {
        return (float) (0.5f + (1.0f / Math.PI) * Math.atan(value + offset));
      }
    };
     ...
     */
    abstract float eval(float value, float offset);
  ...
  enum LinkFunction {
    NONE ("none") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return Float.NaN; // no evaluation defined in this case!
      }
    },
    CLOGLOG ("cloglog") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) ((1.0 - Math.exp(-Math.exp(value + offset))) * trials);
      }
    },
    IDENTITY ("identity") {
      float eval(float value, float offset, float trials,
    ...
    LOG ("log") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) (Math.exp(value + offset) * trials);
      }
    },
    LOGC ("logc") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) ((1.0 - Math.exp(value + offset)) * trials);
      }
    },
    LOGIT ("logit") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) ((1.0 / (1.0 + Math.exp(-(value + offset)))) * trials);
      }
    },
    LOGLOG ("loglog") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) (Math.exp(-Math.exp(-(value + offset))) * trials);
      }
    },
    NEGBIN ("negbin") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) ((1.0 / (distParam * (Math.exp(-(value + offset)) - 1.0))) * trials);
      }
    },
    ODDSPOWER ("oddspower") {
      float eval(float value, float offset, float trials,
                  float distParam, float linkParam) {
        return (float) ((linkParam < 0.0 || linkParam > 0.0)
     ...
     * @return the result of the link function
     */
    abstract float eval(float value, float offset, float trials, 
                         float distParam, float linkParam);
  ...
  protected float m_linkParameter = Float.NaN;
  protected String m_trialsVariable;
  protected float m_trialsValue = Float.NaN;
  ...
  protected float m_distParameter = Float.NaN;
  ...
  protected float m_offsetValue = Float.NaN;
  ...
  public GeneralRegression(Element model, Instances dataDictionary,
    ...
    if (m_modelType == ModelType.ORDINALMULTINOMIAL) {
      ...
      if (linkP != null && linkP.length() > 0) {
        try {
          m_linkParameter = Float.parseFloat(linkP);
      ...
      if (trialsV != null && trialsV.length() > 0) {
        try {
          m_trialsValue = Float.parseFloat(trialsV);
        } catch (IllegalArgumentException ex) {
    ...
    if (distP != null && distP.length() > 0) {
      try {
        m_distParameter = Float.parseFloat(distP);
      } catch (IllegalArgumentException ex) {
    ...
    if (offsetVal != null && offsetVal.length() > 0) {
      try {
        m_offsetValue = Float.parseFloat(offsetVal);
      } catch (IllegalArgumentException ex) {
      ...
      if (ppM.getNodeType() == Node.ELEMENT_NODE) {
        ...
        for (int i = 0; i < cellL.getLength(); i++) {
          ...
          if (cell.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (p != null) {
              try {
                expOrIndex = Float.parseFloat(value);
  ...
  private void readParamMatrix(Element model) throws Exception {
    ...
    for (int i = 0; i < pcellL.getLength(); i++) {
      ...
      if (pcell.getNodeType() == Node.ELEMENT_NODE) {
        ...
        try {
          p.m_beta = Float.parseFloat(coefficient);
        } catch (IllegalArgumentException ex) {
  ...
  public String toString() {
    ...
    if (m_linkFunction != LinkFunction.NONE) {
      temp.append("Link function: ").append(m_linkFunction);
      if (m_offsetVariable != null) {
        temp.append("\n\tOffset variable ").append(m_offsetVariable);
      } else if (!Float.isNaN(m_offsetValue)) {
        temp.append("\n\tOffset value ").append(m_offsetValue);
      }
      
      if (m_trialsVariable != null) {
        temp.append("\n\tTrials variable ").append(m_trialsVariable);
      } else if (!Float.isNaN(m_trialsValue)) {
        temp.append("\n\tTrials value ").append(m_trialsValue);
      }
      
      if (m_distribution != Distribution.NONE) {
        temp.append("\nDistribution: ").append(m_distribution);
      }
      
      if (m_linkFunction == LinkFunction.NEGBIN &&
          m_distribution == Distribution.NEGBINOMIAL &&
          !Float.isNaN(m_distParameter)) {
        temp.append("\n\tDistribution parameter ").append(m_distParameter);
      }
      
      if (m_linkFunction == LinkFunction.POWER ||
          m_linkFunction == LinkFunction.ODDSPOWER) {
        if (!Float.isNaN(m_linkParameter)) {
          temp.append("\n\nLink parameter ").append(m_linkParameter);
        }
      }
    }
    
    if (m_cumulativeLinkFunction != CumulativeLinkFunction.NONE) {
      temp.append("Cumulative link function: ").append(m_cumulativeLinkFunction);
  ...
  protected void printPPMatrix(StringBuffer buff) {
    ...
    for (int i = 0; i < m_parameterList.size(); i++) {
      for (int j = 0; j < miningSchemaI.numAttributes(); j++) {
        if (m_ppMatrix[i][j] != null) {
          float width = (float) (Math.log(Math.abs(m_ppMatrix[i][j].m_value)) /
            Math.log(10.0));
          if (width < 0) {
            width = 1;
          }
    ...
    buff.append("\n").append(PMMLUtils.pad("Parameter", " ", maxParamWidth - "Parameter".length(), false));
    // attribute names
    for (int i = 0; i < miningSchemaI.numAttributes(); i++) {
        ...
        if (j != miningSchemaI.classIndex()) {
          ...
          if (p != null) {
            if (miningSchemaI.attribute(j).isNominal() ||
              ...
              val = String.valueOf(Utils.doubleToString(p.m_value, maxAttWidth, 4).trim());
            }
          }
  ...
  protected void printParameterMatrix(StringBuffer buff) {
    ...
    for (int i = 0; i < m_paramMatrix.length; i++) {
      for (int j = 0; j < m_parameterList.size(); j++) {
        ...
        if (p != null) {
          float width = (float) (Math.log(Math.abs(p.m_beta)) / Math.log(10));
          if (width < 0) {
            width = 1;
          }
          // decimal + # decimal places + 1
   ...
   * parameter vector
   */
  private float[] incomingParamVector(float[] incomingInst) throws Exception {
    Instances miningSchemaI = m_miningSchema.getFieldsAsInstances();
    float[] incomingPV = new float[m_parameterList.size()];
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
    	  ...
    	  if(m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()
    			  ...
    			  message= "[GeneralRegression] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made (zero probabilities output).";
    	  else
    		  message= "[GeneralRegression] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made (NaN output).";
        if (m_log == null) {
      ...
      float[] inputParamVector = incomingParamVector(incoming);
   ...
   */
  private void computeResponses(float[] incomingInst, 
                                float[] incomingParamVector,
                                float[] responses) throws Exception {
   ...
   */
  private static void computeProbabilitiesMultinomialLogistic(float[] responses) {
    float[] r = new float[responses.length];
    System.arraycopy(responses, 0, r, 0, responses.length);
   ...
   */
  private void computeResponseGeneralizedLinear(float[] incomingInst, 
                                                float[] responses) 
    throws Exception {
    float[] r = new float[responses.length];
    System.arraycopy(responses, 0, r, 0, responses.length);
    ...
    if (m_linkFunction == LinkFunction.NEGBIN && 
        m_distribution == Distribution.NEGBINOMIAL) {
      if (Float.isNaN(m_distParameter)) {
   ...
   * @throws Exception if a problem occurs. 
   */
  private void computeResponseOrdinalMultinomial(float[] incomingInst, 
                                                  float[] responses) throws Exception {
    ...
    for (int i = 0; i < r.length; i++) {
      if (i == 0) {
        ...
        responses[i] = 1.0f - responses[i - 1];
      } else {
