...
public class SupportVectorMachineModel extends PMMLClassifier 
  ...
  static abstract class Kernel implements Serializable {
     ...
     */
    public abstract float evaluate(VectorInstance x, VectorInstance y) 
      throws Exception;
     ...
     * @throws Exception if something goes wrong
     */
    public abstract float evaluate(VectorInstance x, float[] y) 
      throws Exception;
  ...
  static class LinearKernel extends Kernel implements Serializable {
     ...
     * @throws Exception if something goes wrong
     */
    public float evaluate(VectorInstance x, VectorInstance y) 
      throws Exception {
     ...
     * @throws Exception if something goes wrong
     */
    public float evaluate(VectorInstance x, float[] y)
      throws Exception {
      return x.dotProduct(y);
    }
  ...
  static class PolynomialKernel extends Kernel implements Serializable {
    ...
    protected float m_gamma = 1;
    protected float m_coef0 = 1;
    protected float m_degree = 1;
    ...
    public PolynomialKernel(Element polyNode, Logger log) {
      ...
      if (gammaString != null && gammaString.length() > 0) {
        try {
          m_gamma = Float.parseFloat(gammaString);
      ...
      if (coefString != null && coefString.length() > 0) {
        try {
          m_coef0 = Float.parseFloat(coefString);
        } catch (NumberFormatException e) {
      ...
      if (degreeString != null && degreeString.length() > 0) {
        try {
          m_degree = Float.parseFloat(degreeString);
        } catch (NumberFormatException e) {
    ...
    public float evaluate(VectorInstance x, VectorInstance y)
      throws Exception {
      float dotProd = x.dotProduct(y);
      return (float) Math.pow(m_gamma * dotProd + m_coef0, m_degree);      
    }
     ...
     */
    public String toString() {
      return new StringBuilder("Polynomial kernel: K(x,y) = (").append(m_gamma).append(" * <x,y> + " 
    		  ).append(m_coef0).append(")^").append(m_degree).toString();
    }
  }
  ...
  static class RadialBasisKernel extends Kernel implements Serializable {
    ...
    throws Exception {
      ...
      float result = -m_gamma * diff.dotProduct(diff);
...
//      System.err.println("Result: " + result);
     ...
     */
    public String toString() {
      return new StringBuilder("Radial kernel: K(x,y) = exp(-").append(m_gamma).append(" * ||x - y||^2)").toString(); 
    }    
  }
  ...
  static class SigmoidKernel extends Kernel implements Serializable {
    ...
    public float evaluate(VectorInstance x, VectorInstance y)
      ...
      float dotProd = x.dotProduct(y);
      float z = m_gamma * dotProd + m_coef0;
     ...
     */
    public String toString() {
      return new StringBuilder("Sigmoid kernel: K(x,y) = tanh(").append(m_gamma).append(" * <x,y> + ")
    		  .append(m_coef0).append(")").toString();
    }
  }
  ...
  static class SupportVectorMachine implements Serializable {
    ...
    protected float m_localThreshold = Float.MAX_VALUE; // default - not set
    ...
    /** The constant term - b */
    protected float m_intercept = 0;
    
    /** The coefficients for the vectors */
    protected float[] m_coefficients;
     ...
     * @throws Exception if something goes wrong
     */
    public void distributionForInstance(float[] input, Kernel kernel, 
        VectorDictionary vecDict, float[] preds, classificationMethod cMethod,
...
/*      if (result < 0 && m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
      ...
      if (cMethod == classificationMethod.NONE || 
        ...
        if (cMethod == classificationMethod.ONE_AGAINST_ALL) {
          ...
          // one-vs-one
          float threshold = (m_localThreshold < Float.MAX_VALUE)
            ? m_localThreshold
            : globalThreshold;
...
//      preds[targetIndex] = result;
    ...
    public SupportVectorMachine(Element machineElement, 
      ...
      if (intercept != null && intercept.length() > 0) {
        m_intercept = Float.parseFloat(intercept);
      }
    ...
    public String toString() {
      ...
      if (m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
        temp.append(" (target category = ").append(m_targetCategory).append(")");
        if (m_localAlternateTargetCategoryIndex != -1) {
          temp.append("\n (alternate category = ")
          .append(m_miningSchema.getFieldsAsInstances().classAttribute().
                value(m_localAlternateTargetCategoryIndex)).append(")");
        }
      }
      temp.append("\n\n");
      
      for (int i = 0; i < m_supportVectors.size(); i++) {
        temp.append("\n").append(m_coefficients[i]).append(" * [")
        .append(m_supportVectors.get(i).getValues()).append(" * X]");
      }
  ...
  public SupportVectorMachineModel(Element model, Instances dataDictionary,
    ...
    if (thresholdS != null && thresholdS.length() > 0) {
      m_threshold = Float.parseFloat(thresholdS);
    }
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
        ...
        if(m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()
        	...
        	message= "[SupportVectorMachineModel] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made (zero probabilities output).";
        else
        	message= "[SupportVectorMachineModel] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made NaN output).";
