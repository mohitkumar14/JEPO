...
public class NeuralNetwork extends PMMLClassifier {
  ...
  static class NeuralInput implements Serializable {
    ...
    protected float getValue(float[] incoming) throws Exception {
    ...
    public String toString() {
      ...
      temp.append("Nueral input (").append(getID()).append(")\n");
      temp.append(m_field);
  ...
  class NeuralLayer implements Serializable {
    ...
    private float m_layerThreshold = Float.NaN; 
    
    /** Width (if defined overrides one in NeuralNetwork) */
    private float m_layerWidth = Float.NaN;
    
    /** Altitude (if defined overrides one in NeuralNetwork) */
    private float m_layerAltitude = Float.NaN;
    ...
    private HashMap<String, Float> m_layerOutput = new HashMap<String, Float>();
    ...
    protected NeuralLayer(Element layerE) {
      ...
      if (threshold != null && threshold.length() > 0) {
        m_layerThreshold = Float.parseFloat(threshold);
      } else {
      ...
      if (width != null && width.length() > 0) {
        m_layerWidth = Float.parseFloat(width);
      } else {
      ...
      if (altitude != null && altitude.length() > 0) {
        m_layerAltitude = Float.parseFloat(altitude);
      } else {
     ...
     */
    protected HashMap<String, Float> computeOutput(HashMap<String, Float> incoming) 
      throws Exception {
      ...
      for (int i = 0; i < m_layerNeurons.length; i++) {
        float neuronOut = m_layerNeurons[i].getValue(incoming);
      ...
      if (m_layerNormalization != Normalization.NONE) {
        for (int i = 0; i < m_layerNeurons.length; i++) {
          float val = m_layerOutput.get(m_layerNeurons[i].getID());
//          System.err.println("Normalizing ID " + m_layerNeurons[i].getID() + " " + val);
          if (m_layerNormalization == Normalization.SOFTMAX) {
            val = (float) (Math.exp(val) / normSum);
          } else {
            val = (val / normSum);
          }
    ...
    public String toString() {
      ...
      temp.append("activation: ").append(getActivationFunction()).append("\n");
      if (!Float.isNaN(getThreshold())) {
        temp.append("threshold: ").append(getThreshold()).append("\n");
      }
      if (!Float.isNaN(getWidth())) {
        temp.append("width: ").append(getWidth()).append("\n");
      }
      if (!Float.isNaN(getAltitude())) {
        temp.append("altitude: ").append(getAltitude()).append("\n");
      }
      temp.append("normalization: ").append(m_layerNormalization).append("\n");
      for (int i = 0; i < m_numNeurons; i++) {
        temp.append(m_layerNeurons[i]).append("\n");
      }
  ...
  static class Neuron implements Serializable {
    ...
    private float m_neuronWidth = Float.NaN;
    
    /** The altitude (if defined overrides the one in NeuralLayer or NeuralNetwork) */
    private float m_neuronAltitude = Float.NaN;
    ...
    protected Neuron(Element neuronE, NeuralLayer layer) {
      ...
      for (int i = 0; i < conL.getLength(); i++) {
        ...
        if (conN.getNodeType() == Node.ELEMENT_NODE) {
          ...
          m_weights[i] = Float.parseFloat(weight);
        }
      }
    }
     ...
     * located in the Map
     */
    protected float getValue(HashMap<String, Float> incoming) throws Exception {
      
      float z = 0;
      float result = Float.NaN;
      
      float width = (Float.isNaN(m_neuronWidth))
        ? m_layer.getWidth()
        : m_neuronWidth;

      z = m_bias;
      for (int i = 0; i < m_connectionIDs.length; i++) {
        Float inVal = incoming.get(m_connectionIDs[i]);
        if (inVal == null) {
        ...
        if (m_layer.getActivationFunction() != ActivationFunction.RADIALBASIS) {
          ...
          float inV = inVal.floatValue() * m_weights[i];
          z += inV;
        } else {
          // Euclidean distance to the center (stored in m_weights)
          float inV = (float) Math.pow((inVal.floatValue() - m_weights[i]), 2.0);
          z += inV;
        }
      }
      ...
      float threshold = m_layer.getThreshold();
      float altitude = (Float.isNaN(m_neuronAltitude))
        ? m_layer.getAltitude()
        : m_neuronAltitude;
        
      float fanIn = m_connectionIDs.length;        
    ...
    public String toString() {
      StringBuffer temp = new StringBuffer();
      temp.append("Nueron (").append(m_ID).append(") [bias:").append(m_bias);
      if (!Float.isNaN(m_neuronWidth)) {
        temp.append(" width:").append(m_neuronWidth);
      }
      if (!Float.isNaN(m_neuronAltitude)) {
        temp.append(" altitude:").append(m_neuronAltitude);
      }
      temp.append("]\n");
      temp.append("  con. (ID:weight): ");
      for (int i = 0; i < m_connectionIDs.length; i++) {
        temp.append(m_connectionIDs[i]).append(":").append(Utils.doubleToString(m_weights[i], 2));
  ...
  static class NeuralOutputs implements Serializable {
     ...
     */
    protected void getOuput(HashMap<String, Float> incoming, float[] preds) throws Exception {
      ...
      for (int i = 0; i < m_outputNeurons.length; i++) {
        Float neuronOut = incoming.get(m_outputNeurons[i]);
        if (neuronOut == null) {
      ...
      if (m_classAttribute.isNominal()) {
        ...
        float min = preds[Utils.minIndex(preds)];
        if (min < 0) {
    ...
    public String toString() {
      ...
      for (int i = 0; i < m_outputNeurons.length; i++) {
        temp.append("Output neuron (").append(m_outputNeurons[i]).append(")\n");
        temp.append("mapping:\n");
        if (m_classAttribute.isNumeric()) {
          temp.append(m_regressionMapping).append("\n");
        } else {
          temp.append(m_classAttribute.name()).append(" = ")
          .append(m_classAttribute.value(m_categoricalIndexes[i])).append("\n");
        }
      }
  ...
  enum ActivationFunction {
    THRESHOLD("threshold") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        if (z > threshold) {
          return 1.0f;
        }
        return 0.0f;
    ...
    TANH("tanh") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        float a = (float) Math.exp( z );
        float b = (float) Math.exp( -z );
        return ((a-b)/(a+b));
    ...
    COSINE("cosine") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        return (float) Math.cos(z);
      }
    },
    ELLICOT("ellicot") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        return (float) (z / (1.0 + Math.abs(z)));
      }
    },
    ARCTAN("arctan") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        return (float) (2.0 * Math.atan(z) / Math.PI);
      }
    },
    RADIALBASIS("radialBasis") {
      float eval(float z, float threshold, float altitude, float fanIn) {
        return (float) Math.exp(fanIn * Math.log(altitude) - z);
      }
    };
    
    abstract float eval(float z, float threshold, float altitude, float fanIn);
  ...
  protected float m_threshold = 0.0f; // default = 0
  
  /** Width for radial basis */
  protected float m_width = Float.NaN; // no default
  
  /** Altitude for radial basis */
  protected float m_altitude = 1.0f; // default = 1
  ...
  protected HashMap<String, Float> m_inputMap = new HashMap<String, Float>();
  ...
  public NeuralNetwork(Element model, Instances dataDictionary,
    ...
    if (thresh != null && thresh.length() > 0) {
      m_threshold = Float.parseFloat(thresh);
    }
    String width = model.getAttribute("width");
    if (width != null && width.length() > 0) {
      m_width = Float.parseFloat(width);
    }
    String alt = model.getAttribute("altitude");
    if (alt != null && alt.length() > 0) {
      m_altitude = Float.parseFloat(alt);
    }
   ...
   */
  public float[] distributionForInstance(Instance inst) throws Exception {
    if (!m_initialized) {
    ...
    if (m_miningSchema.getFieldsAsInstances().classAttribute().isNumeric()) {
      ...
      preds = new float[m_miningSchema.getFieldsAsInstances().classAttribute().numValues()];
    }
    
    float[] incoming = m_fieldsMap.instanceToSchema(inst, m_miningSchema);
    
    boolean hasMissing = false;
    for (int i = 0; i < incoming.length; i++) {
      if (i != m_miningSchema.getFieldsAsInstances().classIndex() && 
          Float.isNaN(incoming[i])) {
        hasMissing = true;
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
        ...
        if(m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()
        		...
        		message= "[NeuralNetwork] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made (zero probabilities output).";
        else
        	message= "[NeuralNetwork] WARNING: Instance to predict has missing value(s) but there is no missing value handling meta data and no prior probabilities/default value to fall back to. No prediction will be made NaN output).";
        if (m_log == null) {
      ...
      for (int i = 0; i < m_inputs.length; i++) {
        float networkInVal = m_inputs[i].getValue(incoming);
        String ID = m_inputs[i].getID();
        m_inputMap.put(ID, networkInVal);
      }
      
      // now compute the output of each layer
      HashMap<String, Float> layerOut = m_layers[0].computeOutput(m_inputMap);
  ...
  public String toString() {
    ...
    temp.append("PMML version ").append(getPMMLVersion());
    if (!getCreatorApplication().equals("?")) {
      temp.append("\nApplication: ").append(getCreatorApplication());
    }
    ...
    temp.append("Inputs:\n");
    for (int i = 0; i < m_inputs.length; i++) {
      temp.append(m_inputs[i]).append("\n");
    }

    for (int i = 0; i < m_layers.length; i++) {
      temp.append("Layer: ").append(i+1).append("\n");
      temp.append(m_layers[i]).append("\n");
    }
