...
public class LinearRegression extends AbstractClassifier implements
  ...
  protected float m_ClassStdDev;
  /** The mean of the class attribute */
  protected float m_ClassMean;
  /** The index of the class attribute */
  protected int m_ClassIndex;
  /** The attributes means */
  protected float[] m_Means;
  /** The attribute standard deviations */
  protected float[] m_StdDevs;
  /**
  ...
  /** The ridge parameter */
  protected float m_Ridge = 1.0e-8f;
  /** Conserve memory? */
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    m_SelectedAttributes = new boolean[data.numAttributes()];
    m_Means = new float[data.numAttributes()];
    m_StdDevs = new float[data.numAttributes()];
    for (int j = 0; j < data.numAttributes(); j++) {
      if (j != m_ClassIndex) {
        m_SelectedAttributes[j] = true; // Turn attributes on for a start
        m_Means[j] = data.meanOrMode(j);
        m_StdDevs[j] = (float) Math.sqrt(data.variance(j));
        if (m_StdDevs[j] == 0) {
    ...
    m_ClassStdDev = (float) Math.sqrt(data.variance(m_TransformedData.classIndex()));
    ...
    if (m_outputAdditionalStats) {
      ...
      float se = calculateSE(m_SelectedAttributes, m_Coefficients);
   ...
   */
  @Override
  public float classifyInstance(Instance instance) throws Exception {
  ...
  public String toString() {
    ...
    try {
      ...
      if (m_outputAdditionalStats) {
        ...
        text.append("\n\nRegression Analysis:\n\n").append(Utils.padRight("Variable", maxAttLength)).
        append("  Coefficient     SE of Coef        t-Stat");
        column = 0;
        for (int i = 0; i < m_TransformedData.numAttributes(); i++) {
          if ((i != m_ClassIndex) && (m_SelectedAttributes[i])) {
            text.append("\n"
  ...
  public float[] coefficients() {
    ...
    float[] coefficients = new float[m_SelectedAttributes.length + 1];
    int counter = 0;
  ...
  public String[] getOptions() {
    ...
    result.add("-S");
    result.add(String.valueOf(getAttributeSelectionMethod().getSelectedTag().getID()));
    ...
    result.add("-R");
    result.add(String.valueOf(getRidge()));
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (ridgeString.length() != 0) {
      setRidge(new Float(ridgeString).floatValue());
    } else {
      setRidge((float) 1.0e-8);
    }
  ...
  protected boolean deselectColinearAttributes(boolean[] selectedAttributes,
    ...
    if (maxAttr >= 0) {
      ...
      if (m_Debug) {
        System.out.println(new StringBuilder("Deselected colinear attribute:").append(maxAttr + 1)
        		.append(" with standardised coefficient: ").append(maxSC));
      }
      return true;
    }
    return false;
  ...
  protected void findBestModel() throws Exception {
    ...
    float fullMSE = calculateSE(m_SelectedAttributes, m_Coefficients);
    float akaike = (numInstances - numAttributes) + 2 * numAttributes;
    if (m_Debug) {
    ...
    case SELECTION_GREEDY:
      ...
      do {
        boolean[] currentSelected=new boolean[m_SelectedAttributes.length];
        System.arraycopy(m_SelectedAttributes, 0,currentSelected , 0, m_SelectedAttributes.length);
        improved = false;
        ...
        for (int i = 0; i < m_SelectedAttributes.length; i++) {
          if (currentSelected[i]) {
            ...
            float[] currentCoeffs = doRegression(currentSelected);
            float currentMSE = calculateSE(currentSelected, currentCoeffs);
            float currentAkaike =
    ...
    case SELECTION_M5:
      ...
      do {
        ...
        if (minAttr >= 0) {
          ...
          float[] currentCoeffs = doRegression(m_SelectedAttributes);
          float currentMSE = calculateSE(m_SelectedAttributes, currentCoeffs);
          float currentAkaike =
            currentMSE / fullMSE * (numInstances - numAttributes) + 2
              * currentNumAttributes;
          if (m_Debug) {
   ...
   */
  protected float calculateSE(boolean[] selectedAttributes,
    float[] coefficients) throws Exception {
    ...
    for (int i = 0; i < m_TransformedData.numInstances(); i++) {
      float prediction =
        regressionPrediction(m_TransformedData.instance(i), selectedAttributes,
          coefficients);
      float error = prediction - m_TransformedData.instance(i).classValue();
      mse += error * error;
    }
    return mse;
  }
   ...
   *           assigned
   */
  protected float regressionPrediction(Instance transformedInstance,
    boolean[] selectedAttributes, float[] coefficients) throws Exception {
   ...
   */
  protected float[] doRegression(boolean[] selectedAttributes)
    throws Exception {
    ...
    if (numAttributes > 0) {
      ...
      for (int i = 0; i < m_TransformedData.numInstances(); i++) {
        ...
        float sqrt_weight = (float) Math.sqrt(inst.weight());
        int index = 0;
        for (int j = 0; j < m_TransformedData.numAttributes(); j++) {
          if (j == m_ClassIndex) {
            ...
            if (selectedAttributes[j]) {
              float value = inst.value(j) - m_Means[j];
    ...
    if (m_useQRDecomposition && m_Ridge > 0) {
      float sqrtRidge = (float) Math.sqrt(m_Ridge);
    ...
    float[] coefficients = new float[numAttributes + 1];
    if (numAttributes > 0) {
