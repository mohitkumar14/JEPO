...
public class RegSMO extends RegOptimizer implements TechnicalInformationHandler {
  ...
  protected final static float m_Del = 1e-10f; // 1000 * Double.MIN_VALUE;
  ...
  protected float m_alpha1;

  /** alpha* value for first candidate **/
  protected float m_alpha1Star;

  /** alpha value for second candidate **/
  protected float m_alpha2;

  /** alpha* value for second candidate **/
  protected float m_alpha2Star;
  ...
  protected void init(Instances data) throws Exception {
    ...
    // init error cache
    m_error = new float[m_nInstances];
   ...
   * @return
   */
  protected boolean findOptimalPointOnLine(int i1, float alpha1,
    float alpha1Star, float C1, int i2, float alpha2, float alpha2Star,
    float C2, float gamma, float eta, float deltaPhi) {
    if (eta <= 0) {
    ...
    while (!finished) {
      ...
      if ((case1 == false) && (alpha1 > 0 || (alpha1Star == 0 && deltaPhi > 0))
        ...
        // compute L, H (wrt. alpha1, alpha2)
        float L = Math.max(0, gamma - C1);
        float H = Math.min(C2, gamma);
        if (L < H) {
          float a2 = alpha2 - deltaPhi / eta;
          a2 = Math.min(a2, H);
          a2 = Math.max(L, a2);
      ...
      // case2 = 1;
        ...
        // compute L, H (wrt. alpha1, alpha2*)
        float L = Math.max(0, -gamma);
        float H = Math.min(C2, -gamma + C1);
        if (L < H) {
          float a2 = alpha2Star + (deltaPhi - 2 * m_epsilon) / eta;
          a2 = Math.min(a2, H);
          a2 = Math.max(L, a2);
          // To prevent precision problems
          if (a2 > C2 - m_Del * C2) {
      ...
      // endif
        ...
        float H = Math.min(C2, -gamma);
        if (L < H) {
          float a2 = alpha2Star + deltaPhi / eta;
          a2 = Math.min(a2, H);
          a2 = Math.max(L, a2);
          // To prevent precision problems
          if (a2 > C2 - m_Del * C2) {
          ...
          float a1 = alpha1Star - (a2 - alpha2Star);
          if (a1 > C1 - m_Del * C1) {
            a1 = C1;
          } else if (a1 <= m_Del * C1) {
            a1 = 0;
   ...
   * @throws Exception
   */
  protected int takeStep(int i1, int i2, float alpha2, float alpha2Star,
    float phi2) throws Exception {
    // if (i1 == i2) return 0
    if (i1 == i2) {
      return 0;
    }
    ...
    float alpha1 = m_alpha[i1];
    float alpha1Star = m_alphaStar[i1];
    float phi1 = m_error[i1];
    ...
    float k11 = m_kernel.eval(i1, i1, m_data.instance(i1));
    float k12 = m_kernel.eval(i1, i2, m_data.instance(i1));
    float k22 = m_kernel.eval(i2, i2, m_data.instance(i2));
    float eta = -2 * k12 + k11 + k22; // note, Smola's psuedocode has signs
    ...
    float gamma = alpha1 - alpha1Star + alpha2 - alpha2Star;
    ...
    float alpha1old = alpha1;
    float alpha1Starold = alpha1Star;
    float alpha2old = alpha2;
    float alpha2Starold = alpha2Star;
    ...
    if (findOptimalPointOnLine(i1, alpha1, alpha1Star, C1, i2, alpha2,
      ...
      float dAlpha1 = alpha1 - alpha1old - (alpha1Star - alpha1Starold);
      float dAlpha2 = alpha2 - alpha2old - (alpha2Star - alpha2Starold);
