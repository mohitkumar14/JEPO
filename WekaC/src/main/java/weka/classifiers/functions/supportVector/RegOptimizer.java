...
public class RegOptimizer implements OptionHandler, Serializable,
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setEpsilonParameter(Float.parseFloat(tmpStr));
    } else {
      setEpsilonParameter(1.0e-3f);
    }
  ...
  public String[] getOptions() {
    ...
    result.add("-L");
    result.add(String.valueOf(getEpsilonParameter()));
  ...
  protected void init(Instances data) throws Exception {
    ...
    // init m_target
    m_target = new float[m_nInstances];
    ...
    m_alpha = new float[m_target.length];
    m_alphaStar = new float[m_target.length];
   ...
   * @throws Exception if something goes wrong
   */
  protected float getScore() throws Exception {
    float res = 0;
    float t = 0, t2 = 0;
    for (int i = 0; i < m_nInstances; i++) {
   ...
   * @throws Exception if something goes wrong
   */
  protected float SVMOutput(int index) throws Exception {
    float result = -m_b;
   ...
   * @throws Exception
   */
  public float SVMOutput(Instance inst) throws Exception {
   ...
   */
  public float getEpsilonParameter() {
    return m_epsilon;
  }
   ...
   */
  public void setEpsilonParameter(float v) {
    m_epsilon = v;
  }
  ...
  public String toString() {
    ...
    if (m_weights != null) {
      ...
      for (int i = 0; i < m_data.numAttributes(); i++) {
        if (i != m_classIndex) {
        	if(m_weights[i] >= 0)
        		text.append((" + ")).append(Utils.doubleToString((float) Math.abs(m_weights[i]), 12, 4)).append(" * ");
        	else
        		text.append((" - ")).append(Utils.doubleToString((float) Math.abs(m_weights[i]), 12, 4)).append(" * ");
          ...
          text.append(m_data.attribute(i).name()).append("\n");
        }
      }
    } else {
      ...
      for (int i = 0; i < m_nInstances; i++) {
        if (m_alpha[i] > 0) {
          text.append("+").append(m_alpha[i]).append(" * k[").append(i).append("]\n");
        }
        if (m_alphaStar[i] > 0) {
          text.append("-").append(m_alphaStar[i]).append(" * k[").append(i).append( "]\n");
        }
      }
    }
    
    if(m_b <= 0)
    	text.append((" + ")).append(Utils.doubleToString(Math.abs(m_b), 12, 4)).append("\n\n");
    else
        text.append((" - ")).append(Utils.doubleToString(Math.abs(m_b), 12, 4)).append("\n\n");

    text.append("\n\nNumber of kernel evaluations: ").append(m_nEvals);
    if (m_nCacheHits >= 0 && m_nEvals > 0) {
      float hitRatio = 1 - m_nEvals * 1.0f / (m_nCacheHits + m_nEvals);
