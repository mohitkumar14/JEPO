...
public abstract class CachedKernel extends Kernel {
   ...
   * @throws Exception if something goes wrong
   */
  protected abstract float evaluate(int id1, int id2, Instance inst1)
    throws Exception;
      ...
      if (m_cacheSize == 0) {
        if (m_kernelMatrix == null) {
          ...
          for (int i = 0; i < m_data.numInstances(); i++) {
            m_kernelMatrix[i] = new float[i + 1];
            for (int j = 0; j <= i; j++) {
              m_kernelEvals++;
   ...
   */
  protected final float dotProd(Instance inst1, Instance inst2)
    throws Exception {
  ...
  public void setCacheSize(int value) {
    if (value >= -1) {
      ...
      System.out.println(new StringBuilder("Cache size cannot be smaller than -1 (provided: ")
    		  .append(value).append(")!"));
    }
  }
  ...
  protected void initVars(Instances data) {
    ...
    if (getCacheSize() > 0) {
      // Use LRU cache
      m_storage = new float[m_cacheSize * m_cacheSlots];
