...
public class RBFKernel extends CachedKernel {
   ...
   */
  public RBFKernel(Instances data, int cacheSize, float gamma) throws Exception {
  ...
  protected float evaluate(int id1, int id2, Instance inst1) throws Exception {
    ...
    if (id1 == id2) {
      ...
      if (id1 == -1) {
        return (float) Math.exp(-m_gamma * (dotProd(inst1, inst1) - 2 * dotProd(inst1, m_data.instance(id2))
                + m_kernelPrecalc[id2]));
      } else {
        return (float) Math.exp(-m_gamma * (m_kernelPrecalc[id1] - 2 * dotProd(inst1, m_data.instance(id2))
                + m_kernelPrecalc[id2]));
      }
    }
  }
  ...
  public String toString() {
    return new StringBuilder("RBF Kernel: K(x,y) = exp(-").append(m_gamma).append("*(x-y)^2)").toString();
  }
