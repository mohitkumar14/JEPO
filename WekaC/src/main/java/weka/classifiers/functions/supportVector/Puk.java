...
public class Puk extends CachedKernel implements TechnicalInformationHandler {
  ...
  protected float m_kernelPrecalc[];

  /** Omega for the Puk kernel. */
  protected float m_omega = 1.0f;

  /** Sigma for the Puk kernel. */
  protected float m_sigma = 1.0f;
   ...
   */
  public Puk(Instances data, int cacheSize, float omega, float sigma) throws Exception {
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setOmega(Float.parseFloat(tmpStr));
    } else {
      setOmega(1.0f);
    }
    ...
    if (tmpStr.length() != 0) {
      setSigma(Float.parseFloat(tmpStr));
    } else {
      setSigma(1.0f);
    }
  ...
  public String[] getOptions() {
    ...
    result.add("-O");
    result.add(String.valueOf(getOmega()));

    result.add("-S");
    result.add(String.valueOf(getSigma()));
   ...
   */
  @Override
  protected float evaluate(int id1, int id2, Instance inst1) throws Exception {
    ...
    if (id1 == id2) {
      ...
      float squaredDifference = -2.0f * dotProd(inst1, inst2) + precalc1
        + m_kernelPrecalc[id2];
      float intermediate = (float) (m_factor * Math.sqrt(squaredDifference));
      float result = (float) (1.0f / Math.pow(1.0f + intermediate * intermediate,
        getOmega()));
      return result;
    }
  }
   ...
   */
  protected float computeFactor(float omega, float sigma) {
    float root = (float) Math.sqrt(Math.pow(2.0, 1.0 / omega) - 1);
    return 2.0f * root / sigma;
  }
  ...
  protected void initVars(Instances data) {
    ...
    m_factor = computeFactor(m_omega, m_sigma);
    m_kernelPrecalc = new float[data.numInstances()];
  }
