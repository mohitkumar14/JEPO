...
public class KernelEvaluation implements RevisionHandler {
  ...
  public void setUserOptions(String[] options) {
	  System.arraycopy(options, 0, m_Options, 0, options.length);
  }
  ...
  protected static String makeOptionString(Kernel Kernel) {
    ...
    if (Kernel instanceof OptionHandler) {
      text.append("\nOptions specific to ").append(Kernel.getClass().getName().replaceAll(".*\\.", "")).append(":\n\n");
  ...
  public static String evaluate(Kernel Kernel, String[] options)
    ...
    int classIndex = -1;
    Instances train;
    String[] userOptions = new String[options.length];;
    ...
    try {
      ...
      System.arraycopy(options, 0, userOptions, 0, options.length);
  ...
  public String evaluate(Kernel kernel, Instances data) throws Exception {
    ...
    if (Utils.joinOptions(getUserOptions()).trim().length() != 0) {
      m_Result.append("Options: ").append(Utils.joinOptions(getUserOptions())).append("\n\n");
    }
    m_Result.append(kernel.toString() + "\n");

    // evaluate dataset
    m_Evaluations = new float[data.numInstances()][data.numInstances()];
    ...
    m_Result.append("\n=== Evaluation ===\n\n");
    if (kernel instanceof CachedKernel) {
      m_Result.append("Cache size   : ").append(((CachedKernel) kernel).getCacheSize()).append("\n");
    }
    m_Result.append("# Evaluations: ").append(m_NumEvals).append("\n");
    m_Result.append("# Cache hits : ").append(m_NumCacheHits).append("\n");
    m_Result.append("Elapsed time : ").append((((double) (endTime - startTime)) / 1000)).append("s\n");
