...
public class PolyKernel extends CachedKernel {
   ...
   */
  public PolyKernel(Instances data, int cacheSize, float exponent,
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setExponent(Float.parseFloat(tmpStr));
    } else {
      setExponent(1.0f);
    }
  ...
  public String[] getOptions() {
    ...
    result.add("-E");
    result.add(String.valueOf(getExponent()));
  ...
  protected float evaluate(int id1, int id2, Instance inst1) throws Exception {
    ...
    if (m_exponent != 1.0) {
      result = (float) Math.pow(result, m_exponent);
    }
    return result;
  }
