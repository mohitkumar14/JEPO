...
public class StringKernel extends Kernel implements TechnicalInformationHandler {
  ...
  protected float[] m_powersOflambda = null;
  ...
  private float[] cachekh;
  private int[] cachekhK;
  private float[] cachekh2;
  ...
  public StringKernel(Instances data, int cacheSize, int subsequenceLength,
    float lambda, boolean debug) throws Exception {
  ...
  public Enumeration<Option> listOptions() {
    ...
    for (i = 0; i < TAGS_PRUNING.length; i++) {
      ...
      param += String.valueOf(tag.getSelectedTag().getID());
      desc += new StringBuilder("\t").append(tag.getSelectedTag().getID()).append(" = "
    		  ).append(tag.getSelectedTag().getReadable()).append("\n").toString();
    }

    result.addElement(new Option(new StringBuilder("\tThe pruning method to use:\n").append(desc
    		).append("\t(default: ").append(PRUNING_NONE).append(")").toString(), "P", 1, "-P <" + param + ">"));
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setLambda(Float.parseFloat(tmpStr));
    } else {
      setLambda(0.5f);
    }
    ...
    if (getMaxSubsequenceLength() < 2 * getSubsequenceLength()) {
      throw new IllegalArgumentException(
        "Lambda Pruning forbids even contiguous substring matches! Use a bigger value for ssl-max (at least 2*ssl).");
    }
  ...
  public String[] getOptions() {
    ...
    result.add("-P");
    result.add(String.valueOf(m_PruningMethod));

    result.add("-C");
    result.add(String.valueOf(getCacheSize()));

    result.add("-IC");
    result.add(String.valueOf(getInternalCacheSize()));

    result.add("-L");
    result.add(String.valueOf(getLambda()));

    result.add("-ssl");
    result.add(String.valueOf(getSubsequenceLength()));

    result.add("-ssl-max");
    result.add(String.valueOf(getMaxSubsequenceLength()));
  ...
  public void setCacheSize(int value) {
    if (value >= 0) {
      ...
      System.out.println(new StringBuilder("Cache size cannot be smaller than 0 (provided: ")
    		  .append(value).append(")!"));
    }
  }
  ...
  public void setInternalCacheSize(int value) {
    if (value >= 0) {
      ...
      System.out.println(new StringBuilder("Cache size cannot be smaller than 0 (provided: ").append(value)
      .append(")!"));
    }
  }
   ...
   */
  @Override
  public float eval(int id1, int id2, Instance inst1) throws Exception {
   ...
   */
  public float normalizedKernel(char[] s, char[] t) {
    float k1 = unnormalizedKernel(s, s);
    float k2 = unnormalizedKernel(t, t);
    float normTerm = (float) Math.sqrt(k1 * k2);
   ...
   * @return a double indicating their distance, or similarity
   */
  public float unnormalizedKernel(char[] s, char[] t) {
    ...
    if (m_PruningMethod == PRUNING_NONE) {
      ...
      cachekh = new float[maxCache];
      cachekh2 = new float[maxCache];
   ...
   * @return
   */
  protected float getReturnValue(int n) {
    if (n == 0) {
      return 1;
    } else {
      return 0;
   ...
   */
  protected float kernel(int n, char[] s, int endIndexS, char[] t,
    int endIndexT) {
   ...
   */
  protected float kernelHelper(int n, char[] s, int endIndexS, char[] t,
    int endIndexT) {
   ...
   * @return a partial result for K'
   */
  protected float kernelHelper2(int n, char[] s, int endIndexS, char[] t,
    int endIndexT) {
    ...
    if (x == t[endIndexT]) {
      ...
      float ret = m_lambda * kernelHelper2(n, s, endIndexS, t, endIndexT - 1);
      if (m_useRecursionCache) {
        cachekh2K[adr % maxCache] = adr + 1;
        cachekh2[adr % maxCache] = ret;
      }
   ...
   *         according to and depending on the initial value for n.
   */
  protected float kernelLP(int n, char[] s, int endIndexS, char[] t,
   ...
   * @return a partial result for K
   */
  protected float kernelHelperLP(int n, char[] s, int endIndexS, char[] t,
    int endIndexT, int remainingMatchLength) {
   ...
   * @return a partial result for K'
   */
  protected float kernelHelper2LP(int n, char[] s, int endIndexS, char[] t,
    int endIndexT, int remainingMatchLength) {
    ...
    for (i = endIndexT; i >= minIndex; i--) {
      if (x == t[i]) {
        ...
        float ret = getPowerOfLambda(skipLength)
   ...
   * @return the powers
   */
  private float[] calculatePowersOfLambda() {
    float[] powers = new float[MAX_POWER_OF_LAMBDA + 1];
    powers[0] = 1.0f;
    float val = 1.0f;
   ...
   */
  private float getPowerOfLambda(int exponent) {
    if (exponent > MAX_POWER_OF_LAMBDA) {
      return (float) Math.pow(m_lambda, exponent);
    }
  ...
  protected void initVars(Instances data) {
    ...
    m_storage = new float[m_cacheSize];
