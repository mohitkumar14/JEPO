...
public class NeuralNode
  ...
  private float[] m_bestWeights;
  
  /** The change in the weights. */
  private float[] m_changeInWeights;
  ...
  public NeuralNode(String id, Random r, NeuralMethod m) {
    super(id);
    m_weights = new float[1];
    m_bestWeights = new float[1];
    m_changeInWeights = new float[1];
    
    m_random = r;
    
    m_weights[0] = m_random.nextFloat() * .1f - .05f;
   ...
   */
  public float weightValue(int n) {
   ...
   */
  public float[] getChangeInWeights() {
   ...
   */
  public void updateWeights(float l, float m) {
    
    if (!m_weightsUpdated && !Float.isNaN(m_unitError)) {
  ...
  protected boolean connectInput(NeuralConnection i, int n) {
    ...
    m_weights[m_numInputs] = m_random.nextFloat() * .1f - .05f;
  ...
  protected void allocateInputs() {
    ...
    float[] temp4 = new float[m_weights.length + 15];
    float[] temp5 = new float[m_changeInWeights.length + 15];
    float[] temp6 = new float[m_bestWeights.length + 15];
  ...
  public void removeAllInputs() {
    ...
    float temp1 = m_weights[0];
    float temp2 = m_changeInWeights[0];
