...
public class SigmoidUnit
   ...
   * @return The value.
   */
  public float outputValue(NeuralNode node) {
   ...
   * @return The error.
   */
  public float errorValue(NeuralNode node) {
	...
	* outputs[noa].weightValue(oNums[noa]);
    ...
    float value = node.outputValue(false);
    error *= value * (1 - value);
   ...
   */
  public void updateWeights(NeuralNode node, float learn, float momentum) {

    NeuralConnection[] inputs = node.getInputs();
    float[] cWeights = node.getChangeInWeights();
    float[] weights = node.getWeights();
    float learnTimesError = 0;
    learnTimesError = learn * node.errorValue(false);
    float c = learnTimesError + momentum * cWeights[0];
    weights[0] += c;
    cWeights[0] = c;
