...
public class GaussianProcesses extends RandomizableClassifier implements
  ...
  protected float m_delta = 1;

  /** The squared noise value. */
  protected float m_deltaSquared = 1;

  /**
   * The parameters of the linear transformation realized by the filter on the
   * class attribute
   */
  protected float m_Alin;
  protected float m_Blin;
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    m_avg_target = (float) (sum / insts.sumOfWeights());

    // Store squared noise level
    m_deltaSquared = m_delta * m_delta;

    // Store square roots of instance m_weights
    m_weights = new float[insts.numInstances()];
    for (int i  = 0; i < insts.numInstances(); i++) {
      m_weights[i] = (float) Math.sqrt(insts.instance(i).weight());
    }
  ...
  public float classifyInstance(Instance inst) throws Exception {
    ...
    float result = (float) ((k.dot(m_t) + m_avg_target - m_Blin) / m_Alin);
   ...
   */
  protected float computeStdDev(Instance inst, Vector k) throws Exception {

    float kappa = (float) (m_actualKernel.eval(-1, -1, inst) + m_deltaSquared);

    float s = (float) m_L.mult(k, new DenseVector(k.size())).dot(k);

    float sigma = m_delta;
    if (kappa > s) {
      sigma = (float) Math.sqrt(kappa - s);
    }
   ...
   */
  @Override
  public float[][] predictIntervals(Instance inst, float confidenceLevel)
    throws Exception {
    ...
    float estimate = (float) (k.dot(m_t) + m_avg_target);

    float sigma = computeStdDev(inst, k);

    confidenceLevel = 1.0f - ((1.0f - confidenceLevel) / 2.0f);

    float z =  Statistics.normalInverse(confidenceLevel);

    float[][] interval = new float[1][2];
   ...
   */
  public float getStandardDeviation(Instance inst) throws Exception {
   ...
   */
  @Override
  public float logDensity(Instance inst, float value) throws Exception {
    ...
    float z = (float) (-Math.log(sigma * Math.sqrt(2 * Math.PI)) - value * value
      / (2.0 * sigma * sigma));

    return (float) (z + Math.log(m_Alin));
  }
  ...
  public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option(
      "\tWhether to 0=normalize/1=standardize/2=neither. (default 0=normalize)", "N", 1, "-N"));
    ...
    result.addElement(new Option("", "", 0, new StringBuilder("\nOptions specific to kernel ")
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setNoise(Float.parseFloat(tmpStr));
    } else {
      setNoise(1);
    }
  ...
  public String[] getOptions() {
    ...
    result.addElement("-L");
    result.addElement(String.valueOf(getNoise()));

    result.addElement("-N");
    result.addElement(String.valueOf(m_filterType));

    result.addElement("-K");
    result.addElement(new StringBuilder( m_kernel.getClass().getName()).append(" ")
    		.append(Utils.joinOptions(m_kernel.getOptions())).toString());
  ...
  public String toString() {
    ...
    try {
      ...
      text.append(new StringBuilder("Kernel used:\n  ").append(m_kernel.toString()).append("\n\n").toString());

      text.append(new StringBuilder("All values shown based on: ")
    		  .append(TAGS_FILTER[m_filterType].getReadable()).append("\n\n").toString());

      text.append(new StringBuilder("Average Target Value : ").append(m_avg_target).append("\n").toString());

      text.append("Inverted Covariance Matrix:\n");
      float min = (float) m_L.get(0, 0);
      float max = (float) m_L.get(0, 0);
      for (int i = 0; i < m_NumTrain; i++) {
        for (int j = 0; j <= i; j++) {
          if (m_L.get(i, j) < min) {
            min = (float) m_L.get(i, j);
          } else if (m_L.get(i, j) > max) {
            max = (float) m_L.get(i, j);
          }
        }
      }
      text.append(new StringBuilder("    Lowest Value = ").append(min).append("\n").toString());
      text.append(new StringBuilder("    Highest Value = ").append(max).append("\n").toString());
