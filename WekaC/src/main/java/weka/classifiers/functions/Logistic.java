...
public class Logistic extends AbstractClassifier implements OptionHandler,
  ...
  public String globalInfo() {
    return new StringBuilder("Class for building and using a multinomial logistic "
    ).append("regression model with a ridge estimator.\n\n"
    ).append("There are some modifications, however, compared to the paper of "
    ).append("leCessie and van Houwelingen(1992): \n\n"
    ).append("If there are k classes for n instances with m attributes, the "
    ).append("parameter matrix B to be calculated will be an m*(k-1) matrix.\n\n"
    ).append("The probability for class j with the exception of the last class is\n\n"
    ).append("Pj(Xi) = exp(XiBj)/((sum[j=1..(k-1)]exp(Xi*Bj))+1) \n\n"
    ).append("The last class has probability\n\n"
    ).append("1-(sum[j=1..(k-1)]Pj(Xi)) \n\t= 1/((sum[j=1..(k-1)]exp(Xi*Bj))+1)\n\n"
    ).append("The (negative) multinomial log-likelihood is thus: \n\n"
    ).append("L = -sum[i=1..n]{\n\tsum[j=1..(k-1)](Yij * ln(Pj(Xi)))"
    ).append("\n\t+(1 - (sum[j=1..(k-1)]Yij)) \n\t* ln(1 - sum[j=1..(k-1)]Pj(Xi))"
    ).append("\n\t} + ridge * (B^2)\n\n"
    ).append("In order to find the matrix B for which L is minimised, a "
    ).append("Quasi-Newton Method is used to search for the optimized values of "
    ).append("the m*(k-1) variables.  Note that before we use the optimization "
    ).append("procedure, we 'squeeze' the matrix B into a m*(k-1) vector.  For "
    ).append("details of the optimization procedure, please check "
    ).append("weka.core.Optimization class.\n\n"
    ).append("Although original Logistic Regression does not deal with instance "
    ).append("weights, we modify the algorithm a little bit to handle the "
    ).append("instance weights.\n\n"
    ).append("For more information see:\n\n"
    ).append(getTechnicalInformation().toString()
    ).append("\n\n"
    ).append("Note: Missing values are replaced using a ReplaceMissingValuesFilter, and "
    ).append("nominal attributes are transformed into numeric attributes using a "
    ).append("NominalToBinaryFilter.").toString();
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (ridgeString.length() != 0) {
      m_Ridge = Float.parseFloat(ridgeString);
    } else {
      m_Ridge = 1.0e-8f;
    }
  ...
  public String[] getOptions() {
    ...
    options.add("-R");
    options.add(String.valueOf( m_Ridge));
    options.add("-M");
    options.add(String.valueOf( m_MaxIts));
   ...
   */
  public void setRidge(float ridge) {
    m_Ridge = ridge;
  }
  ...
  private class OptEng extends Optimization {
    ...
    @Override
    protected float objectiveFunction(float[] x) {
      return m_oO.objectiveFunction(x);
    }

    @Override
    protected float[] evaluateGradient(float[] x) {
  ...
  private class OptObject {
     ...
     * 
     * @param weights2 the weights to be set
     */
    ...
    protected float[] evaluateGradient(float[] x) {
      ...
      for (int i = 0; i < cls.length; i++) { // ith instance
        float[] num = new float[m_NumClasses - 1]; // numerator of
                                                     // [-log(1+sum(exp))]'
        int index;
        ...
        float max = num[Utils.maxIndex(num)];
        float denom = (float) Math.exp(-max); // Denominator of [-log(1+sum(exp))]'
        for (int offset = 0; offset < m_NumClasses - 1; offset++) {
          num[offset] = (float) Math.exp(num[offset] - max);
          denom += num[offset];
        }
        ...
        for (int offset = 0; offset < m_NumClasses - 1; offset++) { // Which
                                                                    // part of x
          index = offset * dim;
          firstTerm = (float) (weights[i] * num[offset]);
          for (int q = 0; q < dim; q++) {
  ...
  public void buildClassifier(Instances train) throws Exception {
    ...
    m_Data = new float[nC][nR + 1]; // Data values
    int[] Y = new int[nC]; // Class labels
    float[] xMean = new float[nR + 1]; // Attribute means
    float[] xSD = new float[nR + 1]; // Attribute stddev's
    float[] sY = new float[nK + 1]; // Number of classes
    float[] weights = new float[nC]; // Weights of instances
    float totWeights = 0; // Total weights of the instances
    m_Par = new float[nR + 1][nK]; // Optimized parameter values
    ...
    for (int j = 1; j <= nR; j++) {
      ...
      if (totWeights > 1) {
        xSD[j] = (float) Math.sqrt(Math.abs(xSD[j] - totWeights * xMean[j] * xMean[j])
          / (totWeights - 1));
      } else {
        xSD[j] = 0;
      }
    ...
    if (m_Debug) {
      ...
      System.out.println("\n Variable     Avg       SD    ");
      for (int j = 1; j <= nR; j++) {
        System.out.println(new StringBuilder(Utils.doubleToString(j, 8, 4))
        .append(Utils.doubleToString(xMean[j], 10, 4))
        .append(Utils.doubleToString(xSD[j], 10, 4)));
      }
    }
    ...
    float x[] = new float[(nR + 1) * nK];
    float[][] b = new float[2][x.length]; // Boundary constraints, N/A here

    // Initialize
    for (int p = 0; p < nK; p++) {
      int offset = p * (nR + 1);
      x[offset] = (float) (Math.log(sY[p] + 1.0) - Math.log(sY[nK] + 1.0)); // Null model
      b[0][offset] = Float.NaN;
      b[1][offset] = Float.NaN;
      for (int q = 1; q <= nR; q++) {
        x[offset + q] = 0.0f;
        b[0][offset + q] = Float.NaN;
        b[1][offset + q] = Float.NaN;
      }
    }
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] instDat = new float[m_NumPredictors + 1];
    int j = 1;
    instDat[0] = 1;
    ...
    float[] distribution = evaluateProbability(instDat);
    return distribution;
  }
   ...
   */
  private float[] evaluateProbability(float[] data) {
    float[] prob = new float[m_NumClasses], v = new float[m_NumClasses];
    ...
    for (int m = 0; m < m_NumClasses; m++) {
      ...
      prob[m] = (float) (1 / (sum + Math.exp(-v[m])));
    }
  ...
  public String toString() {
    ...
    for (int j = 1; j <= m_NumPredictors; j++) {
      for (int k = 0; k < m_NumClasses - 1; k++) {
        ...
        float ORc = (float) Math.exp(m_Par[j][k]);
        String t = " "
    ...
    temp.append("\nCoefficients...\n");
    temp.append(Utils.padLeft(" ", attLength)).append(Utils.padLeft("Class", colWidth)).append("\n");
