...
public class SimpleLinearRegression extends AbstractClassifier implements
  ...
  private float m_classMeanForMissing;
  ...
  private float m_seSlope = Float.NaN;

  /** standard error of the intercept */
  private float m_seIntercept = Float.NaN;

  /** t-statistic of the slope */
  private float m_tstatSlope = Float.NaN;

  /** t-statistic of the intercept */
  private float m_tstatIntercept = Float.NaN;

  /** R^2 value for the regression */
  private float m_rsquared = Float.NaN;

  /** Adjusted R^2 value for the regression */
  private float m_rsquaredAdj = Float.NaN;

  /** F-statistic for the regression */
  private float m_fstat = Float.NaN;
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    float[] sum = new float[insts.numAttributes()];
    float[] count = new float[insts.numAttributes()];
    float[] classSumForMissing = new float[insts.numAttributes()];
    float[] classSumSquaredForMissing = new float[insts.numAttributes()];
    float classCount = 0;
    float classSum = 0;
    ...
    // Compute means
    float[] mean = new float[insts.numAttributes()];
    float[] classMeanForMissing = new float[insts.numAttributes()];
    float[] classMeanForKnown = new float[insts.numAttributes()];
    for (int i = 0; i < insts.numAttributes(); i++) {
    ...
    float[] slopes = new float[insts.numAttributes()];
    float[] sumWeightedDiffsSquared = new float[insts.numAttributes()];
    float[] sumWeightedClassDiffsSquared = new float[insts.numAttributes()];
      ...
      if (!inst.classIsMissing()) {
        ...
        for (int i = 0; i < insts.numAttributes(); i++) {
          if (!inst.isMissing(i) && (i != insts.classIndex())) {
            float yDiff = inst.classValue() - classMeanForKnown[i];
            float weightedYDiff = inst.weight() * yDiff;
            float diff = inst.value(i) - mean[i];
            float weightedDiff = inst.weight() * diff;
    ...
    m_attribute = null;
    int chosen = -1;
    float chosenSlope = Float.NaN;
    float chosenIntercept = Float.NaN;
    float chosenMeanForMissing = Float.NaN;
    for (int i = 0; i < insts.numAttributes(); i++) {

      // Do we have missing values for this attribute?
      float sseForMissing = classSumSquaredForMissing[i] -
      ...
      float numerator = slopes[i];
      slopes[i] /= sumWeightedDiffsSquared[i];
      float intercept = classMeanForKnown[i] - slopes[i] * mean[i];

      // Compute sum of squared errors
      float sse = sumWeightedClassDiffsSquared[i] - slopes[i] * numerator;
    ...
    if (chosen == -1) {
      ...
      if (m_outputAdditionalStats) {
        ...
        float[] stdErrors = RegressionAnalysis.calculateStdErrorOfCoef(insts,
        ...
        coef[0] = m_slope;
        coef[1] = m_intercept;
        float[] tStats = RegressionAnalysis
          .calculateTStats(coef, stdErrors, 2);
        m_tstatSlope = tStats[0];
        m_tstatIntercept = tStats[1];
        float ssr = RegressionAnalysis.calculateSSR(insts, m_attribute,
          m_slope, m_intercept);
  ...
  public String toString() {
    ...
    if (m_attribute == null) {
      ...
      text.append("Linear regression on ").append(m_attribute.name()).append("\n\n");
      text
        .append(Utils.doubleToString(m_slope, 2)).append(" * ").append(m_attribute.name());
      if (m_intercept > 0) {
      ...
      if (m_outputAdditionalStats) {
        ...
        text.append("\n\nRegression Analysis:\n\n").append(Utils.padRight("Variable", attNameLength)
        ...
        text.append("\nAdjusted R^2 = ").append(Utils.doubleToString(m_rsquaredAdj, 5));
