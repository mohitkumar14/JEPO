...
public class VotedPerceptron 
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (exponentsString.length() != 0) {
      m_Exponent = (new Float(exponentsString)).floatValue();
    } else {
      m_Exponent = 1.0f;
    }
  ...
  public String[] getOptions() {
    ...
    options.add("-I"); options.add(String.valueOf(m_NumIterations));
    options.add("-E"); options.add(String.valueOf(m_Exponent));
    options.add("-S"); options.add(String.valueOf(m_Seed));
    options.add("-M"); options.add(String.valueOf(m_MaxK));
  ...
  public float[] distributionForInstance(Instance inst) throws Exception {
    ...
    result[1] = (float) (1 / (1 + Math.exp(-output)));
    result[0] = 1 - result[1];
   ...
   */
  public void setExponent(float v) {
   ...
   */
  private float innerProduct(Instance i1, Instance i2) throws Exception {
    ...
    if (m_Exponent != 1) {
      return (float) Math.pow(result, m_Exponent);
    } else {
      return result;
    }
  }
