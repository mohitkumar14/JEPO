...
public class CheckSource
  ...
  public String[] getOptions() {
    ...
    if (getClassifier() != null) {
      result.add("-W");
      result.add(new StringBuilder(getClassifier().getClass().getName()).append(" ")
          .append(Utils.joinOptions(((OptionHandler) getClassifier()).getOptions())).toString());
    }
  ...
  public boolean execute() throws Exception {
    ...
    for (i = 0; i < data.numInstances(); i++) {
      ...
      if (different) {
        result = false;
        if (numeric)
          System.out.println(new StringBuilder(i+1).append(". instance (Classifier/Source code): ")
              .append(predClassifier).append(" != ").append(predSource));
        else
          System.out.println(new StringBuilder(i+1).append(". instance (Classifier/Source code): ")
        		  .append( data.classAttribute().value((int) predClassifier))
        		  .append(" != ").append(data.classAttribute().value((int) predSource)));
      }
    }
  ...
  public static void main(String[] args) throws Exception{
    ...
    if (Utils.getFlag('h', args)) {
      ...
      while (enm.hasMoreElements()) {
        ...
        text.append(option.synopsis()).append("\n");
        text.append(option.description()).append("\n");
      }
