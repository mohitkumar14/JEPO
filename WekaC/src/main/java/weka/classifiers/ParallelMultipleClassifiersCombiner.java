...
public abstract class ParallelMultipleClassifiersCombiner extends
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_numExecutionSlots > 1) {
      if (m_Debug) {
        System.out.println(new StringBuilder("Starting executor pool with ").append(m_numExecutionSlots)
            .append(" slots..."));
      }
      startExecutorPool();
    }
      ...
      if (m_numExecutionSlots > 1) {
        ...
        Runnable newTask = new Runnable() {
          public void run() {
            try {
              if (m_Debug) {
                System.out.println(new StringBuilder("Training classifier (").append(iteration +1).append(")"));
              }
              currentClassifier.buildClassifier(data);
              if (m_Debug) {
                System.out.println(new StringBuilder("Finished classifier (").append(iteration +1).append(")"));
              }
