...
public class HTML
  ...
  protected void doPrintHeader() {
    append("<html>\n");
    append("<head>\n");
    append(new StringBuilder("<title>Predictions for dataset ").append(sanitize(m_Header.relationName())).append("</title>\n").toString());
    append("</head>\n");
    append("<body>\n");
    append("<div align=\"center\">\n");
    append(new StringBuilder("<h3>Predictions for dataset ").append(sanitize(m_Header.relationName())).append("</h3>\n").toString());
    append("<table border=\"1\">\n");
    append("<tr>\n");
    if (m_Header.classAttribute().isNominal())
      if (m_OutputDistribution)
	append(new StringBuilder("<td>inst#</td><td>actual</td><td>predicted</td><td>error</td><td colspan=\"").append(m_Header.classAttribute().numValues()).append("\">distribution</td>").toString());
      else
  ...
  protected void doPrintClassification(float[] dist, Instance inst, int index) throws Exception {
    ...
    // index
    append("<tr>");
    append(new StringBuilder("<td>").append(index+1).append("</td>").toString());
    ...
    if (inst.dataset().classAttribute().isNumeric()) {
      ...
      if (inst.classIsMissing())
        ...
        append(new StringBuilder("<td align=\"right\">").append(Utils.doubleToString(inst.classValue(), prec)).append("</td>").toString());
      // predicted
      if (Utils.isMissingValue(predValue))
        append("<td align=\"right\">?</td>");
      else
        append(new StringBuilder("<td align=\"right\">").append(Utils.doubleToString(predValue, prec)).append("</td>").toString());
      // error
      if (Utils.isMissingValue(predValue) || inst.classIsMissing())
        append("<td align=\"right\">?</td>");
      else
        append(new StringBuilder("<td align=\"right\">").append(Utils.doubleToString(predValue - inst.classValue(), prec)).append("</td>").toString());
    } else {
      // actual
      append(new StringBuilder("<td>").append(((int) inst.classValue()+1)).append(":").append(sanitize(inst.toString(inst.classIndex()))).append("</td>").toString());
      // predicted
      if (Utils.isMissingValue(predValue))
        append("<td>?</td>");
      else
        append(new StringBuilder("<td>").append(((int) predValue+1)).append(":").append(sanitize(inst.dataset().classAttribute().value((int)predValue))).append("</td>").toString());
      // error?
      ...
      if (m_OutputDistribution) {
        ...
        if (Utils.isMissingValue(predValue))
          ...
          append(new StringBuilder("<td align=\"right\">").append(Utils.doubleToString(dist[(int)predValue], prec)).append("</td>").toString());
      }
    }
  ...
  protected void doPrintClassification(Classifier classifier, Instance inst, int index) throws Exception {
    ...
    float[] d = classifier.distributionForInstance(inst);
