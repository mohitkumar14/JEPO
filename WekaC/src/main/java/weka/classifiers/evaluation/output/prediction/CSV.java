...
public class CSV extends AbstractOutput {
  ...
  protected void doPrintHeader() {
    if (m_Header.classAttribute().isNominal()) {
      if (m_OutputDistribution) {
        append(new StringBuilder("inst#").append(m_Delimiter).append("actual").append(m_Delimiter).append("predicted"
        		).append(m_Delimiter).append("error").append(m_Delimiter).append("distribution").toString());
        for (int i = 1; i < m_Header.classAttribute().numValues(); i++) {
          append(m_Delimiter);
        }
      } else {
        append(new StringBuilder("inst#").append(m_Delimiter).append("actual").append(m_Delimiter).append("predicted"
        		).append(m_Delimiter).append("error").append(m_Delimiter).append("prediction").toString());
      }
    } else {
      append(new StringBuilder("inst#").append(m_Delimiter).append("actual").append(m_Delimiter).append("predicted"
    		  ).append(m_Delimiter).append("error").toString());
    }
   ...
   */
  @Override
  protected void doPrintClassification(float[] dist, Instance inst, int index)
    throws Exception {
    ...
    if (inst.dataset().classAttribute().isNumeric()) {
      ...
      // actual
      append(new StringBuilder(m_Delimiter).append((int) inst.classValue() + 1).append(":"
    		  ).append(inst.toString(inst.classIndex())).toString());
      // predicted
      if (Utils.isMissingValue(predValue)) {
        append(m_Delimiter + "?");
      } else {
        append(new StringBuilder(m_Delimiter).append((int) predValue + 1).append(":"
        		).append(inst.dataset().classAttribute().value((int) predValue)).toString());
      }
      // error?
      if (!Utils.isMissingValue(predValue) && !inst.classIsMissing()
