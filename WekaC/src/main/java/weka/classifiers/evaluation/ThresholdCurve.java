...
public class ThresholdCurve implements RevisionHandler {
  ...
  public Instances getCurve(ArrayList<Prediction> predictions, int classIndex) {
    ...
    float totPos = 0, totNeg = 0;
    float[] probs = getProbabilities(predictions, classIndex);
    ...
    float threshold = 0;
    float cumulativePos = 0;
    float cumulativeNeg = 0;
    ...
    if (tc.getFalseNegative() != totPos || tc.getTrueNegative() != totNeg) {
      ...
      threshold = (float) (probs[sorted[sorted.length - 1]] + 10e-6);
      insts.add(makeInstance(tc, threshold));
    }
   ...
   */
  public static float getNPointPrecision(Instances tcurve, int n) {
    ...
    float[] recallVals = tcurve.attributeToDoubleArray(recallInd);
    int[] sorted = Utils.sort(recallVals);
    float isize = 1.0f / (n - 1);
    float psum = 0;
    for (int i = 0; i < n; i++) {
      int pos = binarySearch(sorted, recallVals, i * isize);
      float recall = recallVals[sorted[pos]];
      float precis = tcurve.instance(sorted[pos]).value(precisInd);
      /*
      ...
      while ((pos != 0) && (pos < sorted.length - 1)) {
        pos++;
        float recall2 = recallVals[sorted[pos]];
        if (recall2 != recall) {
          float precis2 = tcurve.instance(sorted[pos]).value(precisInd);
          float slope = (precis2 - precis) / (recall2 - recall);
          float offset = precis - recall * slope;
   ...
   *         generated Instances.
   */
  public static float getPRCArea(Instances tcurve) {
    ...
    final float[] pVals = tcurve.attributeToDoubleArray(pInd);
    final float[] rVals = tcurve.attributeToDoubleArray(rInd);

    float area = 0;
    float xlast = rVals[n - 1];

    // start from the first real p/r pair (not the artificial zero point)
    for (int i = n - 2; i >= 0; i--) {
      float recallDelta = rVals[i] - xlast;
   ...
   *         generated Instances.
   */
  public static float getROCArea(Instances tcurve) {
    ...
    final float[] tpVals = tcurve.attributeToDoubleArray(tpInd);
    final float[] fpVals = tcurve.attributeToDoubleArray(fpInd);

    float area = 0.0f, cumNeg = 0.0f;
    final float totalPos = tpVals[0];
    final float totalNeg = fpVals[0];
    for (int i = 0; i < n; i++) {
      float cip, cin;
      if (i < n - 1) {
   ...
   *         target)
   */
  public static int getThresholdInstance(Instances tcurve, float threshold) {
    ...
    float[] tvals = tcurve.attributeToDoubleArray(tcurve.numAttributes() - 1);
   ...
   */
  private static int binarySearch(int[] index, float[] vals, float target) {

    int lo = 0, hi = index.length - 1;
    while (hi - lo > 1) {
      int mid = lo + (hi - lo) / 2;
      float midval = vals[index[mid]];
      if (target > midval) {
        lo = mid;
   ...
   */
  private float[] getProbabilities(ArrayList<Prediction> predictions,
    int classIndex) {

    // sort by predicted probability of the desired class.
    float[] probs = new float[predictions.size()];
    for (int i = 0; i < probs.length; i++) {
      NominalPrediction pred = (NominalPrediction) predictions.get(i);
   ...
   */
  private Instance makeInstance(TwoClassStats tc, float prob) {
    ...
    float ss = (tc.getTruePositive() + tc.getFalsePositive())
      / (tc.getTruePositive() + tc.getFalsePositive() + tc.getTrueNegative() + tc
        .getFalseNegative());
    vals[count++] = ss;
    float expectedByChance = (ss * (tc.getTruePositive() + tc
      .getFalseNegative()));
    ...
    vals[count++] = prob;
    return new DenseInstance(1.0f, vals);
  }
