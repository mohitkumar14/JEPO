...
public class MarginCurve implements RevisionHandler {
  ...
  public Instances getCurve(ArrayList<Prediction> predictions) {
    ...
    float[] margins = getMargins(predictions);
    int[] sorted = Utils.sort(margins);
    int binMargin = 0;
    int totalMargin = 0;
    insts.add(makeInstance(-1, binMargin, totalMargin));
    for (int element : sorted) {
      float current = margins[element];
      float weight = ((NominalPrediction) predictions.get(element)).weight();
      totalMargin += weight;
      binMargin += weight;
      if (true) {
   ...
   */
  private float[] getMargins(ArrayList<Prediction> predictions) {

    // sort by predicted probability of the desired class.
    float[] margins = new float[predictions.size()];
   ...
   */
  private Instance makeInstance(float margin, int current, int cumulative) {
