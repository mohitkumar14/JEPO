...
public class DiscreteEstimatorBayes extends Estimator
   ...
   * Hold the counts
   */
  protected float[] m_Counts;

  /**
   * Hold the sum of counts
   */
  protected float   m_SumOfCounts;
   ...
   * @param fPrior
   */
  public DiscreteEstimatorBayes(int nSymbols, float fPrior) {
    m_fPrior = fPrior;
    m_nSymbols = nSymbols;
    m_Counts = new float[m_nSymbols];

    for (int iSymbol = 0; iSymbol < m_nSymbols; iSymbol++) {
      m_Counts[iSymbol] = m_fPrior;
    } 

    m_SumOfCounts = m_fPrior * (float) m_nSymbols;
  ...
  public float getProbability(float data) {
    ...
    return (float) m_Counts[(int) data] / m_SumOfCounts;
  } 
   ...
   */
  public float getCount(float data) {
    if (m_SumOfCounts == 0) {
      // this can only happen if numSymbols = 0 in constructor
      return 0;
    } 
   ...
   * @return the score
   */
  public float logScore(int nType, int nCardinality) {
	  float fScore = 0.0f;
		...
		fScore += Statistics.lnGamma(m_Counts[iSymbol]);
	      ...
	      fScore -= m_nSymbols * Statistics.lnGamma(1.0f/(m_nSymbols * nCardinality));
	      fScore += Statistics.lnGamma(1.0f/nCardinality);
		} 
		  break;
	    ...
	    case (Scoreable.ENTROPY): {
	      for (int iSymbol = 0; iSymbol < m_nSymbols; iSymbol++) {
		float fP = getProbability(iSymbol);
  ...
  public String toString() {
	...
	result += " " + Utils.doubleToString(m_Counts[i], 2);
      ...
      result += new StringBuilder("  (Total = ").append(Utils.doubleToString(m_SumOfCounts, 2) 
    		  ).append(")\n");
    } else {
      for (int i = 0; i < m_Counts.length; i++) {
	result += " " + m_Counts[i];
      ...
      result += new StringBuilder("  (Total = ").append(m_SumOfCounts).append(")\n");
    } 
  ...
  public static void main(String[] argv) {
	...
	System.out.println(new StringBuilder("Prediction for ").append(current).append(" = " 
			).append(newEst.getProbability(current)));
