...
public class HillClimber extends LocalScoreSearchAlgorithm {
  ...
  class Operation implements Serializable, RevisionHandler {
    ...
    public float m_fDeltaScore = (float) -1E100;
  ...
  class Cache implements RevisionHandler {
    ...
    float[][] m_fDeltaScoreAdd;
    /** change in score due to deleting an arc **/
    float[][] m_fDeltaScoreDel;
    ...
    Cache(int nNrOfNodes) {
      m_fDeltaScoreAdd = new float[nNrOfNodes][nNrOfNodes];
      m_fDeltaScoreDel = new float[nNrOfNodes][nNrOfNodes];
    }
     ...
     */
    public void put(Operation oOperation, float fValue) {
     ...
     * @return cache value
     */
    public float get(Operation oOperation) {
  ...
  void initCache(BayesNet bayesNet, Instances instances) throws Exception {
    ...
    float[] fBaseScores = new float[instances.numAttributes()];
  ...
  void performOperation(BayesNet bayesNet, Instances instances,
    ...
    case Operation.OPERATION_ADD:
      ...
      if (bayesNet.getDebug()) {
        System.out.print(new StringBuilder("Add ").append(oOperation.m_nHead).append(" -> "
        		).append(oOperation.m_nTail));
      }
      break;
    case Operation.OPERATION_DEL:
      applyArcDeletion(bayesNet, oOperation.m_nHead, oOperation.m_nTail,
        instances);
      if (bayesNet.getDebug()) {
        System.out.print(new StringBuilder("Del ").append(oOperation.m_nHead).append(" -> "
        		).append( oOperation.m_nTail));
      }
      break;
    case Operation.OPERATION_REVERSE:
      ...
      applyArcAddition(bayesNet, oOperation.m_nTail, oOperation.m_nHead,
        instances);
      if (bayesNet.getDebug()) {
        System.out.print(new StringBuilder("Rev ").append(oOperation.m_nHead).append(" -> "
        		).append(oOperation.m_nTail));
      }
      break;
    }
  ...
  void updateCache(int iAttributeHead, int nNrOfAtts, ParentSet parentSet) {
    ...
    float fBaseScore = calcNodeScore(iAttributeHead);
    ...
    for (int iAttributeTail = 0; iAttributeTail < nNrOfAtts; iAttributeTail++) {
      if (iAttributeTail != iAttributeHead) {
        if (!parentSet.contains(iAttributeTail)) {
          ...
          m_Cache.put(oOperation,
            (float)calcScoreWithMissingParent(iAttributeHead, iAttributeTail)
              - fBaseScore);
        }
      }
    }
  ...
  public String[] getOptions() {
    ...
    options.add("-P");
    options.add(String.valueOf(m_nMaxNrOfParents));
