...
public class SimulatedAnnealing extends LocalScoreSearchAlgorithm implements
    ...
    for (int iRun = 0; iRun < m_nRuns; iRun++) {
      ...
      while (!bRunSucces) {
        ...
        if (isArc(bayesNet, iHeadNode, iTailNode)) {
          ...
          float fScore = calcNodeScore(iHeadNode);
          ...
          if (addArcMakesSense(bayesNet, instances, iHeadNode, iTailNode)) {
            bRunSucces = true;
            float fScore = calcScoreWithExtraParent(iHeadNode, iTailNode);
            fDeltaScore = fScore - fBaseScores[iHeadNode];
   ...
   */
  public void setTStart(float fTStart) {
    m_fTStart = fTStart;
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (sTStart.length() != 0) {
      setTStart(Float.parseFloat(sTStart));
    }
    ...
    if (sDelta.length() != 0) {
      setDelta(Float.parseFloat(sDelta));
    }
  ...
  public String[] getOptions() {
    ...
    options.add("-A");
    options.add(String.valueOf(getTStart()));

    options.add("-U");
    options.add(String.valueOf(getRuns()));
