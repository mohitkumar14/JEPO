...
public class LocalScoreSearchAlgorithm extends SearchAlgorithm {
   ...
   * @return log score.
   */
  public float logScore(int nType) {
   ...
   * @return log score
   */
  public float calcNodeScore(int nNode) {
   ...
   * @return log score
   */
  private float calcNodeScoreADTree(int nNode) {
    Instances instances = m_BayesNet.m_Instances;
  ...
  private float calcNodeScorePlain(int nNode) {
    Instances instances = m_BayesNet.m_Instances;
    ParentSet oParentSet = m_BayesNet.getParentSet(nNode);
   ...
   * @return log score
   */
  protected float calcScoreOfCounts(int[] nCounts, int nCardinality,
    ...
    for (int iParent = 0; iParent < nCardinality; iParent++) {
      ...
      case (Scoreable.BDeu): {
        ...
        for (int iSymbol = 0; iSymbol < numValues; iSymbol++) {
          if (m_fAlpha + nCounts[iParent * numValues + iSymbol] != 0) {
            fLogScore += Statistics.lnGamma(1.0f / (numValues * nCardinality)
              + nCounts[iParent * numValues + iSymbol]);
        ...
        fLogScore -= numValues
          * Statistics.lnGamma(1.0f / (numValues * nCardinality));
        fLogScore += Statistics.lnGamma(1.0f / nCardinality);
      }
        break;
  ...
  protected float calcScoreOfCounts2(int[][] nCounts, int nCardinality,
    int numValues, Instances instances) {
    ...
    for (int iParent = 0; iParent < nCardinality; iParent++) {
      ...
      case (Scoreable.BDeu): {
        ...
        fLogScore -= numValues
          * Statistics.lnGamma(1.0f / (nCardinality * numValues));
        fLogScore += Statistics.lnGamma(1.0f / nCardinality);
      }
        break;
   ...
   * @return log score
   */
  public float calcScoreWithExtraParent(int nNode, int nCandidateParent) {
    ParentSet oParentSet = m_BayesNet.getParentSet(nNode);
    ...
    // calculate the score
    float logScore = calcNodeScore(nNode);
   ...
   * @return log score
   */
  public float calcScoreWithMissingParent(int nNode, int nCandidateParent) {
    ParentSet oParentSet = m_BayesNet.getParentSet(nNode);
