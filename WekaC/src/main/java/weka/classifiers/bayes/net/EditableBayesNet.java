...
public class EditableBayesNet extends BayesNet {
  ...
  protected ArrayList<float[]> m_fMarginP;
  ...
  public EditableBayesNet() {
    ...
    m_fMarginP = new ArrayList<float[]>(0);
  ...
  public EditableBayesNet(Instances instances) {
    ...
    m_fMarginP = new ArrayList<float[]>(nNodes);
    for (int i = 0; i < nNodes; i++) {
      float[] P = new float[getCardinality(i)];
      m_fMarginP.add(P);
    }
  ...
  public void addNode(String sName, int nCardinality, int nPosX, int nPosY)
    ...
    distributions[nAtts - 1][0] = new DiscreteEstimatorBayes(nCardinality, 0.5f);
    ...
    m_nEvidence.add(-1);
    float[] fMarginP = new float[nCardinality];
    for (int iValue = 0; iValue < nCardinality; iValue++) {
      fMarginP[iValue] = 1.0f / nCardinality;
    }
  ...
  void paste(String sXML, int mode) throws Exception {
    ...
    for (int iNode = 0; iNode < nodelist.getLength(); iNode++) {
      ...
      while (getNode2(sNodeName) >= 0) {
        sNodeName = new StringBuilder("Copy (").append(iAttempt).append(") of ").append(sBaseName).toString();
        iAttempt++;
      }
    ...
    for (int iNode = 0; iNode < sBaseNames.size(); iNode++) {
      ...
      for (int i = 0; i < nCardinality; i++) {
        ...
        for (int iValue = 0; iValue < nValues; iValue++) {
          ...
          d.addValue(iValue, new Float(sWeight).floatValue());
        }
      }
      if (mode == EXECUTE) {
        m_nEvidence.add(nBase + iNode, -1);
        m_fMarginP
          .add(nBase + iNode, new float[getCardinality(nBase + iNode)]);
      }
    }
    if (mode == EXECUTE) {
      m_Distributions = distributions;
   ...
   * @throws Exception if parent or child cannot be found in network
   */
  public void setDistribution(String sName, float[][] P) throws Exception {
    int nTargetNode = getNode(sName);
   ...
   * @throws Exception if parent or child cannot be found in network
   */
  public void setDistribution(int nTargetNode, float[][] P) throws Exception {
    // update undo stack
    if (m_bNeedsUndoAction) {
   ...
   */
  public float[][] getDistribution(String sName) {
  ...
  public void delNodeValue(int nTargetNode, String sValue) throws Exception {
    ...
    for (int iParent = 0; iParent < distributions.length; iParent++) {
      ...
      if (sum > 0) {
        ...
        for (int iValue = 0; iValue < nCard; iValue++) {
          distribution.addValue(iValue, 1.0f / nCard);
        }
      }
      distributions[iParent] = distribution;
    }
   ...
   * @param iNode index of node of interest
   */
  public float[] getMargin(int iNode) {
   ...
   * @param fs marginal distribution
   */
  public void setMargin(int iNode, float[] fs) {
    m_fMarginP.set(iNode, fs);
  }
  ...
  public String toXMLBIF03() {
    ...
    text.append("<NETWORK>\n");
    text.append("<NAME>").append(XMLNormalize(m_Instances.relationName())).append("</NAME>\n");
    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      text.append("<VARIABLE TYPE=\"nature\">\n");
      text.append("<NAME>").append(XMLNormalize(m_Instances.attribute(iAttribute).name())).append("</NAME>\n");
      for (int iValue = 0; iValue < m_Instances.attribute(iAttribute)
        .numValues(); iValue++) {
        text.append("<OUTCOME>").append(XMLNormalize(m_Instances.attribute(iAttribute).value(iValue))).append("</OUTCOME>\n");
      }
      text.append("<PROPERTY>position = (").append(getPositionX(iAttribute)).append(",")
      .append(getPositionY(iAttribute)).append(")</PROPERTY>\n");
      text.append("</VARIABLE>\n");
    }

    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      text.append("<DEFINITION>\n");
      text.append("<FOR>").append(XMLNormalize(m_Instances.attribute(iAttribute).name())).append("</FOR>\n");
      for (int iParent = 0; iParent < m_ParentSets[iAttribute].getNrOfParents(); iParent++) {
        text
          .append("<GIVEN>").append(XMLNormalize(m_Instances.attribute(
              m_ParentSets[iAttribute].getParent(iParent)).name())).append("</GIVEN>\n");
      }
      text.append("<TABLE>\n");
  ...
  public String toXMLBIF03(ArrayList<Integer> nodes) {
    ...
    for (int iNode = 0; iNode < nodes.size(); iNode++) {
      int nNode = nodes.get(iNode);
      text.append("<VARIABLE TYPE=\"nature\">\n");
      text.append("<NAME>").append(XMLNormalize(m_Instances.attribute(nNode).name())).append("</NAME>\n");
      for (int iValue = 0; iValue < m_Instances.attribute(nNode).numValues(); iValue++) {
        text.append("<OUTCOME>").append(XMLNormalize(m_Instances.attribute(nNode).value(iValue))).append("</OUTCOME>\n");
      }
      text.append("<PROPERTY>position = (").append(getPositionX(nNode)).append( ",").append(getPositionY(nNode))
    		  .append(")</PROPERTY>\n");
      text.append("</VARIABLE>\n");
    }
    ...
    for (int iNode = 0; iNode < nodes.size(); iNode++) {
      int nNode = nodes.get(iNode);
      text.append("<DEFINITION>\n");
      text.append("<FOR>").append(XMLNormalize(m_Instances.attribute(nNode).name())).append("</FOR>\n");
      for (int iParent = 0; iParent < m_ParentSets[nNode].getNrOfParents(); iParent++) {
        text.append("<GIVEN>").append(XMLNormalize(m_Instances.attribute(
            m_ParentSets[nNode].getParent(iParent)).name())).append("</GIVEN>\n");
      }
      text.append("<TABLE>\n");
  ...
  class DeleteNodeAction extends UndoAction {
    ...
    public void undo() {
      try {
        ...
        m_fMarginP
          .add(m_nTargetNode, new float[getCardinality(m_nTargetNode)]);
      } catch (Exception e) {
        e.printStackTrace();
      }
    } // undo
    ...
    public void undo() {
      try {
        ...
        for (int iNode = 0; iNode < m_nodes.size(); iNode++) {
          ...
          m_fMarginP.add(nTargetNode, new float[getCardinality(nTargetNode)]);
        }
        for (int deletedArc = 0; deletedArc < m_deleteArcActions.size(); deletedArc++) {
          DeleteArcAction action = m_deleteArcActions.get(deletedArc);
          action.undo();
  ...
  class SetDistributionAction extends UndoAction {
    ...
    SetDistributionAction(int nTargetNode, float[][] P) {
      try {
        m_nTargetNode = nTargetNode;
        SerializedObject so = new SerializedObject(m_Distributions[nTargetNode]);
        m_CPT = (Estimator[]) so.getObject();
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Distribution of node ").append(getNodeName(m_nTargetNode)).append(" changed").toString();
    }
  ...
  class RenameValueAction extends RenameAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Value of node ").append(getNodeName(m_nTargetNode)).append(" changed from "
    	  ).append(m_sNewName).append(" to ").append(m_sOldName).toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Value of node ").append(getNodeName(m_nTargetNode)).append(" changed from ")
    		  .append(m_sOldName).append(" to ").append(m_sNewName).toString();
    }
  ...
  class AddValueAction extends UndoAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Value ").append(m_sValue).append(" removed from node ").append(getNodeName(m_nTargetNode)).toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Value ").append(m_sValue).append(" added to node ").append(getNodeName(m_nTargetNode)).toString();
    }
  ...
  class alignLeftAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from aliging nodes to the left.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Aligning ").append(m_nodes.size()).append(" nodes to the left.").toString();
    }
  ...
  class alignRightAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from aliging nodes to the right.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Aligning ").append(m_nodes.size()).append(" nodes to the right.").toString();
    }
  } // class alignLeftAction
  ...
  class alignTopAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from aliging nodes to the top.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Aligning ").append(m_nodes.size()).append(" nodes to the top.").toString();
    }
  ...
  class centerHorizontalAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from centering horizontally.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Centering ").append(m_nodes.size()).append(" nodes horizontally.").toString();
    }
  ...
  class centerVerticalAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from centering vertically.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Centering ").append(m_nodes.size()).append(" nodes vertically.").toString();
    }
  ...
  class spaceHorizontalAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from spaceing horizontally.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("spaceing ").append(m_nodes.size()).append(" nodes horizontally.").toString();
    }
  ...
  class spaceVerticalAction extends alignAction {
    ...
    @Override
    public String getUndoMsg() {
      return new StringBuilder("Returning ").append(m_nodes.size()).append(" from spaceng vertically.").toString();
    }

    @Override
    public String getRedoMsg() {
      return new StringBuilder("Spaceng ").append(m_nodes.size()).append(" nodes vertically.").toString();
    }
