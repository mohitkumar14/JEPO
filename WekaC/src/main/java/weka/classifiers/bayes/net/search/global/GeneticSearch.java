...
public class GeneticSearch extends GlobalScoreSearchAlgorithm {
  ...
  protected void search(BayesNet bayesNet, Instances instances)
    ...
    float fBestScore = calcScore(bayesNet);
    ...
    for (int iRun = 0; iRun < m_nRuns; iRun++) {
      ...
      for (int i = 0; i < getPopulationSize(); i++) {
        ...
        if (m_bUseTournamentSelection) {
          ...
          float fScore = descendantPopulation[iSelected].getScore();
  ...
  public String[] getOptions() {
    ...
    options.add("-L");
    options.add(String.valueOf(getPopulationSize()));

    options.add("-A");
    options.add(String.valueOf(getDescendantPopulationSize()));
