...
public class K2 extends GlobalScoreSearchAlgorithm implements
    ...
    for (int iOrder = 1; iOrder < instances.numAttributes(); iOrder++) {
      ...
      while (bProgress
        ...
        for (int iOrder2 = 0; iOrder2 < iOrder; iOrder2++) {
          ...
          float fScore = calcScoreWithExtraParent(iAttribute, iAttribute2);
  ...
  public String globalInfo() {
    return new StringBuilder("This Bayes Network learning algorithm uses a hill climbing algorithm restricted by an order on the variables.\n\nFor more information see:\n\n"
    		).append(getTechnicalInformation().toString()
    				).append("\n\nWorks with nominal variables and no missing values only.").toString();
  }
