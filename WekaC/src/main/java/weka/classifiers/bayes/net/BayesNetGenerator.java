...
public class BayesNetGenerator extends EditableBayesNet {
  ...
  public void generateInstances() throws Exception {
    ...
    for (int iInstance = 0; iInstance < m_nNrOfInstances; iInstance++) {
      ...
      float[] instance = new float[nNrOfAtts];
      for (int iAtt2 = 0; iAtt2 < nNrOfAtts; iAtt2++) {
        ...
        float fRandom = random.nextInt(1000) / 1000.0f;
        int iValue = 0;
      ...
      m_Instances.add(new DenseInstance(1.0f, instance));
    }
  } // GenerateInstances
  ...
  public String[] getOptions() {
    ...
    options[current++] = "-N";
    options[current++] = String.valueOf(m_nNrOfNodes);

    options[current++] = "-A";
    options[current++] = String.valueOf( m_nNrOfArcs);

    options[current++] = "-M";
    options[current++] = String.valueOf( m_nNrOfInstances);

    options[current++] = "-C";
    options[current++] = String.valueOf( m_nCardinality);

    options[current++] = "-S";
    options[current++] = String.valueOf(m_nSeed);

    if (m_sBIFFile.length() != 0) {
      options[current++] = "-F";
      options[current++] = String.valueOf(m_sBIFFile);
    }
  ...
  protected static void printOptions(OptionHandler o) {
    ...
    System.out.println(new StringBuilder("Options for ").append(o.getClass().getName()).append(":\n"));
