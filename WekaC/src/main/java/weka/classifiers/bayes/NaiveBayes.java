...
public class NaiveBayes extends AbstractClassifier implements OptionHandler,
  ...
  public void buildClassifier(Instances instances) throws Exception {
    ...
    while (enu.hasMoreElements()) {
      ...
      float numPrecision = DEFAULT_NUM_PRECISION;
      if (attribute.type() == Attribute.NUMERIC) {
        m_Instances.sort(attribute);
        if ((m_Instances.numInstances() > 0)
          && !m_Instances.instance(0).isMissing(attribute)) {
          float lastVal = m_Instances.instance(0).value(attribute);
          float currentVal, deltaSum = 0;
          int distinct = 0;
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] probs = new float[m_NumClasses];
    ...
    while (enumAtts.hasMoreElements()) {
      ...
      if (!instance.isMissing(attribute)) {
        float temp, max = 0;
        for (int j = 0; j < m_NumClasses; j++) {
          temp =  (float)Math.max(1e-30, Math.pow(m_Distributions[attIndex][j]
    ...
    if (m_Instances == null) {
      ...
      for (Estimator[] m_Distribution : m_Distributions) {
        for (int j = 0; j < m_Instances.numClasses(); j++) {
          if (m_Distribution[0] instanceof NormalEstimator) {
            ...
            float mean = (float) (Math.log(Math.abs(n.getMean())) / Math.log(10.0));
            float precision = (float) (Math.log(Math.abs(n.getPrecision()))
              / Math.log(10.0));
            float width = (mean > precision) ? mean : precision;
            if (width < 0) {
              width = 1;
            }
            ...
            String temps = new StringBuilder("K").append(numK ).append(": mean (weight)").toString();
            if (maxAttWidth < temps.length()) {
              maxAttWidth = temps.length();
            }
            // check means + weights against maxWidth
            if (ke.getNumKernels() > 0) {
              float[] means = ke.getMeans();
              float[] weights = ke.getWeights();
              for (int k = 0; k < ke.getNumKernels(); k++) {
                String m = Utils.doubleToString(means[k], maxWidth, 4).trim();
                m += new StringBuilder(" (").append(Utils.doubleToString(weights[k], maxWidth, 1).trim())
                		.append(")");
            ...
            int sum = (String.valueOf(d.getSumOfCounts())).length();
            if (sum > maxWidth) {
              maxWidth = sum;
            }
          }
      ...
      for (int i = 0; i < m_Instances.numClasses(); i++) {
        String priorP = Utils.doubleToString(
          ...
          maxWidth, 2).trim();
        priorP = new StringBuilder("(").append(priorP).append(")").toString();
        ...
        if (m_Distributions[counter][0] instanceof NormalEstimator) {
          ...
          for (int j = 0; j < a.numValues(); j++) {
            ...
            for (int k = 0; k < m_Instances.numClasses(); k++) {
              ...
              String count = String.valueOf(d.getCount(j));
          ...
          for (int k = 0; k < m_Instances.numClasses(); k++) {
            DiscreteEstimator d = (DiscreteEstimator) m_Distributions[counter][k];
            String count = String.valueOf(d.getSumOfCounts());
            temp.append(pad(count, " ", maxWidth + 1 - count.length(), true));
          }
          temp.append("\n\n");
          ...
          for (int k = 0; k < m_Instances.numClasses(); k++) {
            ...
            String nk = String.valueOf(ke.getNumKernels());
          ...
          for (int j = 0; j < maxK; j++) {
            // means first
            String meanL = new StringBuilder("  K").append(j + 1).append(": mean (weight)").toString();
            temp
            ...
            for (int k = 0; k < m_Instances.numClasses(); k++) {
              ...
              if (ke.getNumKernels() == 0) {
                ...
                m += new StringBuilder(" (")
                		.append(Utils.doubleToString(weights[j], maxWidth, 1).trim()).append(")");
              }
    ...
    if (m_Instances == null) {
      ...
      try {
        for (int i = 0; i < m_Distributions[0].length; i++) {
          text.append("\n\nClass ").append( m_Instances.classAttribute().value(i)).
          append(": Prior probability = ")
          .append(Utils.doubleToString(m_ClassDistribution.getProbability(i), 4, 2)).append("\n\n");
          Enumeration<Attribute> enumAtts = m_Instances.enumerateAttributes();
          int attIndex = 0;
          while (enumAtts.hasMoreElements()) {
            Attribute attribute = enumAtts.nextElement();
            if (attribute.weight() > 0) {
              text.append(attribute.name()).append(":  ").append(m_Distributions[attIndex][i]);
            }
            attIndex++;
          }
        }
