...
public class NaiveBayesMultinomial extends AbstractClassifier
   ...
   */
  protected float[][] m_probOfWordGivenClass;
  ...
  public String globalInfo() {
    return 
    		new StringBuilder("Class for building and using a multinomial Naive Bayes classifier. ")
  ...
  protected void initializeClassifier(Instances instances) throws Exception {
    ...
    m_probOfWordGivenClass = new float[m_numClasses][];

    // Initialize the matrix of word counts
    for (int c = 0; c < m_numClasses; c++) {
      m_probOfWordGivenClass[c] = new float[m_numAttributes];
      for (int att = 0; att < m_numAttributes; att++) {
        m_probOfWordGivenClass[c][att] = 1.0f;
      }
    }

    // Initialize class counts
    m_probOfClass = new float[m_numClasses];
    for (int i = 0; i < m_numClasses; i++) {
      m_probOfClass[i] = 1.0f;
    }
  }
  ...
  public void buildClassifier(Instances instances) throws Exception {
    ...
    float[] wordsPerClass = new float[m_numClasses];
    for (Instance instance : instances) {
      float classValue = instance.value(instance.classIndex());
        ...
        for (int a = 0; a < instance.numValues(); a++) {
          if (instance.index(a) != instance.classIndex()) {
            if (!instance.isMissingSparse(a)) {
              float numOccurrences = (float) (instance.valueSparse(a) * instance.weight());
              if (numOccurrences < 0)
    ...
    */
    for (int c = 0; c < m_numClasses; c++) {
      for (int v = 0; v < m_numAttributes; v++) {
        m_probOfWordGivenClass[c][v] = (float) (Math.log(m_probOfWordGivenClass[c][v]) -
  ...
  public float [] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] probOfClassGivenDoc = new float[m_numClasses];

    //calculate the array of log(Pr[D|C])
    float[] logDocGivenClass = new float[m_numClasses];
    ...
    for (int i = 0; i < m_numClasses; i++) {
      probOfClassGivenDoc[i] =  (float) (Math.exp(logDocGivenClass[i] - max) * m_probOfClass[i]);
    }
  ...
  protected float probOfDocGivenClass(Instance inst, int classIndex) {
  ...
  public String toString()
  {
    ...
    for(int w = 0; w<m_numAttributes; w++)
    {
      if (w != m_headerInfo.classIndex()) {
        ...
        for(int c = 0; c<m_numClasses; c++)
          result.append(Utils.doubleToString((float) Math.exp(m_probOfWordGivenClass[c][w]), getNumDecimalPlaces())).
                  append("\t");
        result.append("\n");
      }
    }
