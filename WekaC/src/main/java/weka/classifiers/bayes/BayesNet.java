...
public class BayesNet extends AbstractClassifier implements OptionHandler,
   ...
   * @throws Exception if there is a problem generating the prediction
   */
  public float[] countsForInstance(Instance instance) throws Exception {
    float[] fCounts = new float[m_NumClasses];
      ...
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
        ...
        for (int iParent = 0; iParent < m_ParentSets[iAttribute]
          ...
          if (nParent == m_Instances.classIndex()) {
            ...
            iCPT = (float) (iCPT * m_Instances.attribute(nParent).numValues()
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("", "", 0,
    		...
    		append(getSearchAlgorithm().getClass().getName()).append(":").toString()));
    newVector.addAll(Collections.list(getSearchAlgorithm().listOptions()));

    newVector.addElement(new Option("", "", 0,
    		new StringBuilder("\nOptions specific to estimator method ").
    		append(getEstimator().getClass().getName()).append(":").toString()));
  ...
  public String[] getOptions() {
    ...
    options.add("-Q");
    options.add(String.valueOf(getSearchAlgorithm().getClass().getName()));
    options.add("--");
    Collections.addAll(options, getSearchAlgorithm().getOptions());

    options.add("-E");
    options.add(String.valueOf(getEstimator().getClass().getName()));
    options.add("--");
  ...
  public String toString() {
    ...
    if (m_Instances == null) {
      ...
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
        text.append(m_Instances.attribute(iAttribute).name()).append("(")
        .append(m_Instances.attribute(iAttribute).numValues()).append("): ");

        for (int iParent = 0; iParent < m_ParentSets[iAttribute]
          .getNrOfParents(); iParent++) {
          text.append(m_Instances.attribute(
            m_ParentSets[iAttribute].getParent(iParent)).name()).append(" ");
        }
      ...
      text.append("LogScore Bayes: ").append(measureBayesScore()).append("\n");
      text.append("LogScore BDeu: ").append(measureBDeuScore()).append("\n");
      text.append("LogScore MDL: ").append(measureMDLScore()).append("\n");
      text.append("LogScore ENTROPY: ").append(measureEntropyScore()).append("\n");
      text.append("LogScore AIC: ").append(measureAICScore()).append("\n");

      if (m_otherBayesNet != null) {
        text.append("Missing: ").append(m_otherBayesNet.missingArcs(this)).
        append(" Extra: ").append(m_otherBayesNet.extraArcs(this)).append(" Reversed: ").
        append(m_otherBayesNet.reversedArcs(this)).append("\n");
        text.append("Divergence: ").append(m_otherBayesNet.divergence(this)).append("\n");
      }
    }
  ...
  public String toXMLBIF03() {
    ...
    text.append("<NETWORK>\n");
    text.append("<NAME>").append(XMLNormalize(Utils.quote(m_Instances.relationName()))).append("</NAME>\n");
    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      text.append("<VARIABLE TYPE=\"nature\">\n");
      text.append("<NAME>").
      append(XMLNormalize(Utils.quote(m_Instances.attribute(iAttribute).name()))).append("</NAME>\n");
      for (int iValue = 0; iValue < m_Instances.attribute(iAttribute)
        .numValues(); iValue++) {
        text.append("<OUTCOME>")
        .append(XMLNormalize(Utils.quote(m_Instances.attribute(iAttribute).value(iValue))))
        .append("</OUTCOME>\n");
      }
    ...
    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      ...
      text.append("<FOR>")
      .append(XMLNormalize(Utils.quote(m_Instances.attribute(iAttribute).name()))).append("</FOR>\n");
      for (int iParent = 0; iParent < m_ParentSets[iAttribute].getNrOfParents(); iParent++) {
        text
          .append("<GIVEN>").append(XMLNormalize(Utils.quote(m_Instances.attribute(
   ...
   * @return probability
   */
  public float getProbability(int iNode, int iParent, int iValue) {
  ...
  public float measureMissingArcs() {
    if (m_otherBayesNet != null) {
  ...
  public float measureReversedArcs() {
    if (m_otherBayesNet != null) {
  ...
  public float measureDivergence() {
    if (m_otherBayesNet != null) {
  ...
  public float measureBayesScore() {
    try {
  ...
  public float measureEntropyScore() {
   ...
   */
  @Override
  public float getMeasure(String measureName) {
