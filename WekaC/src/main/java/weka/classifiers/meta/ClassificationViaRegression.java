...
public class ClassificationViaRegression 
  ...
  public float[] distributionForInstance(Instance inst) throws Exception {
    ...
    for (int i = 0; i < inst.numClasses(); i++) {
      ...
      if (Utils.isMissingValue(probs[i])) {
        return new float[inst.numClasses()]; // Leave instance unclassified
      }
      if (probs[i] > 1) {
        probs[i] = 1;
      }
  ...
  public float[][] distributionsForInstances(Instances insts) throws Exception {
    ...
    float[][] probs;
    if (m_Classifier instanceof BatchPredictor) {
      probs = new float[insts.numInstances()][insts.numClasses()];
      for (int i = 0; i < insts.numClasses(); i++) {
        float[][] p =
  ...
  public String toString() {
    ...
    for (int i = 0; i < m_Classifiers.length; i++) {
      text.append("Classifier for class with index ").append(i).append(":\n\n");
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 14707 $");
  }
