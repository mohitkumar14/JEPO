...
public class Bagging
  ...
  public String [] getOptions() {
    ...
    options.add("-P"); 
    options.add(String.valueOf(getBagSizePercent()));
   ...
   */
  public float measureOutOfBagError() {
    ...
    if (getCalcOutOfBag()) {
      ...
      for (int i = 0; i < m_data.numInstances(); i++) {
        ...
        if (m_Numeric)
          ...
          votes = new float[m_data.numClasses()];
        ...
        for (int j = 0; j < m_Classifiers.length; j++) {
          ...
          if (m_Numeric) {
            float pred = m_Classifiers[j].classifyInstance(m_data.instance(i));
            ...
            voteCount++;
            float[] newProbs = m_Classifiers[j].distributionForInstance(m_data.instance(i));
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] sums = new float[instance.numClasses()], newProbs;

    float numPreds = 0;
    for (int i = 0; i < m_NumIterations; i++) {
      if (m_Numeric) {
        float pred = m_Classifiers[i].classifyInstance(instance);
        if (!Utils.isMissingValue(pred)) {
          sums[0] += pred;
          numPreds++;
        }
  ...
  public String toString() {
    ...
    text.append("Bagging with ").append(getNumIterations()).append(" iterations and base learner\n\n").append(getClassifierSpec());
    if (getPrintClassifiers()) {
  ...
  public float[] getMembershipValues(Instance inst) throws Exception {
    ...
    if (m_Classifier instanceof PartitionGenerator) {
      ...
      int size = 0;
      for (int i = 0; i < m_Classifiers.length; i++) {
        float[] r = ((PartitionGenerator)m_Classifiers[i]).
