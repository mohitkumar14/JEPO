...
public class CVParameterSelection 
  ...
  protected class CVParameter 
    ...
    public CVParameter(String param) throws Exception {
      ...
      try {
        m_Lower = Float.parseFloat(parts[1]);
      } catch (NumberFormatException n) {
        throw new Exception("CVParameter " + param 
      ...
      if (parts[2].equals("A")) {
        ...
        try {
          m_Upper = Float.parseFloat(parts[2]);
      ...
      try {
        m_Steps = Float.parseFloat(parts[3]);
      } catch (NumberFormatException n) {
        throw new Exception("CVParameter " + param 
    ...
    public String toString() {
      ...
      String result = new StringBuilder(m_ParamChar).append(" ").append(m_Lower).append(" ").toString();
  ...
  protected float m_BestPerformance;
  ...
  protected String [] createOptions() {
    ...
    for (int i = 0; i < m_CVParams.size(); i++) {
      ...
      float paramValue = cvParam.m_ParamValue;
      if (cvParam.m_RoundParam) {
        //	paramValue = (double)((int) (paramValue + 0.5));
        paramValue = (float) Math.rint(paramValue);
      }
	...
	if(cvParam.m_RoundParam || isInt)
		options[start++] = String.valueOf(Utils.doubleToString(paramValue,4));
	else
		options[start++] = String.valueOf(cvParam.m_ParamValue);
	//+ Utils.doubleToString(paramValue,4);
      }
    }
  ...
  protected void findParamsByCrossValidation(int depth, Instances trainData,
    ...
    if (depth < m_CVParams.size()) {
      ...
      float increment = (upper - cvParam.m_Lower) / (cvParam.m_Steps - 1);
      ...
      float error = evaluation.errorRate();
      if (m_Debug) {
  ...
  public String [] getOptions() {
    ...
    for (int i = 0; i < m_CVParams.size(); i++) {
      options.add("-P"); options.add(String.valueOf(getCVParameter(i)));
    }
    options.add("-X"); options.add(String.valueOf(getNumFolds()));
  ...
  public String toString() {
    ...
    String result = new StringBuilder("Cross-validated Parameter selection.\nClassifier: ")
    		.append(m_Classifier.getClass().getName()).append("\n").toString();
    try {
      for (int i = 0; i < m_CVParams.size(); i++) {
	CVParameter cvParam = (CVParameter)m_CVParams.elementAt(i);
	result += new StringBuilder("Cross-validation Parameter: '-").append(cvParam.m_ParamChar)
			.append("' ranged from ").append(cvParam.m_Lower).append(" to ");
	switch ((int)(cvParam.m_Lower - cvParam.m_Upper + 0.5)) {
	case 1:
	...
	result += new StringBuilder(" with ").append(cvParam.m_Steps).append(" steps\n");
      }
    } catch (Exception ex) {
      result += ex.getMessage();
    }
    result += new StringBuilder("Classifier Options: ").append(Utils.joinOptions(m_BestClassifierOptions))
