...
public class AdditiveRegression extends IteratedSingleClassifierEnhancer implements OptionHandler, 
  ...
  protected float m_shrinkage = 1.0f;

  /** The mean or median */
  protected float m_InitialPrediction;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      Float temp = Float.valueOf(optionString);
      setShrinkage(temp.floatValue());
    }
  ...
  public String [] getOptions() {
    ...
    options.add("-S"); options.add(String.valueOf(getShrinkage()));
   ...
   */
  public void setShrinkage(float l) {
    m_shrinkage = l;
  }
  ...
  public float classifyInstance(Instance inst) throws Exception {
    ...
    float prediction = m_InitialPrediction;
    ...
    for (Classifier classifier : m_Classifiers) {
      float toAdd = classifier.classifyInstance(inst);
  ...
  private Instances residualReplace(Instances data, Classifier c) throws Exception {
    ...
    for (int i = 0; i < newInst.numInstances(); i++) {
      float pred = c.classifyInstance(newInst.instance(i));
   ...
   * @throws Exception if something goes wrong
   */
  private Instances residualReplace(Instances data, float c) throws Exception {
   ...
   * models)
   */
  public float measureNumIterations() {
