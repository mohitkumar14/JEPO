...
public class MultiClassClassifier 
   ...
   */
  private float m_RandomWidthFactor = 2.0f;
  ...
  public void buildClassifier(Instances insts) throws Exception {
	...
	for (int j=0; j<insts.numClasses(); j++) {
      ...
      m_SumOfWeights = new float[numClassifiers];

      // generate the classifiers
      for (int i=0; i<numClassifiers; i++) {
	RemoveWithValues classFilter = new RemoveWithValues();
	classFilter.setAttributeIndex(String.valueOf(insts.classIndex() + 1));
   ...
   */
  public float[] individualPredictions(Instance inst) throws Exception {
    ...
    if (m_Classifiers.length == 1) {
      ...
      result = new float[m_ClassFilters.length];
  ...
  public float[] distributionForInstance(Instance inst) throws Exception {
    ...
    float[] probs = new float[inst.numClasses()];
    ...
    if (m_Method == METHOD_1_AGAINST_1) {
      ...
      for(int i = 0; i < m_ClassFilters.length; i++) {
	    if (m_Classifiers[i] != null) {
          Instance tempInst = (Instance)inst.copy();
	      tempInst.setDataset(m_TwoClassDataset);
	      float [] current = m_Classifiers[i].distributionForInstance(tempInst);
      ...
      if (getLogLossDecoding()) {
        ...
        for (int i = 0; i < m_ClassFilters.length; i++) {
          m_ClassFilters[i].input(inst);
          m_ClassFilters[i].batchFinished();
          float[] current = m_Classifiers[i].distributionForInstance(m_ClassFilters[i].output());
  ...
  public String toString() {
    ...
    for (int i = 0; i < m_Classifiers.length; i++) {
	  ...
	  if (m_ClassFilters[i] instanceof RemoveWithValues) {
	    ...
	    range.setUpper(m_ClassAttribute.numValues());
	    int[] pair = range.getSelection();
	    text.append(", ").append(pair[0]+1).append(" vs ").append(pair[1]+1);
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (rfactorString.length() != 0) {
      setRandomWidthFactor((new Float(rfactorString)).floatValue());
    } else {
      setRandomWidthFactor(2.0f);
    }
  ...
  public String [] getOptions() {
    ...
    options.add("-M");
    options.add(String.valueOf(m_Method));
    ...
    options.add("-R");
    options.add(String.valueOf(m_RandomWidthFactor));
   ...
   */
  public float getRandomWidthFactor() {
   ...
   */
  public void setRandomWidthFactor(float newRandomWidthFactor) {
   ...
   */
  public static float[] pairwiseCoupling(float[][] n, float[][] r) {

    // Initialize p and u array
    float[] p = new float[r.length];
    for (int i =0; i < p.length; i++) {
      p[i] = 1.0f / (float)p.length;
    }
    float[][] u = new float[r.length][r.length];
    ...
    float[] firstSum = new float[p.length];
    for (int i = 0; i < p.length; i++) {
    ...
    do {
      changed = false;
      float[] secondSum = new float[p.length];
      for (int i = 0; i < p.length; i++) {
	for (int j = i + 1; j < p.length; j++) {
	...
	if ((firstSum[i] == 0) || (secondSum[i] == 0)) {
	  ...
	  float factor = firstSum[i] / secondSum[i];
	  float pOld = p[i];
	  p[i] *= factor;
