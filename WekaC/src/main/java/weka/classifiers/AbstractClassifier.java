...
public abstract class AbstractClassifier implements Classifier, BatchPredictor,
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] dist = new float[instance.numClasses()];
    switch (instance.classAttribute().type()) {
    case Attribute.NOMINAL:
      float classification = classifyInstance(instance);
      if (Utils.isMissingValue(classification)) {
        return dist;
      } else {
        dist[(int) classification] = 1.0f;
      }
      return dist;
    case Attribute.NUMERIC:
    case Attribute.DATE:
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
    		new StringBuilder("\tThe number of decimal places for the output of numbers in the model (default ")
    		.append(m_numDecimalPlaces).append(").").toString(),
      "num-decimal-places", 1, "-num-decimal-places"));
    newVector.addElement(new Option(
    		new StringBuilder("\tThe desired batch size for batch prediction  (default ").append(m_BatchSize)
    		.append(").").toString(),
  ...
  public String[] getOptions() {
    ...
    if (getNumDecimalPlaces() != NUM_DECIMAL_PLACES_DEFAULT) {
      ...
      options.add(String.valueOf(getNumDecimalPlaces()));
    }
    if (!(getBatchSize().equals(BATCH_SIZE_DEFAULT))) {
      options.add("-batch-size");
      options.add(String.valueOf(getBatchSize()));
    }
   ...
   */
  @Override
  public float[][] distributionsForInstances(Instances batch)
    throws Exception {
    float[][] batchPreds = new float[batch.numInstances()][];
