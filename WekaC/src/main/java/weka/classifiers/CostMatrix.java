...
public class CostMatrix implements Serializable, RevisionHandler {
  ...
  public void initialize() {
    ...
    for (int i = 0; i < m_size; i++) {
      for (int j = 0; j < m_size; j++) {
    	  if(i == j)
    		  setCell(i, j,  new Float(0.0));
    	  else
    		  setCell(i, j,  new Float(1.0));
      }
    }
  }
  ...
  public Instances applyCostMatrix(Instances data, Random random)
    ...
    float sumOfWeightFactors = 0, sumOfMissClassWeights, sumOfWeights;
    float[] weightOfInstancesInClass, weightFactor, weightOfInstances;
    ...
    if (replaceStrings(data)) {
      ...
      if (data.classAttribute().numValues() > 2) {
        ...
        // Store new weights
        weightOfInstances = new float[data.numInstances()];
        for (int i = 0; i < data.numInstances(); i++) {
          ...
          if (element instanceof Float) {
            factor = ((Float) element).floatValue();
          } else {
            factor = ((InstanceExpression) element).evaluate(inst);
          }
          weightOfInstances[i] =  (float) (inst.weight() * factor);
          /*
    ...
    weightFactor = new float[data.numClasses()];
    weightOfInstancesInClass = new float[data.numClasses()];
    ...
    for (int i = 0; i < m_size; i++) {
      if (!Utils.eq(((Float) getCell(i, i)).floatValue(), 0)) {
    ...
    for (int i = 0; i < data.numClasses(); i++) {
      ...
      for (int j = 0; j < data.numClasses(); j++) {
        if (Utils.sm(((Float) getCell(i, j)).floatValue(), 0)) {
          throw new Exception("Neg. weights in misclassification "
            + "cost matrix!");
        }
        sumOfMissClassWeights += ((Float) getCell(i, j)).floatValue();
      }
  ...
  public float[] expectedCosts(float[] pred) throws Exception {
    ...
    for (int x = 0; x < m_size; x++) {
      for (int y = 0; y < m_size; y++) {
        ...
        costs[x] += pred[y] * ((Float) element).floatValue();
      }
    }
   ...
   */
  public float[] expectedCosts(float[] pred, Instance inst)
    throws Exception {
    ...
    for (int x = 0; x < m_size; x++) {
      for (int y = 0; y < m_size; y++) {
        ...
        if (!(element instanceof Float)) {
          ...
          costVal = ((Float) element).floatValue();
        }
        costs[x] += pred[y] * costVal;
      }
    }
   ...
   */
  public float getMaxCost(int classVal) throws Exception {
    ...
    for (int i = 0; i < m_size; i++) {
      ...
      float cost = ((Float) element).floatValue();
      if (cost > maxCost)
        maxCost = cost;
    }
   ...
   * @exception Exception if cost matrix contains non-fixed costs
   */
  public float getMaxCost(int classVal, Instance inst) throws Exception {
  ...
  public void normalize() {
    ...
    for (int y = 0; y < m_size; y++) {
      float diag = ((Float) getCell(y, y)).floatValue();
      for (int x = 0; x < m_size; x++) {
        setCell(x, y, new Float(((Float) getCell(x, y)).floatValue() - diag));
      }
    }
  }
  ...
  public void readOldFormat(Reader reader) throws Exception {
    ...
    int currentToken;
    float firstIndex, secondIndex, weight;
    ...
    while (StreamTokenizer.TT_EOF != (currentToken = tokenizer.nextToken())) {
      ...
      firstIndex = (float) tokenizer.nval;
      ...
      secondIndex = (float) tokenizer.nval;
      ...
      weight = (float) tokenizer.nval;
      if (!Utils.gr(weight, 0)) {
        throw new Exception("Only positive weights allowed!");
      }
      setCell((int) firstIndex, (int) secondIndex, new Float(weight));
    }
  }
  ...
  public void write(Writer w) throws Exception {
    ...
    w.write(new StringBuilder("").append(m_size).append("\t").append(m_size).append("\n").toString());
    w.write("% Matrix elements\n");
    for (int i = 0; i < m_size; i++) {
      for (int j = 0; j < m_size; j++) {
        w.write(new StringBuilder("").append(getCell(i, j)).append( "\t").toString());
      }
      w.write("\n");
    }
    w.flush();
    ...
    while (tokRow.hasMoreTokens()) {
      ...
      while (tokCol.hasMoreTokens()) {
        ...
        try {
          ...
          result.setCell(rows, cols, new Float(val));
  ...
  public final float getElement(int rowIndex, int columnIndex)
    throws Exception {
    if (!(m_matrix[rowIndex][columnIndex] instanceof Float)) {
      throw new Exception("Cost matrix contains non-fixed costs!");
    }
    return ((Float) m_matrix[rowIndex][columnIndex]).floatValue();
  }
   ...
   * @exception Exception if something goes wrong
   */
  public final float getElement(int rowIndex, int columnIndex, Instance inst)
    throws Exception {
  ...
  public final void setElement(int rowIndex, int columnIndex, float value) {
    m_matrix[rowIndex][columnIndex] = new Float(value);
  }
  ...
  public String toString() {
    ...
    for (int i = 0; i < size(); i++) {
      for (int j = 0; j < size(); j++) {
        element = getCell(i, j);
        if (element instanceof Float) {
          float current = ((Float) element).floatValue();

          if (current < 0)
            current *= -11;
          if (current > maxval)
            maxval = current;
          float fract = (float) Math.abs(current - Math.rint(current));
    ...
    int width;
    if(widthNumber > widthExpression)
    	width = widthNumber;
    else
    ...
    for (int i = 0; i < size(); i++) {
      for (int j = 0; j < size(); j++) {
        ...
        if (element instanceof Float) {
          text.append(" ").append(
            Utils.doubleToString(((Float) element).floatValue(), width,
              (fractional ? 2 : 0)));
        } else {
  ...
  private static class InstanceExpression {
    ...
    public float evaluate(Instance inst) {
