...
public abstract class ConjugateGradientOptimization extends Optimization
  ...
  public ConjugateGradientOptimization() {
    ...
    m_BETA = 0.1f; // To make line search more exact, recommended for non-linear
                  // CGD
  }
   ...
   */
  @Override
  public float[] findArgmin(float[] initX, float[][] constraints)
    throws Exception {
    ...
    float[] grad = evaluateGradient(initX), oldGrad, oldX, deltaX = new float[l], direct = new float[l], x = new float[l];
    ...
    float[][] consts = new float[2][initX.length];
    for (int i = 0; i < initX.length; i++) {
      if (!Float.isNaN(constraints[0][i])
        || (!Float.isNaN(constraints[1][i]))) {
    ...
    for (int step = 0; step < m_MAXITS; step++) {
      ...
      float[] directB = Arrays.copyOf(direct, direct.length);
      ...
      test = 0.0f;
      for (int g = 0; g < l; g++) {
        float tmp = Math.abs(grad[g]) * Math.max(Math.abs(directB[g]), 1.0f)
      ...
      float betaHSNumerator = 0, betaDYNumerator = 0;
      float betaHSandDYDenominator = 0;
      for (int i = 0; i < grad.length; i++) {
        betaDYNumerator += grad[i] * grad[i];
        betaHSNumerator += (grad[i] - oldGrad[i]) * grad[i];
        betaHSandDYDenominator += (grad[i] - oldGrad[i]) * direct[i];
      }
      float betaHS = betaHSNumerator / betaHSandDYDenominator;
      float betaDY = betaDYNumerator / betaHSandDYDenominator;
