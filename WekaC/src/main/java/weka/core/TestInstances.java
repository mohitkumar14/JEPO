...
public class TestInstances implements Cloneable, Serializable, OptionHandler,
   ...
   */
  protected float generateClassValue(Instances data) throws Exception {
    float result = Float.NaN;

    switch (m_ClassType) {
    case Attribute.NUMERIC:
      result = m_Random.nextFloat() * 0.25f + m_Random.nextInt(Math.max(2, m_NumNominal));
      break;
   ...
   * @throws Exception if something goes wrong
   */
  protected float generateAttributeValue(Instances data, int index,
    float classVal) throws Exception {
    float result = Float.NaN;

    switch (data.attribute(index).type()) {
    case Attribute.NUMERIC:
      result = classVal * 4 + m_Random.nextFloat() * 1 - 0.5f;
      break;
  ...
  public Instances generate(String namePrefix) throws Exception {
    if (getMultiInstance()) {
      ...
      for (int i = 0; i < getNumInstances(); i++) {
        ...
        float val = m_Data.attribute(1).addRelation(bagData);
