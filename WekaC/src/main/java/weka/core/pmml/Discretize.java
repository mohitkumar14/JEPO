...
public class Discretize extends Expression {
  ...
  protected class DiscretizeBin implements Serializable {
     ...
     */
    private float m_numericBinValue = Utils.missingValue();
    ...
    protected DiscretizeBin(Element bin, FieldMetaInfo.Optype opType) throws Exception {
      ...
      if (opType == FieldMetaInfo.Optype.CONTINUOUS
        ...
        try {
          m_numericBinValue = Float.parseFloat(m_binValue);
     ...
     */
    protected float getBinValueNumeric() {
   ...
   * optype, the index of the nominal value is returned as a float.
   * 
  ...
  public float getResult(float[] incoming) throws Exception {
    ...
    // logic applies
    float result = Utils.missingValue();

    float value = incoming[m_fieldIndex];
    ...
    if (Utils.isMissingValue(value)) {
      if (m_mapMissingDefined) {
        if (m_outputDef.isNominal()) {
          ...
          try {
            result = Float.parseFloat(m_mapMissingTo);
          } catch (NumberFormatException ex) {
            throw new Exception(
      ...
      if (!found) {
        if (m_defaultValueDefined) {
          if (m_outputDef.isNominal()) {
            ...
            try {
              result = Float.parseFloat(m_defaultValue);
            } catch (NumberFormatException ex) {
              throw new Exception(
  ...
  public String getResultCategorical(float[] incoming) throws Exception {
    float index = getResult(incoming);
