...
public class MiningFieldMetaInfo extends FieldMetaInfo implements Serializable {
  ...
  /** outlier low value */
  protected float m_lowValue;
  /** outlier high value */
  protected float m_highValue;
  ...
  protected float m_missingValueReplacementNumeric;
   ...
   */
  public float applyMissingValueTreatment(float value) throws Exception {
    float newVal = value;
    if (m_missingValueTreatmentMethod != Missing.ASIS && 
   ...
   * @throws Exception if there is a problem
   */
  public float applyOutlierTreatment(float value) throws Exception {
    float newVal = value;
  ...
  public MiningFieldMetaInfo(Element field) throws Exception {
    ...
    if (importance.length() > 0) {
      m_importance = Float.parseFloat(importance);
    }
    ...
    if (outliers.length() > 0 && m_outlierTreatmentMethod == Outlier.ASEXTREMEVALUES) {
      ...
      if (lowValue.length() > 0) {
        m_lowValue = Float.parseFloat(lowValue);
      } else {
      ...
      if (highValue.length() > 0) {
        m_highValue = Float.parseFloat(highValue);
      } else {
        throw new Exception("[MiningFieldMetaInfo] as extreme values outlier treatment "
    ...
    if (missingReplacement.length() > 0) {
      ...
      try {
        m_missingValueReplacementNumeric = Float.parseFloat(missingReplacement);
