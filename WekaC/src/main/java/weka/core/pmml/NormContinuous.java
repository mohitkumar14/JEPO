...
public class NormContinuous extends Expression {
  ...
  protected float m_mapMissingTo;
  ...
  protected float[] m_linearNormOrig;
  
  /** norm values for the LinearNorm entries */
  protected float[] m_linearNormNorm;
  ...
  public NormContinuous(Element normCont, FieldMetaInfo.Optype opType, ArrayList<Attribute> fieldDefs) 
    ...
    if (mapMissing != null && mapMissing.length() > 0) {
      m_mapMissingTo = Float.parseFloat(mapMissing);
    ...
    m_linearNormOrig = new float[lnL.getLength()];
    m_linearNormNorm = new float[lnL.getLength()];
    ...
    for (int i = 0; i < lnL.getLength(); i++) {
      ...
      if (lnN.getNodeType() == Node.ELEMENT_NODE) {
        ...
        m_linearNormOrig[i] = Float.parseFloat(orig);
        
        String norm = lnE.getAttribute("norm");
        m_linearNormNorm[i] = Float.parseFloat(norm);
      }
    }
...
//      System.err.println("NormCont... index of " + m_fieldName + " " + m_fieldIndex);
   ...
   */
  public float getResult(float[] incoming) throws Exception {
    
    float[] a = m_linearNormOrig;
    float[] b = m_linearNormNorm;
   ...
   * 
   * @param preds the incoming parameter values
   * @return the unormalized value
   */
  public float getResultInverse(float[] preds) {
    float[] a = m_linearNormNorm;
    float[] b = m_linearNormOrig;
    
    return computeNorm(a, b, preds);
  }
  
  private float computeNorm(float[] a, float[] b, float[] incoming) {
    float result = 0.0f;
    ...
    if (Utils.isMissingValue(incoming[m_fieldIndex])) {
      ...
      if (x < a[0]) {
        ...
        int length = a.length;
        if (m_outlierTreatmentMethod == MiningFieldMetaInfo.Outlier.ASIS) {
          float slope = (b[length - 1] - b[length - 2]) /
            (a[length - 1] - a[length - 2]);
          float offset = b[length - 1] - (slope * a[length - 1]);
          result = slope * x + offset;
        } else if (m_outlierTreatmentMethod == MiningFieldMetaInfo.Outlier.ASEXTREMEVALUES) {
          result = b[length - 1];
        } else {
   ...
   */
  public String getResultCategorical(float[] incoming) throws Exception {
