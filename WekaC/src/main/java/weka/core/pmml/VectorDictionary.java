...
public class VectorDictionary implements Serializable {
   ...
   * 
   * @param input an incoming instance
   ...
   */
  public float[] incomingInstanceToVectorFieldVals(float[] input) 
    throws Exception {
    float[] newInst = new float[m_vectorFields.size()];
    
    for (int i = 0; i < m_vectorFields.size(); i++) {
      FieldRef fr = m_vectorFields.get(i);
      newInst[i] = fr.getResult(input);
    }
