...
public class TargetMetaInfo extends FieldMetaInfo implements Serializable {
  ...
  /** min and max */
  protected float m_min = Float.NaN;
  protected float m_max = Float.NaN;

  /** re-scaling of target value (if defined) */
  protected float m_rescaleConstant = 0;
  protected float m_rescaleFactor = 1.0f;
  ...
  protected float[] m_defaultValueOrPriorProbs;
  ...
  protected TargetMetaInfo(Element target) throws Exception {
    ...
    if (rsc != null && rsc.length() > 0) {
      try {
        m_rescaleConstant = Float.parseFloat(rsc);
      } catch (IllegalArgumentException ex) {
    ...
    if (rsf != null && rsf.length() > 0) {
      try {
        m_rescaleFactor = Float.parseFloat(rsf);
      } catch (IllegalArgumentException ex) {
    ...
    if (vals.getLength() > 0) {
      m_defaultValueOrPriorProbs = new float[vals.getLength()];
        ...
        if (value.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (valueName != null && valueName.length() > 0) {
            ...
            if (prior != null && prior.length() > 0) {
              try {
                m_defaultValueOrPriorProbs[i] = Float.parseFloat(prior);
              } catch (IllegalArgumentException ex) {
            ...
            if (defaultV != null && defaultV.length() > 0) {
              try {
                m_defaultValueOrPriorProbs[i] = Float.parseFloat(defaultV);
              } catch (IllegalArgumentException ex) {
   ...
   */
  public float getPriorProbability(String value) throws Exception {
   ...
   */
  public float getDefaultValue() throws Exception {
    if (m_defaultValueOrPriorProbs == null) {
      throw new Exception("[TargetMetaInfo] no TargetValues defined (getPriorProbability)");
    }
   ...
   */
  public float applyMinMaxRescaleCast(float prediction) throws Exception {
    if (m_optype != Optype.CONTINUOUS) {
    ...
    if (m_castInteger.length() > 0) {
      if (m_castInteger.equals("round")) {
        ...
        prediction = (float) Math.floor(prediction);
      } else {
