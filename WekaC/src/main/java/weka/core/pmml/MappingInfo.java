...
public class MappingInfo implements Serializable {
   ...
   */
  public float[] instanceToSchema(Instance inst, MiningSchema miningSchema)
    throws Exception {
    ...
    // fields
    float[] result = new float[miningSchema.getFieldsAsInstances()
      .numAttributes()];
    ...
    for (int i = 0; i < derivedFields.size(); i++) {
      ...
      float r = temp.getDerivedValue(result);
