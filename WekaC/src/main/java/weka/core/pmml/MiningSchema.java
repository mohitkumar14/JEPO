...
public class MiningSchema implements Serializable {
   ...
   */
  public void applyMissingValuesTreatment(float[] values) throws Exception {
   ...
   */
  public void applyOutlierTreatment(float[] values) throws Exception {
    for (int i = 0; i < m_miningMeta.size(); i++) {
      MiningFieldMetaInfo mfi = m_miningMeta.get(i);
   ...
   * @param result an array of doubles in order of the fields in the mining schema
   ...
   */
  public void applyMissingAndOutlierTreatments(float[] result) throws Exception {
    for (int i = 0; i < m_miningMeta.size(); i++) {
      MiningFieldMetaInfo mfi = m_miningMeta.get(i);
      result[i] = mfi.applyMissingValueTreatment(result[i]);
      result[i] = mfi.applyOutlierTreatment(result[i]);
    }
  }
