...
public class LinearRegression
  ...
  /** the coefficients */
  protected float[] m_Coefficients = null;
   ...
   */
  public LinearRegression(Matrix a, Matrix y, float ridge) {
    calculate(a, y, ridge);
  }
   ...
   * provided.
   */
  public LinearRegression(Matrix a, Matrix y, float[] w, float ridge) {
    ...
    for (int i = 0; i < w.length; i++) {
      float sqrt_weight = (float) Math.sqrt(w[i]);
   ...
   * @throws IllegalArgumentException if not successful
   */
  protected void calculate(Matrix a, Matrix y, float ridge) {
  ...
  private static Matrix aTa(Matrix a) {
    ...
    float[][] X = x.getArray();
    float[] Acol = new float[a.getRowDimension()];
