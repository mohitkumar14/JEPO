...
public class SparseInstance extends AbstractInstance {
  ...
  public SparseInstance(Instance instance) {
    ...
    if (instance instanceof SparseInstance) {
      ...
      float[] tempValues = new float[instance.numAttributes()];
   ...
   * @param newVals a vector of attribute values
   */
  public SparseInstance(float weight, float[] newVals) {

    m_Weight = weight;
    m_Dataset = null;
    m_NumAttributes = newVals.length;
    float[] tempValues = new float[m_NumAttributes];
    int[] tempIndices = new int[m_NumAttributes];
    int vals = 0;
    for (int i = 0; i < m_NumAttributes; i++) {
      if (newVals[i] != 0) {
        tempValues[vals] = newVals[i];
        tempIndices[vals] = i;
        vals++;
      }
    }
   ...
   */
  public SparseInstance(float weight, float[] attValues, int[] indices,
    int maxNumValues) {

    int vals = 0;
    m_AttValues = new float[attValues.length];
  ...
  public SparseInstance(int numAttributes) {
    ...
    m_AttValues = new float[numAttributes];
   ...
   */
  public Instance copy(float[] values) {
  ...
  public Instance mergeInstance(Instance inst) {
    ...
    float[] values = new float[numValues() + inst.numValues()];
    ...
    return new SparseInstance(1.0f, values, indices, numAttributes()
      + inst.numAttributes());
  }
  ...
  public void setValue(int attIndex, float value) {
    ...
    if ((index >= 0) && (m_Indices[index] == attIndex)) {
      if (value != 0) {
        ...
        float[] tempValues = new float[m_AttValues.length - 1];
   ...
   */
  @Override
  public void setValueSparse(int indexOfIndex, float value) {
