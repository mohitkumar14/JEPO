...
public class MinkowskiDistance extends NormalizableDistance implements
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() > 0) {
      setOrder(Float.parseFloat(tmpStr));
    } else {
      setOrder(2);
    }
  ...
  public float distance(Instance first, Instance second) {
    return (float) Math.pow(distance(first, second, Float.POSITIVE_INFINITY),
      1 / m_Order);
  }
   ...
   */
  @Override
  public float distance(Instance first, Instance second, PerformanceStats stats) { // debug
                                                                                    // method
                                                                                    // pls
                                                                                    // remove
                                                                                    // after
                                                                                    // use
    return (float) Math.pow(distance(first, second, Float.POSITIVE_INFINITY, stats),
      1 / m_Order);
  }
  ...
  public void postProcessDistances(float distances[]) {
    for (int i = 0; i < distances.length; i++) {
      distances[i] = (float) Math.pow(distances[i], 1 / m_Order);
    }
  }
