...
public class EuclideanDistance
  ...
  public float distance(Instance first, Instance second) {
    return (float) Math.sqrt(distance(first, second, Float.POSITIVE_INFINITY));
  }
   ...
   */
  public float distance(Instance first, Instance second, PerformanceStats stats) { //debug method pls remove after use
    return (float) Math.sqrt(distance(first, second, Float.POSITIVE_INFINITY, stats));
  }
   ...
   */
  protected float updateDistance(float currDist, float diff) {
    float	result;
   ...
   */
  public float sqDifference(int index, float val1, float val2) {
    float val = difference(index, val1, val2);
    return val*val;
  }
   ...
   */
  public float getMiddle(float[] ranges) {

    float middle = ranges[R_MIN] + ranges[R_WIDTH] * 0.5f;
    return middle;
  }
  ...
  public int closestPoint(Instance instance, Instances allPoints,
    ...
    for (int i = 0; i < pointList.length; i++) {
      float dist = distance(instance, allPoints.instance(pointList[i]), Float.POSITIVE_INFINITY);
      if (dist < minDist) {
        minDist = dist;
        bestPoint = i;
      }
