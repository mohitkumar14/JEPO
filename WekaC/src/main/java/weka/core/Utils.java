...
public final class Utils implements RevisionHandler {
   ...
   */
  public static float SMALL = 1e-6f;
   ...
   */
  public static float dateToMillis(String dateString, String dateFormat)
   ...
   */
  public static boolean isMissingValue(float val) {
   ...
   * 
   * @param numAttVals double vector 1
   * @param classVals double vector 2
   * @param n the length of two double vectors
   * @return the correlation coefficient
   */
  public static final float correlation(float[] numAttVals, float[] classVals, int n) {

    int i;
    float av1 = 0.0f, av2 = 0.0f, y11 = 0.0f, y22 = 0.0f, y12 = 0.0f, c;
    ...
    av2 /= n;
    for (i = 0; i < n; i++) {
      y11 += (numAttVals[i] - av1) * (numAttVals[i] - av1);
      y22 += (classVals[i] - av2) * (classVals[i] - av2);
      y12 += (numAttVals[i] - av1) * (classVals[i] - av2);
    }
    if (y11 * y22 == 0.0) {
      c = 1.0f;
    } else {
      c = (float) (y12 / Math.sqrt(Math.abs(y11 * y22)));
    }
  ...
  public static String padRight(String inString, int length) {
    ...
    return String.format(new StringBuilder("%1$-").append(length).append(".").append(length).append("s").toString(), inString);
  }
   ...
   */
  public static/* @pure@ */String doubleToString(float value,
   ...
   * @return the double as a formatted string
   */
  public static/* @pure@ */String doubleToString(float value, int width,
    int afterDecimalPoint) {
   ...
   * @param b a double
   */
  public static/* @pure@ */boolean eq(float a, float b) {
   ...
   *         array [a b c]
   */
  public static/* @pure@ */float info(int counts[]) {
   ...
   * @param b a double
   */
  public static/* @pure@ */boolean smOrEq(float a, float b) {
   ...
   * @param b a double
   */
  public static/* @pure@ */boolean grOrEq(float a, float b) {
   ...
   * @param b a double
   */
  public static/* @pure@ */boolean sm(float a, float b) {
   ...
   * @param b a double
   */
  public static/* @pure@ */boolean gr(float a, float b) {
   ...
   */
  public static int kthSmallestValue(int[] vals, int k) {

    int[] index = initialIndex(vals.length);
    return vals[index[select(vals, index, 0, vals.length - 1, k)]];
  }
   ...
   * @return the kth-smallest value
   */
  public static float kthSmallestValue(float[] array, int k) {
   ...
   */
  public static/* @pure@ */float log2(float a) {

    return (float) (Math.log(a) / log2);
  }
   ...
   * is returned.
   * 
   * @param logDocGivenClass the array of doubles
   * @return the index of the maximum element
   */
  public static/* @pure@ */int maxIndex(float[] logDocGivenClass) {

    float maximum = 0;
    int maxIndex = 0;

    for (int i = 0; i < logDocGivenClass.length; i++) {
      if ((i == 0) || (logDocGivenClass[i] > maximum)) {
        maxIndex = i;
        maximum = logDocGivenClass[i];
      }
    }
   ...
   * @return the mean
   */
  public static/* @pure@ */float mean(float[] vector) {
   ...
   * @return the index of the minimum element
   */
  public static/* @pure@ */int minIndex(float[] doubles) {
   ...
   * 
   * @param probs the array of double
   ...
   * 
   * @param num the array of double
   * @param sum the value by which the doubles are to be normalized
   * @exception IllegalArgumentException if sum is zero or NaN
   */
  public static void normalize(float[] num, float sum) {
   ...
   * to one.
   * 
   * @param sums an array holding the natural logarithms of the probabilities
   * @return the converted array
   */
  public static float[] logs2probs(float[] sums) {

    float max = sums[maxIndex(sums)];
    float sum = 0.0f;

    float[] result = new float[sums.length];
    for (int i = 0; i < sums.length; i++) {
      result[i] = (float) Math.exp(sums[i] - max);
      sum += result[i];
    }
   ...
   *         1-Utils.SMALL]
   */
  public static/* @pure@ */float probToLogOdds(float prob) {

    if (gr(prob, 1) || (sm(prob, 0))) {
      throw new IllegalArgumentException("probToLogOdds: probability must "
        + "be in [0,1] " + prob);
    }
    float p = SMALL + (1.0f - 2 * SMALL) * prob;
   ...
   */
  public static/* @pure@ */int round(float value) {
   ...
   * @return the resulting integer value
   */
  public static int probRound(float value, Random rand) {

    if (value >= 0) {
      float lower = (float) Math.floor(value);
      float prob = value - lower;
      ...
      float lower = (float) Math.floor(Math.abs(value));
      float prob = Math.abs(value) - lower;
      if (rand.nextFloat() < prob) {
        return -((int) lower + 1);
      } else {
        return -(int) lower;
   ...
   */
  public static void replaceMissingWithMAX_VALUE(float[] array) {
   ...
   */
  public static/* @pure@ */float roundDouble(float value,
    int afterDecimalPoint) {

    float mask = (float) Math.pow(10.0f, afterDecimalPoint);
   ...
   * order.)
   * 
   * @param m_Objects this array is not changed by the method!
   * @return an array of integers with the positions in the sorted array.
   */
  public static/* @pure@ */int[] sort(int[] m_Objects) {

    int[] index = initialIndex(m_Objects.length);
    int[] newIndex = new int[m_Objects.length];
    int[] helpIndex;
    int numEqual;

    quickSort(m_Objects, index, 0, m_Objects.length - 1);

    // Make sort stable
    int i = 0;
    while (i < index.length) {
      numEqual = 1;
      for (int j = i + 1; ((j < index.length) && (m_Objects[index[i]] == m_Objects[index[j]])); j++) {
        numEqual++;
      }
      if (numEqual > 1) {
   ...
   * 
   * @param vals the array to be sorted, which is modified if it has missing
   *          values
   * @return an array of integers with the positions in the sorted array.
   */
  ...
  /* @non_null@ */float[] vals) {
    ...
    int[] index = initialIndex(vals.length);
    if (vals.length > 1) {
      quickSort(vals, index, 0, vals.length - 1);
    }
    return index;
  }
   ...
   * 
   * @param predictedDistribution this array is not changed by the method!
   * @return an array of integers with the positions in the sorted array.
   */
  public static/* @pure@ */int[] stableSort(float[] predictedDistribution) {

    int[] index = initialIndex(predictedDistribution.length);

    if (predictedDistribution.length > 1) {

      int[] newIndex = new int[predictedDistribution.length];
      int[] helpIndex;
      int numEqual;

      predictedDistribution = predictedDistribution.clone();
      replaceMissingWithMAX_VALUE(predictedDistribution);
      quickSort(predictedDistribution, index, 0, predictedDistribution.length - 1);
      ...
      while (i < index.length) {
        numEqual = 1;
        for (int j = i + 1; ((j < index.length) && (predictedDistribution[index[i]] == predictedDistribution[index[j]])); j++) {
          numEqual++;
        }
        if (numEqual > 1) {
          helpIndex = new int[numEqual];
   ...
   * 
   * @param numAttVals the array
   * @return the variance
   */
  public static/* @pure@ */float variance(float[] numAttVals) {

    if (numAttVals.length <= 1)
      return Float.NaN;

    float mean = 0;
    float var = 0;

    for (int i = 0; i < numAttVals.length; i++) {
      float delta = numAttVals[i] - mean;
      mean += delta / (i + 1);
      var += (numAttVals[i] - mean) * delta;
    }

    var /= numAttVals.length - 1;
   ...
   */
  public static/* @pure@ */float xlogx(int c) {
   ...
   * pivot.
   */
  private static int sortLeftRightAndCenter(float[] array, int[] index, int l,
    int r) {
   ...
   */
  private static void conditionalSwap(float[] array, int[] index, int left,
    int right) {
   ...
   * 
   * @param m_rankResults the array of doubles to be sorted
   * @param index the index into the array of doubles
  ...
  // assignable index;
  private static void quickSort(/* @non_null@ */float[] m_rankResults, /* @non_null@ */
    ...
    case 1:
      ...
      conditionalSwap(m_rankResults, index, left, right);
      return;
    case 2:

      // Just need to sort three elements
      conditionalSwap(m_rankResults, index, left, left + 1);
      conditionalSwap(m_rankResults, index, left, right);
      conditionalSwap(m_rankResults, index, left + 1, right);
      return;
    default:

      // Establish pivot
      int pivotLocation = sortLeftRightAndCenter(m_rankResults, index, left, right);

      // Move pivot to the right, partition, and restore pivot
      swap(index, pivotLocation, right - 1);
      int center =
        partition(m_rankResults, index, left, right, m_rankResults[index[right - 1]]);
      swap(index, center, right - 1);

      // Sort recursively
      quickSort(m_rankResults, index, left, center - 1);
      quickSort(m_rankResults, index, center + 1, right);
    }
  }
   ...
   */
  // @ requires 0 <= first && first <= right && right < array.length;
  private static int select(/* @non_null@ */float[] array, /* @non_null@ */
   ...
   *
   * @param attributeWeights the (positive) weights to be used to determine sample probabilities by normalization
   * @param random the random number generator to be used
   *
   * @return the histogram of items in the sample
   */
  public static int[] takeSample(float[] attributeWeights, Random random) {

    // Walker's method, see pp. 232 of "Stochastic Simulation" by B.D. Ripley
    float[] P = new float[attributeWeights.length];
    System.arraycopy(attributeWeights, 0, P, 0, attributeWeights.length);
    Utils.normalize(P);
    float[] Q = new float[attributeWeights.length];
    int[] A = new int[attributeWeights.length];
    int[] W = new int[attributeWeights.length];
    int M = attributeWeights.length;
    int NN = -1;
    int NP = M;
    for (int I = 0; I < M; I++) {
      if (P[I] < 0) {
    ...
    int[] result = new int[attributeWeights.length];

    for (int i = 0; i < attributeWeights.length; i++) {
      int ALRV;
      float U = M * random.nextFloat();
      int I = (int) U;
      if (U < Q[I]) {
        ALRV = I;
      } else {
  ...
  public static void main(String[] ops) {
    ...
    float[] doublesWithNaN = { 4.5f, 6.7f, Float.NaN, 3.4f, 4.8f, 1.2f, 3.4f };
    float[] doubles = { 4.5f, 6.7f, 6.7f, 3.4f, 4.8f, 1.2f, 3.4f, 6.7f, 6.7f, 3.4f };
    ...
    try {
      ...
      for (float element : doublesWithNaN) {
      ...
      // Pretty-printing
      System.out.println("-4.58: " + Utils.doubleToString(-4.57826535f, 2));
      System.out.println("-6.78: " + Utils.doubleToString(-6.78214234f, 6, 2));

      // Comparisons
      System.out.println("5.70001 == 5.7 ? " + Utils.eq(5.70001f, 5.7f));
      System.out.println("5.70001 > 5.7 ? " + Utils.gr(5.70001f, 5.7f));
      System.out.println("5.70001 >= 5.7 ? " + Utils.grOrEq(5.70001f, 5.7f));
      System.out.println("5.7 < 5.70001 ? " + Utils.sm(5.7f, 5.70001f));
      System.out.println("5.7 <= 5.70001 ? " + Utils.smOrEq(5.7f, 5.70001f));

      // Math
      System.out.println("Info (ints): " + Utils.info(ints));
      System.out.println("log2(4.6): " + Utils.log2(4.6f));
      System.out.println("5 * log(5): " + Utils.xlogx(5));
      System.out.println("5.5 rounded: " + Utils.round(5.5f));
      System.out.println("5.55555 rounded to 2 decimal places: "
        + Utils.roundDouble(5.55555f, 2));
