...
public class DenseInstance extends AbstractInstance {
   ...
   * @param instance a vector of attribute values
   */
  // @ ensures m_Dataset == null;
  public DenseInstance(float weight, /* @non_null@ */float[] instance) {

    m_AttValues = instance;
    m_Weight = weight;
    m_Dataset = null;
  }
   ...
   * of floats. The copy has access to the same dataset.
   *
  ...
  public Instance mergeInstance(Instance inst) {
    ...
    int m = 0;
    float[] newVals = new float[numAttributes() + inst.numAttributes()];
    ...
    return new DenseInstance(1.0f, newVals);
  }
  ...
  public float[] toDoubleArray() {
    ...
    float[] newValues = new float[m_AttValues.length];
   ...
   */
  @Override
  public/* @pure@ */float value(int attIndex) {
  ...
  protected void forceDeleteAttributeAt(int position) {
    ...
    float[] newValues = new float[m_AttValues.length - 1];
