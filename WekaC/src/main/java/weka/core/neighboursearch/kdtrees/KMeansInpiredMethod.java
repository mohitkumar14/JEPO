...
public class KMeansInpiredMethod extends KDTreeNodeSplitter implements
  ...
  public void splitNode(KDTreeNode node, int numNodesCreated,
    float[][] nodeRanges, float[][] universe) throws Exception {

    correctlyInitialized();

    int splitDim = -1;
    float splitVal = Float.NEGATIVE_INFINITY;

    float leftAttSum[] = new float[m_Instances.numAttributes()], rightAttSum[] = new float[m_Instances
      .numAttributes()], leftAttSqSum[] = new float[m_Instances
      .numAttributes()], rightAttSqSum[] = new float[m_Instances
      .numAttributes()], rightSqMean, leftSqMean, leftSqSum, rightSqSum, minSum = Float.POSITIVE_INFINITY, val;
    ...
    for (int dim = 0; dim < m_Instances.numAttributes(); dim++) {
      ...
      for (int i = node.m_Start; i <= node.m_End; i++) {
        for (int j = 0; j < m_Instances.numAttributes(); j++) {
          ...
          if (i == node.m_Start) {
            leftAttSum[j] = rightAttSum[j] = leftAttSqSum[j] = rightAttSqSum[j] = 0.0f;
          }
          rightAttSum[j] += val;
  ...
  protected static int partition(Instances insts, int[] index, int attidx,
    ...
    float pivot = insts.instance(index[(l + r) / 2]).value(attidx);
    int help;
  ...
  protected int rearrangePoints(int[] indices, final int startidx,
    final int endidx, final int splitDim, final float splitVal) {
