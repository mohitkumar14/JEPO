  ...
  public Instances kNearestNeighbours(Instance target, int kNN) throws Exception {
    ...
    for(int i=0; i<m_Instances.numInstances(); i++) {
      ...
      if(firstkNN<kNN) {
        ...
        distance = m_DistanceFunction.distance(target, m_Instances.instance(i), Float.POSITIVE_INFINITY, m_Stats);
    ...
    m_Distances = new float[heap.size()+heap.noOfKthNearest()];
