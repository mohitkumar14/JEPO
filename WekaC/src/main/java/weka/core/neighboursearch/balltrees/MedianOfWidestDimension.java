...
public class MedianOfWidestDimension extends BallSplitter implements
  ...
  public void splitNode(BallNode node, int numNodesCreated) throws Exception {
    ...
    float[][] ranges = m_DistanceFunction.initializeRanges(m_Instlist,
   ...
   * 
   * @param ranges The attributes' range of the points inside the node that
   *          is to be split.
   * @param fs The attributes' range for the whole point-space.
   * @return The index of the attribute/dimension in which the points of the
   *         node have widest spread.
   */
  protected int widestDim(float[][] ranges, float[][] fs) {
    final int classIdx = m_Instances.classIndex();
    double widest = 0.0;
    int w = -1;
    if (m_NormalizeDimWidths) {
      for (int i = 0; i < ranges.length; i++) {
        double newWidest = ranges[i][NormalizableDistance.R_WIDTH]
          / fs[i][NormalizableDistance.R_WIDTH];
        if (newWidest > widest) {
          if (i == classIdx) {
            continue;
          }
      ...
      for (int i = 0; i < ranges.length; i++) {
        if (ranges[i][NormalizableDistance.R_WIDTH] > widest) {
          if (i == classIdx) {
            continue;
          }
          widest = ranges[i][NormalizableDistance.R_WIDTH];
          w = i;
        }
      }
    }
