...
public class BallNode
   ...
   */
  public BallNode(int start, int end, int nodeNumber, Instance pivot, float radius) {
    m_Start = start;
    m_End = end;
    m_NodeNumber = nodeNumber; 
    m_Pivot = pivot;
   ...
   * radius. 
   */
  public static float calcRadius(int[] instList, Instances insts,Instance pivot, 
   ...
   * radius. 
   */
  public static float calcRadius(int start, int end, int[] instList, 
    ...
    for(int i=start; i<=end; i++) {
      float dist = distanceFunction.distance(pivot, 
                                              insts.instance(instList[i]), Float.POSITIVE_INFINITY);
  ...
  public static Instance calcPivot(BallNode child1, BallNode child2, 
    ...
    float[] attrVals = new float[p1.numAttributes()];
    ...
    p1 = new DenseInstance(1.0f, attrVals);
    return p1;
  }
   ...
   */
  public static float calcRadius(BallNode child1, BallNode child2, 
                                  Instance pivot, 
                                  DistanceFunction distanceFunction) 
                                                             throws Exception {
    Instance p1 = child1.getPivot(), p2 = child2.getPivot();                                                               
    
    float radius = child1.getRadius() + distanceFunction.distance(p1, p2) + 
                    child2.getRadius();
