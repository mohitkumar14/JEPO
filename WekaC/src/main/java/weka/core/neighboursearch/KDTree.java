...
public class KDTree
  ...
  protected void buildKDTree(Instances instances) throws Exception {
    ...
    float[][] universe = m_EuclideanDistance.getRanges();
   ...
   * splitting.
   */
  protected void splitNodes(KDTreeNode node, float[][] universe,
      int depth) throws Exception {
    float[][] nodeRanges = m_EuclideanDistance.initializeRanges(m_InstList,
  ...
  protected void findNearestNeighbours(Instance target, KDTreeNode node, int k,
      MyHeap heap, float distanceToParents) throws Exception {
    if (node.isALeaf()) {
      if (m_TreeStats != null) {
      ...
      for (int idx = node.m_Start; idx <= node.m_End; idx++) {
        ...
        if (heap.size() < k) {
          distance = m_EuclideanDistance.distance(target, m_Instances
              .instance(m_InstList[idx]), Float.POSITIVE_INFINITY, m_Stats);
      ...
      if (heap.size() < k) { // if haven't found the first k
        float distanceToSplitPlane = distanceToParents
  ...
  public Instances kNearestNeighbours(Instance target, int k) throws Exception {
    ...
    findNearestNeighbours(target, m_Root, k, heap, 0.0f);
    ...
    Instances neighbours = new Instances(m_Instances, (heap.size() + heap
        .noOfKthNearest()));
    m_DistanceList = new float[heap.size() + heap.noOfKthNearest()];
   ...
   */
  protected float getMaxRelativeNodeWidth(float[][] nodeRanges,
      float[][] universe) {
   ...
   * dimension/attribute.
   */
  protected int widestDim(float[][] nodeRanges, float[][] universe) {
    final int classIdx = m_Instances.classIndex();
    float widest = 0.0f;
    int w = -1;
    if (m_NormalizeNodeWidth) {
      for (int i = 0; i < nodeRanges.length; i++) {
        float newWidest = nodeRanges[i][WIDTH] / universe[i][WIDTH];
        if (newWidest > widest) {
          if (i == classIdx)
            continue;
          widest = newWidest;
   ...
   */
  public void centerInstances(Instances centers, int[] assignments, float pc)
      throws Exception {
  ...
  protected void determineAssignments(KDTreeNode node, Instances centers,
      int[] candidates, int[] assignments, float pc) throws Exception {
  ...
  protected int[] refineOwners(KDTreeNode node, Instances centers,
    ...
    float[] distance = new float[numCand];
   ...
   */
  protected float distanceToHrect(KDTreeNode node, Instance x) throws Exception {
    float distance = 0.0f;
  ...
  protected boolean candidateIsFullOwner(KDTreeNode node, Instance candidate,
    ...
    // get extreme point
    float[] extreme = new float[m_Instances.numAttributes()];
    for (int i = 0; i < m_Instances.numAttributes(); i++) {
  ...
  protected float m_MinBoxRelWidth = 1.0E-2f;
   ...
   */
  public void setMinBoxRelWidth(float i) {
    m_MinBoxRelWidth = i;
  }
   ...
   */
  public float getMinBoxRelWidth() {
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0)
      setMinBoxRelWidth(Float.parseFloat(optionString));
    else
      setMinBoxRelWidth(1.0E-2f);
