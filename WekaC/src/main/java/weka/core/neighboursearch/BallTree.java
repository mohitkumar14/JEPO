...
public class BallTree
  ...
  public Instances kNearestNeighbours(Instance target, int k) throws Exception {
    ...
    m_Distances = new float[heap.totalSize()];
  ...
  protected void nearestNeighbours(MyHeap heap, BallNode node, Instance target,
    ...
    if (distance > -0.000001
      ...
      float leftPivotDist = (float) Math.sqrt(m_DistanceFunction.distance(target,
          node.m_Left.getPivot(), Float.POSITIVE_INFINITY));
      float rightPivotDist = (float) Math.sqrt(m_DistanceFunction.distance(target,
          node.m_Right.getPivot(), Float.POSITIVE_INFINITY));
      float leftBallDist = leftPivotDist - node.m_Left.getRadius();
      float rightBallDist = rightPivotDist - node.m_Right.getRadius();
      ...
      for (int i = node.m_Start; i <= node.m_End; i++) {
        ...
        if (heap.totalSize() < k) {
          distance = m_DistanceFunction.distance(target, m_Instances
              .instance(m_InstList[i]), Float.POSITIVE_INFINITY, m_Stats);
