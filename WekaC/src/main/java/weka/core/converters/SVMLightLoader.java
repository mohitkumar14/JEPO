...
public class SVMLightLoader 
  ...
  protected Vector<float[]> m_Buffer = null;
   ...
   */
  protected float[] svmlightToArray(String row) throws Exception {
    float[]		result;
    StringTokenizer	tok;
    int			index;
    int			max;
	...
	if (index > max)
      ...
      tok    = new StringTokenizer(row, " \t");
      result = new float[max + 1];

      // 1. class
      result[result.length - 1] = Float.parseFloat(tok.nextToken());
	...
	// actual value
	index = Integer.parseInt(col.substring(0, col.indexOf(":")));
	value = Float.parseFloat(col.substring(col.indexOf(":") + 1));
   ...
   */
  protected int determineNumAttributes(float[] values, int num) throws Exception {
    int		result;
    int		count;
  ...
  public Instances getStructure() throws IOException {
	...
	while ((cInt = m_sourceReader.read()) != -1) {
	  ...
	  if ((c == '\n') || (c == '\r')) {
		...
		numAtt = determineNumAttributes((float[]) m_Buffer.lastElement(), numAtt);
	      }
	      catch (Exception e) {
  ...
  public Instances getDataSet() throws IOException {
    ...
    for (i = 0; i < m_Buffer.size(); i++) {
      sparse = (float[]) m_Buffer.get(i);
      
      if (sparse.length != m_structure.numAttributes()) {
	data = new float[m_structure.numAttributes()];
	// attributes
