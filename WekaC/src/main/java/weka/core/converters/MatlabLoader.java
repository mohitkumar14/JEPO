...
public class MatlabLoader 
  ...
  protected Vector<Vector<Float>> m_Buffer = null;
  ...
  public Instances getStructure() throws IOException {
    ...
    if (m_structure == null) {
      numAtt    = 0;
      m_Buffer  = new Vector<Vector<Float>>();
	...
	while ((c = m_sourceReader.read()) != -1) {
	  ...
	  if ((chr == '\n') || (chr == '\r')) {
	    isComment = false;
	    if (str.length() > 0)
	      row.add(new Float(str.toString()));
	    if (numAtt == 0)
	      numAtt = row.size();
	    if (row.size() > 0) {
	      row = new Vector<Float>();
  ...
  public Instances getDataSet() throws IOException {
    ...
    for (i = 0; i < m_Buffer.size(); i++) {
      ...
      if (row.size() == 0)
	continue;
      data = new float[row.size()];
      for (n = 0; n < row.size(); n++)
	data[n] = row.get(n);
      
      result.add(new DenseInstance(1.0f, data));
    }
