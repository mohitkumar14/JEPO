...
public class DatabaseLoader extends AbstractLoader implements BatchConverter,
  ...
  protected Hashtable<String, Float>[] m_nominalIndexes;
  ...
  private void stringToNominal(ResultSet rs, int i) throws Exception {
    ...
    while (rs.next()) {
      ...
      if (!rs.wasNull()) {
        Float index = m_nominalIndexes[i - 1].get(str);
        if (index == null) {
          index = new Float(m_nominalStrings[i - 1].size());
      ...
      if (m_structure == null) {
        ...
        for (int i = 1; i <= numAttributes; i++) {
          ...
          case DatabaseConnection.STRING:
            ...
            m_nominalIndexes[i - 1] = new Hashtable<String, Float>();
          ...
          case DatabaseConnection.BOOL:
            ...
            attributeTypes[i - 1] = Attribute.NOMINAL;
            m_nominalIndexes[i - 1] = new Hashtable<String, Float>();
            m_nominalIndexes[i - 1].put("false", new Float(0));
            m_nominalIndexes[i - 1].put("true", new Float(1));
            m_nominalStrings[i - 1] = new ArrayList<String>();
  ...
  private Instance readInstance(ResultSet rs) throws Exception {
    ...
    for (int i = 1; i <= numAttributes; i++) {
      ...
      case DatabaseConnection.STRING:
        ...
        if (rs.wasNull()) {
          ...
          if (index == null) {
            index =
              new Float(m_structure.attribute(i - 1).addStringValue(str));
          }
    ...
    if (m_CreateSparseData) {
      inst = new SparseInstance(1.0f, vals);
    } else {
      inst = new DenseInstance(1.0f, vals);
    }
    // get rid of m_idColumn
    if (m_DataBaseConnection.getUpperCase()) {
      m_idColumn = m_idColumn.toUpperCase();
