...
RevisionHandler {
  ...
  public boolean allInstanceWeightsIdentical() {
    ...
    if (numInstances() == 0) {
      ...
      float weight = instance(0).weight();
   ...
   */
  public float kthSmallestValue(Attribute att, int k) {
   ...
   * @return the kth-smallest value
   */
  public float kthSmallestValue(int attIndex, int k) {
    ...
    float[] vals = new float[numInstances()];
    for (int i = 0; i < vals.length; i++) {
      float val = instance(i).value(attIndex);
   ...
   */
  public float meanOrMode(int attIndex) {
   ...
   * @return the mean or the mode
   */
  public float meanOrMode(Attribute att) {
  ...
  public/* @pure@ */int numDistinctValues(int attIndex) {
    ...
    HashSet<Float> set = new HashSet<Float>(2 * numInstances());
    for (Instance current : this) {
      float key = current.value(attIndex);
   ...
   */
  public void setAttributeWeight(Attribute att, float weight) {
   ...
   * @param weight the new weight
   */
  public void setAttributeWeight(int att, float weight) {
   ...
   * resampleWithWeights(Random, float[], boolean[]).
   * 
   * @param random a random number generator
   * @return the new dataset
   */
  ...
  public Instances resampleWithWeights(Random random, boolean[] sampled,
    ...
    return resampleWithWeights(random, sampled, representUsingWeights, 100.0f);
  }
   ...
   */
  public Instances resampleWithWeights(Random random,
                                       boolean[] sampled, boolean representUsingWeights, float sampleSize) {
   ...
   *           length or contains negative weights.
   */
  public Instances resampleWithWeights(Random random, float[] weights) {
  ...
  public Instances resampleWithWeights(Random random, float[] weights,
    ...
    return resampleWithWeights(random, weights, sampled, representUsingWeights, 100.0f);
  }
  ...
  public Instances resampleWithWeights(Random random, float[] weights,
    ...
    Utils.normalize(P);
    float[] Q = new float[weights.length];
   ...
   */
  public/* @pure@ */float[] variances() {

    float[] vars = new float[numAttributes()];

    for (int i = 0; i < numAttributes(); i++)
      vars[i] = Float.NaN;

    float[] means = new float[numAttributes()];
    float[] sumWeights = new float[numAttributes()];

    for (int i = 0; i < numInstances(); i++) {
      float weight = instance(i).weight();
      for (int attIndex = 0; attIndex < numAttributes(); attIndex++) {
        if (attribute(attIndex).isNumeric()) {
          if (!instance(i).isMissing(attIndex)) {
            float value = instance(i).value(attIndex);

            if (Float.isNaN(vars[attIndex])) {
            ...
            float delta = weight*(value - means[attIndex]);
            sumWeights[attIndex] += weight;
   ...
   */
  public/* @pure@ */float variance(int attIndex) {
   ...
   * @throws IllegalArgumentException if the attribute is not numeric
   */
  public/* @pure@ */float variance(Attribute att) {
  ...
  public AttributeStats attributeStats(int index) {
    ...
    if (attribute(index).isNominal()) {
      ...
      result.nominalWeights = new float[attribute(index).numValues()];
    }
    ...
    HashMap<Float,float[]> map = new HashMap<Float,float[]>(2 * result.totalCount);
    for (Instance current : this) {
      float key = current.value(index);
      if (Utils.isMissingValue(key)) {
        result.missingCount++;
      } else {
   ...
   */
   // @ requires 0 <= index && index < numAttributes();
  public float[] attributeToDoubleArray(int index) {

	  float[] result = new float[numInstances()];
