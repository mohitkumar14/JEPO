...
public abstract class Optimization implements TechnicalInformationHandler,
  ...
  protected float m_ALF = 1.0e-4f;

  protected float m_BETA = 0.9f;

  protected float m_TOLX = 1.0e-6f;

  protected float m_STPMX = 100.0f;
  ...
  protected static float m_Epsilon, m_Zero;
  static {
    m_Epsilon = 1.0f;
    while (1.0 + m_Epsilon > 1.0f) {
      m_Epsilon /= 2.0f;
    }
    m_Epsilon *= 2.0f;
    m_Zero = (float) Math.sqrt(m_Epsilon);
  }
   ...
   */
  protected abstract float objectiveFunction(float[] x) throws Exception;
   ...
   * @throws Exception if something goes wrong
   */
  protected abstract float[] evaluateGradient(float[] x) throws Exception;
   ...
   * @throws Exception if something goes wrong
   */
  protected float[] evaluateHessian(float[] x, int index) throws Exception {
    return null;
  }
   ...
   */
  public float[] lnsrch(float[] xold, float[] gradient, float[] direct,
    float stpmax, boolean[] isFixed, float[][] nwsBounds,
    ...
    float alam, alamin; // lambda to be found, and its lower bound

    // For convergence and bound test
    float temp, test, alpha = Float.POSITIVE_INFINITY, fold = m_f, sum;

    // For cubic interpolation
    float a, alam2 = 0, b, disc = 0, maxalam = 1.0f, rhs1, rhs2, tmplam;

    float[] x = new float[len]; // New variable values

    // Scale the step
    for (sum = 0.0f, i = 0; i < len; i++) {
      if (!isFixed[i]) {
    ...
    for (i = 0; i < len; i++) {
      if (!isFixed[i]) {// No need for fixed variables
        temp = (float) (Math.abs(direct[i]) / Math.max(Math.abs(x[i]), 1.0));
        if (temp > test) {
          test = temp;
        }
      }
    ...
    for (i = 0; i < len; i++) {
      if (!isFixed[i]) {// No need for fixed variables
        float alpi;
        if ((direct[i] < -m_Epsilon) && !Float.isNaN(nwsBounds[0][i])) {// Not
                                                                         // feasible
          ...
          if (alpi <= m_Zero) { // Zero
            ...
            alpha = 0.0f;
            nwsBounds[0][i] = Float.NaN; // Add cons. to working set
            wsBdsIndx.addElement(i);
          } else if (alpha > alpi) { // Fix one variable in one iteration
            alpha = alpi;
            fixedOne = i;
          }
        } else if ((direct[i] > m_Epsilon) && !Float.isNaN(nwsBounds[1][i])) {// Not
                                                                               // feasible
          ...
          if (alpi <= m_Zero) { // Zero
            ...
            isFixed[i] = true; // Fix this variable
            alpha = 0.0f;
            nwsBounds[1][i] = Float.NaN; // Add cons. to working set
            wsBdsIndx.addElement(i);
          } else if (alpha > alpi) {
            alpha = alpi;
            fixedOne = i;
    ...
    float initF = fold, // Initial function value
    hi = alam, lo = alam, newSlope = 0, fhi = m_f, flo = m_f;// Variables used
                                                             // for beta
                                                             // condition
    float[] newGrad; // Gradient on the new variable values
    ...
    kloop: for (k = 0;; k++) {
      ...
      for (i = 0; i < len; i++) {
        if (!isFixed[i]) {
          ...
          if (!Float.isNaN(nwsBounds[0][i]) && (x[i] < nwsBounds[0][i])) {
            x[i] = nwsBounds[0][i]; // Rounding error
          } else if (!Float.isNaN(nwsBounds[1][i]) && (x[i] > nwsBounds[1][i])) {
      ...
      while (Float.isInfinite(m_f)) { // Avoid infinity
        if (m_Debug) {
      ...
      if (m_f <= fold + m_ALF * alam * m_Slope) {// Alpha condition: sufficient
        ...
        if (newSlope >= m_BETA * m_Slope) { // Beta condition: ensure pos.
          ...
          if ((fixedOne != -1) && (alam >= alpha)) { // Has bounds and over
            if (direct[fixedOne] > 0) {
              ...
              nwsBounds[1][fixedOne] = Float.NaN; // Add cons. to working set
            } else {
              x[fixedOne] = nwsBounds[0][fixedOne]; // Avoid rounding error
              nwsBounds[0][fixedOne] = Float.NaN; // Add cons. to working set
            }
          ...
          float upper = Math.min(alpha, maxalam);
          if (m_Debug) {
          ...
          while (!((alam >= upper) || (m_f > fold + m_ALF * alam * m_Slope))) {
            ...
            if (newSlope >= m_BETA * m_Slope) {
              ...
              if ((fixedOne != -1) && (alam >= alpha)) { // Has bounds and over
                if (direct[fixedOne] > 0) {
                  x[fixedOne] = nwsBounds[1][fixedOne]; // Avoid rounding error
                  nwsBounds[1][fixedOne] = Float.NaN; // Add cons. to working
                                                       // set
                } else {
                  x[fixedOne] = nwsBounds[0][fixedOne]; // Avoid rounding error
                  nwsBounds[0][fixedOne] = Float.NaN; // Add cons. to working
                                                       // set
                }
        ...
        if (k == 0) { // First time backtrack: quadratic interpolation
          if (!Float.isInfinite(initF)) {
            initF = m_f;
          }
          // lambda = -g'(0)/(2*g''(0))
          tmplam = -0.5f * alam * m_Slope / ((m_f - fold) / alam - m_Slope);
          ...
          b = (-alam2 * rhs1 / (alam * alam) + alam * rhs2 / (alam2 * alam2))
            / (alam - alam2);
          if (a == 0.0) {
            tmplam = -m_Slope / (2.0f * b);
          } else {
            disc = b * b - 3.0f * a * m_Slope;
            if (disc < 0.0) {
              disc = 0.0f;
            }
            float numerator = (float) (-b + Math.sqrt(disc));
            if (numerator >= Float.MAX_VALUE) {
              numerator = Float.MAX_VALUE;
              if (m_Debug) {
                System.err
                  .print("-b+sqrt(disc) too large! Set it to MAX_VALUE.");
              }
            }
            tmplam = numerator / (3.0f * a);
          }
          if (m_Debug) {
          ...
          if (tmplam > 0.5 * alam) {
            tmplam = 0.5f * alam; // lambda <= 0.5*lambda_old
          }
        }
      }
      alam2 = alam;
      fhi = m_f;
      alam = Math.max(tmplam, 0.1f * alam); // lambda >= 0.1*lambda_old
    ...
    while ((newSlope < m_BETA * m_Slope) && (ldiff >= alamin)) {
      lincr = -0.5f * newSlope * ldiff * ldiff / (fhi - flo - newSlope * ldiff);
   ...
   * 
   * @param x2 initial point of x, assuming no value's on the bound!
   * @param b2 the bound constraints of each variable constraints[0] is
   *          the lower bounds and constraints[1] is the upper bounds
   * @return the solution of x, null if number of iterations not enough
   * @throws Exception if an error occurs
   */
  public float[] findArgmin(float[] x2, float[][] b2)
    throws Exception {
    int l = x2.length;
    ...
    float[][] nwsBounds = new float[2][l];
    // Record indice of fixed variables, simply for efficiency
    DynamicIntArray wsBdsIndx = new DynamicIntArray(b2.length);
    ...
    m_f = objectiveFunction(x2);
    if (Float.isNaN(m_f)) {
      throw new Exception("Objective function value is NaN!");
    }

    float sum = 0;
    float[] grad = evaluateGradient(x2), oldGrad, oldX, deltaGrad = new float[l], deltaX = new float[l], direct = new float[l], x = new float[l];
    Matrix L = new Matrix(l, l); // Lower triangle of Cholesky factor
    float[] D = new float[l]; // Diagonal of Cholesky factor
    for (int i = 0; i < l; i++) {
      ...
      sum += grad[i] * grad[i];
      x[i] = x2[i];
      nwsBounds[0][i] = b2[0][i];
      nwsBounds[1][i] = b2[1][i];
      isFixed[i] = false;
    }
    float stpmax = (float) (m_STPMX * Math.max(Math.sqrt(sum), l));
    ...
    for (int step = 0; step < m_MAXITS; step++) {
      ...
      if (m_IsZeroStep) { // Zero step, simply delete rows/cols of D and L
        ...
        for (int h = 0; h < l; h++) {
          deltaX[h] = x[h] - oldX[h];
          float tmp = Math.abs(deltaX[h]) / Math.max(Math.abs(x[h]), 1.0f);
          if (tmp > test) {
            test = tmp;
          }
        }
        ...
        grad = evaluateGradient(x);
        test = 0.0f;
        float denom = 0.0f, dxSq = 0.0f, dgSq = 0.0f, newlyBounded = 0.0f;
        for (int g = 0; g < l; g++) {
          if (!isFixed[g]) {
          ...
          float tmp = Math.abs(grad[g]) * Math.max(Math.abs(direct[g]), 1.0f)
            / Math.max(Math.abs(m_f), 1.0f);
          if (tmp > test) {
            test = tmp;
          }
        }
        ...
        if (finish) {
          ...
          for (int m = size - 1; m >= 0; m--) {
            ...
            float[] hessian = evaluateHessian(x, index);
            float deltaL = 0.0f;
            if (hessian != null) {
            ...
            float L1, L2;
            if (x[index] >= b2[1][index]) {
              L1 = -grad[index];
            } else if (x[index] <= b2[0][index]) {
              L1 = grad[index];
            } else {
          ...
          for (int mmm = 0; mmm < toFree.size(); mmm++) {
            ...
            if (x[freeIndx] <= b2[0][freeIndx]) {// Lower bound
              nwsBounds[0][freeIndx] = b2[0][freeIndx];
              if (m_Debug) {
                System.err.println("Free variable " + freeIndx + " from bound "
                  + nwsBounds[0][freeIndx]);
              }
            } else { // Upper bound
              nwsBounds[1][freeIndx] = b2[1][freeIndx];
              if (m_Debug) {
                System.err.println("Free variable " + freeIndx + " from bound "
                  + nwsBounds[1][freeIndx]);
              }
            }
            L.set(freeIndx, freeIndx, 1.0f);
            D[freeIndx] = 1.0f;
            isUpdate = false;
          }
        }
      ...
      float[] LDIR = solveTriangle(LD, b, true, isFixed);
      LD = null;

      for (int m = 0; m < LDIR.length; m++) {
        if (Float.isNaN(LDIR[m])) {
   ...
   */
  public static float[] solveTriangle(Matrix t, float[] b, boolean isLower,
    boolean[] isZero) {
    int n = b.length;
   ...
   */
  protected void updateCholeskyFactor(Matrix L, float[] D, float[] v,
    float coeff, boolean[] isFixed) throws Exception {
    float t, p, b;
    int n = v.length;
    ...
    if (coeff > 0.0) {
      ...
      float[] P = solveTriangle(L, v, true, isFixed);
      t = 0.0f;
      for (int i = 0; i < n; i++) {
        if (!isFixed[i]) {
          t += P[i] * P[i] / D[i];
      ...
      float sqrt = 1.0f + coeff * t;
      sqrt = (float) ((sqrt < 0.0f) ? 0.0f : Math.sqrt(sqrt));

      float alpha = coeff, sigma = coeff / (1.0f + sqrt), rho, theta;
      ...
      for (int j = 0; j < n; j++) {
        ...
        t -= p;
        if (t < 0.0) {
          t = 0.0f; // Rounding error
        }

        float plus = sigma * sigma * p * t;
        ...
        float sigmaOld = sigma;
        sigma *= (1.0 + rho) / (rho * (theta + rho));
        if ((j < n - 1) && (Float.isNaN(sigma) || Float.isInfinite(sigma))) {
