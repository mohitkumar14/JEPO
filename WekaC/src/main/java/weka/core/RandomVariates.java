...
public final class RandomVariates extends Random implements RevisionHandler {
  ...
  public float nextExponential(){
    return (float) -Math.log(1.0-super.nextDouble());
  }
   ...
   */
  public float nextErlang(int a) throws Exception{
    if(a<1)
   ...
   */
  public float nextGamma(float a) throws Exception{
    if(a<=0.0)
      ...
      float b=(float) (1.0f+Math.exp(-1.0f)*a), p,x, condition;
      do{
        p=(float) (b*super.nextDouble());
        if(p<1.0){
          x = (float) Math.exp(Math.log(p)/a);
          condition = x;
        }
        else{
          x = (float) -Math.log((b-p)/a);
          condition = (float) ((1.0f-a)*Math.log(x));
        }
      }
      ...
      float b=a-1.0f, D=(float) Math.sqrt(b), D1,x1,x2,xl,f1,f2,x4,x5,xr,f4,f5,
      p1,p2,p3,p4;
      ...
      if(a<=2.0){
        ...
        x1 = x2-D1;
        xl = 1.0f-b/x1;
        f1 = (float) Math.exp(b*Math.log(x1/b)+2.0*D1);
      }

      f2=(float) Math.exp(b*Math.log(x2/b)+D1);
      x4 = b+D;
      x5 = x4+D;
      xr = 1.0f-b/x5;
      f4 = (float) Math.exp(b*Math.log(x4/b)-D);
      f5 = (float) Math.exp(b*Math.log(x5/b)-2.0*D);
      p1 = 2.0f*f4*D;
      p2 = 2.0f*f2*D1+p1;
      p3 = f5/xr+p2;
      p4 = -f1/xl+p3;
      ...
      while(Math.log(w) > (b*Math.log(x/b)+b-x) || x < 0.0){
        ...
        if(u<=p1){ // step 5-6
          ...
          w = (float) super.nextDouble();
          u = (p3-u)/(p3-p2);
          x = (float) (x5-Math.log(u)/xr);
  ...
  public static void main(String[] ops) {
    ...
    try {
      ...
      for(int i=0; i<n; i++){
        varb[i] = (float) (var.nextGaussian()*2.0f+5.0f);
        System.out.print("["+i+"] "+varb[i]+", ");
      }	  	  
      System.out.println("\nMean is "+Utils.mean(varb)+
          ", Variance is "+Utils.variance(varb)+"\n");
