...
public abstract class NormalizableDistance implements DistanceFunction,
  ...
  public void updateRangesFirst(Instance instance, int numAtt, float[][] ranges) {
    ...
    for (int j = 0; j < numVals; j++) {
      ...
      if (!instance.isMissingSparse(j)) {
        ...
        ranges[currIndex][R_MIN] = Float.POSITIVE_INFINITY;
        ranges[currIndex][R_MAX] = -Float.POSITIVE_INFINITY;
        ranges[currIndex][R_WIDTH] = Float.POSITIVE_INFINITY;
      }
    }
  }
  ...
  public void updateRanges(Instance instance, int numAtt, float[][] ranges) {
    ...
    for (int j = 0; j < numVals; j++) {
      ...
      prevIndex++;
      if (!instance.isMissingSparse(j)) {
        float val = instance.valueSparse(j);
   ...
   * @throws Exception if something goes wrong
   */
  public float[][] initializeRanges(int[] instList, int startIdx, int endIdx)
    throws Exception {
    if (m_Data == null) {
