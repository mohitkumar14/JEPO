...
public class ContingencyTables
  ...
  private static final float MAX_INT_FOR_CACHE_PLUS_ONE = 10000;
  private static final float[] INT_N_LOG_N_CACHE = new float[(int)MAX_INT_FOR_CACHE_PLUS_ONE];

  /** Initialize cache */
  static {
    for (int i = 1; i < MAX_INT_FOR_CACHE_PLUS_ONE; i++) {
      float d = (float)i;
      INT_N_LOG_N_CACHE[i] = (float) (d * Math.log(d));
    }
  }
  ...
  public static float chiSquared(float [][] matrix, boolean yates) {
   ...
   */
  public static float chiVal(float [][] matrix, boolean useYates) {
    
    int df, nrows, ncols, row, col;
    float[] rtotal, ctotal;
    float expect = 0, chival = 0, n = 0;
    boolean yates = true;
    
    nrows = matrix.length;
    ncols = matrix[0].length;
    rtotal = new float [nrows];
    ctotal = new float [ncols];
   ...
   */
  public static boolean cochransCriterion(float[][] matrix) {

    float[] rtotal, ctotal;
    float n = 0, expect, smallfreq = 5;
   ...
   * @return Cramer's V
   */
  public static float CramersV(float [][] matrix) {
    ...
    if ((min == 0) || Utils.eq(n, 0))
      return 0;
    return (float) Math.sqrt(chiVal(matrix, false) / (n * (float)min)); 
  } 
   ...
   * @return the entropy
   */
  public static float entropy(float[] pre) {

    float returnValue = 0, sum = 0;

    for (int i = 0; i < pre.length; i++) {
      returnValue -= lnFunc(pre[i]);
      sum += pre[i];
    }
    if (Utils.eq(sum, 0)) {
      return 0;
   ...
   */
  public static float entropyConditionedOnColumns(float[][] matrix) {
   ...
   */
  public static float entropyConditionedOnRows(float[][] counts) {
    
    float returnValue = 0, sumForRow, total = 0;

    for (int i = 0; i < counts.length; i++) {
      sumForRow = 0;
      for (int j = 0; j < counts[0].length; j++) {
	returnValue = returnValue + lnFunc(counts[i][j]);
	sumForRow += counts[i][j];
      }
   ...
   * @return the entropy
   */
  public static float entropyConditionedOnRows(float[][] train, 
						float[][] test,
						float numClasses) {
    
    float returnValue = 0, trainSumForRow, testSumForRow, testSum = 0;
   ...
   */
  public static float entropyOverRows(float[][] matrix) {
   ...
   */
  public static float entropyOverColumns(float[][] counts){
   ...
   */
  public static float gainRatio(float[][] counts){
    
    float preSplit = 0, postSplit = 0, splitEnt = 0,
    ...
    if (Utils.eq(splitEnt, 0))
      return 0;
    return (float) (infoGain / splitEnt);
  }
   ...
   */
  public static float log2MultipleHypergeometric(float[][] matrix) {

    float sum = 0, sumForRow, sumForColumn, total = 0;
  ...
  public static float[][] reduceMatrix(float[][] matrix) {
    ...
    newMatrix = new float[nonZeroRows][nonZeroColumns];
    currRow = 0;
    for (row = 0; row < nrows; row++) {
      if (Utils.gr(rtotal[row],0)) {
	currCol = 0;
    ...
    *
    */
  public static float symmetricalUncertainty(float[][] counts) {

    float sumForColumn, sumForRow, total = 0, columnEntropy = 0, 
    ...
    for (int i = 0; i < counts.length; i++) {
      ...
      for (int j = 0; j < counts[0].length; j++) { 
	sumForRow += counts[i][j];
	entropyConditionedOnRows += lnFunc(counts[i][j]);
      }
    ...
    if (Utils.eq(columnEntropy, 0) || Utils.eq(rowEntropy, 0))
      return 0;
    return (float) (2.0 * (infoGain / (columnEntropy + rowEntropy)));
  }
   ...
   */
  public static float tauVal(float[][] matrix) {
    
    int nrows, ncols, row, col;
    float [] ctotal;
    float maxcol = 0, max, maxtotal = 0, n = 0;
   ...
   * too close to zero 
   */
  private static float chiCell(float freq, float expected, 
                                boolean yates){
    ...
    float diff = Math.abs(freq - expected);
    if (yates) {
  ...
  public static void main(String[] ops) {
    ...
    float[] firstRow = {10, 5, 20};
    float[] secondRow = {2, 10, 6};
    float[] thirdRow = {5, 10, 10};
    float[][] matrix = new float[3][0];
