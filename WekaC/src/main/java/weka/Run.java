  ...
  public static void main(String[] args) {
    ...
    try {
        ...
        if (prunedMatches.size() == 0) {
          ...
          if (dontPromptIfMultipleMatches) {
            ...
            for (int i = 0; i < prunedMatches.size(); i++) {
              System.out.println(new StringBuilder("\t").append(i + 1).append(") ").append(prunedMatches.get(i)));
            }
      ...
      if (types.size() == 1) {
        ...
        boolean done = false;
        while (!done) {
          System.out.println(new StringBuilder(schemeToRun).append(" can be executed as any of the following:"));
          for (int i = 0; i < types.size(); i++) {
            System.out.println(new StringBuilder("\t").append(i + 1).append(") ").append(types.get(i)));
          }
          System.out.print("\nEnter a number > ");
          String choice = null;
          int typeNumber = 0;
