...
public class ArffTableModel extends DefaultTableModel implements Undoable {
   ...
   */
  public void setAttributeWeightAt(int columnIndex, float weight) {
    if (isAttribute(columnIndex)) {
      addUndoPoint();
  ...
  public void setInstanceWeight(int index, float weight) {
    setInstanceWeight(index, weight,true);
  }

  public void setInstanceWeight(int index, float weight, boolean notify) {
    if (!m_IgnoreChanges) {
      addUndoPoint();
    }
  ...
  public void insertInstance(int index, boolean notify) {
    ...
    float[] vals = new float[m_Data.numAttributes()];
    ...
    Instance toAdd = new DenseInstance(1.0f, vals);
    if (index < 0) {
      m_Data.add(toAdd);
    } else {
  ...
  public void sortInstances(int columnIndex, boolean ascending) {
    if (isAttribute(columnIndex) || (m_ShowInstanceWeights && columnIndex == 1)) {
      ...
      float[] vals = new float[m_Data.numInstances()];
  ...
  public void setValueAt(Object aValue, int rowIndex, int columnIndex, boolean notify) {
    ...
    if (m_ShowInstanceWeights && columnIndex == 1) {
      float oldWeight = m_Data.instance(rowIndex).weight();
      try {
        float newWeight = Float.parseFloat(aValue.toString());
    ...
    if (aValue == null) {
      ...
      case Attribute.NUMERIC:
        try {
          ...
          inst.setValue(index, Float.parseFloat(tmp));
        } catch (Exception e) {
          // ignore
        }
        break;
