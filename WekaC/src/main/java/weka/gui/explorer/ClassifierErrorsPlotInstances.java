...
public class ClassifierErrorsPlotInstances extends AbstractPlotInstances {
  ...
  public void process(Instances batch, float[][] predictions, Evaluation eval) {
    try {

      for (int j = 0; j < batch.numInstances(); j++) {
        Instance toPredict = batch.instance(j);
        float[] preds = predictions[j];
        float probActual = 0;
        float probNext = 0;
        ...
        if (m_PlotInstances != null) {
          float[] values = new float[m_PlotInstances.numAttributes()];
          ...
          m_PlotInstances.add(new DenseInstance(1.0f, values));
          ...
          if (toPredict.classAttribute().isNominal()) {
            ...
            if (m_pointSizeProportionalToMargin) {
              // margin
              m_PlotSizes.add(new Float(probActual - probNext));
            } else {
              int sizeAdj = 0;
            ...
            if (!toPredict.isMissing(toPredict.classIndex())
              && !Utils.isMissingValue(pred)) {
              errd = new Float(pred - toPredict.classValue());
              m_PlotShapes.add(new Integer(Plot2D.CONST_AUTOMATIC_SHAPE));
            } else {
  ...
  public void process(Instance toPredict, Classifier classifier, Evaluation eval) {
    ...
    try {
      ...
      if (m_PlotInstances != null) {
        boolean isNominal = toPredict.classAttribute().isNominal();
        values = new float[m_PlotInstances.numAttributes()];
  ...
  protected void scaleNumericPredictions() {
    ...
    maxErr = Float.NEGATIVE_INFINITY;
    minErr = Float.POSITIVE_INFINITY;
    ...
    if (m_Instances.classAttribute().isNominal()) {
      ...
      for (i = 0; i < m_PlotSizes.size(); i++) {
        errd = (Float) m_PlotSizes.get(i);
        if (errd != null) {
          err = Math.abs(errd.floatValue());
          if (err < minErr) {
            minErr = err;
          }
          if (err > maxErr) {
  ...
  protected void addPredictionIntervals() {
    ...
    for (i = 0; i < m_PlotInstances.numInstances(); i++) {
      ...
      // copy old values
      values = new float[data.numAttributes()];
      System
