...
public class AttributeVisualizationPanel extends PrintablePanel {
   ...
   */
  protected float[] m_histBarCounts;
    ...
    public void run() {
      synchronized (m_locker) {
        ...
        if ((m_classIndex >= 0) && (m_data.attribute(m_classIndex).isNominal())) {
            ...
            if (!m_data.instance(k).isMissing(m_attribIndex)) {
              ...
              if (m_data.instance(k).value(m_attribIndex) != tempAttValueIndex) {
                if (tempClassCounts != null) {
                  ...
                  int numNonZero = 0;
                  for (float tempClassCount : tempClassCounts) {
                    if (tempClassCount > 0) {
                      numNonZero++;
                    }
                  }
                  float[] nonZeroVals = new float[numNonZero];
                  ...
                  SparseInstance tempS = new SparseInstance(1.0f, nonZeroVals,
                    nonZeroIndices, tempClassCounts.length);
                  histClassCounts[tempAttValueIndex] = tempS;
                }

                tempClassCounts = new float[m_data.attribute(m_classIndex)
                  .numValues() + 1];
          ...
          float histCounts[];
          histCounts = new float[m_data.attribute(m_attribIndex).numValues()];
    ...
    public void run() {
      synchronized (m_locker) {
        if ((m_classIndex >= 0) && (m_data.attribute(m_classIndex).isNominal())) {
          ...
          float[][] histClassCounts = new float[intervals][m_data.attribute(
            m_classIndex).numValues() + 1];

          float barRange = (m_as.numericStats.max - m_as.numericStats.min)
          ...
          for (float[] histClassCount : histClassCounts) {
            double sum = 0;
          ...
          for (int i = 0; i < histClassCounts.length; i++) {
            ...
            float[] sparseValues = new float[numSparseValues];
            ...
            SparseInstance tempS = new SparseInstance(1.0f, sparseValues,
          ...
          float[] histCounts = new float[intervals];
          float barRange = (m_as.numericStats.max - m_as.numericStats.min)
            / histCounts.length;
  ...
  public String getToolTipText(MouseEvent ev) {
    ...
    if (m_as != null && m_as.nominalWeights != null) { // if current attrib is
      ...
      int x = 0, barWidth;
      float bar = m_as.numericStats.min;
      ...
      if (m_threadRun == false && m_displayCurrentAttribute) { // calculation
        ...
        if (m_as.nominalWeights != null
          ...
          // else if class attribute is numeric or not set then draw black bars.
            ...
            for (float m_histBarCount : m_histBarCounts) {
