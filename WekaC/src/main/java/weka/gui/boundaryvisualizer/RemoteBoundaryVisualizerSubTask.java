...
public class RemoteBoundaryVisualizerSubTask implements Task {
  ...
  private float m_pixHeight;
  private float m_pixWidth;
  ...
  private float m_samplesBase = 2.0f;
   ...
   */
  public void setPixHeight(float ph) {
    m_pixHeight = ph;
  }
   ...
   */
  public void setPixWidth(float pw) {
    m_pixWidth = pw;
  }
  ...
  public void execute() {
    ...
    try {
      ...
      for (int j = 0; j < m_panelWidth; j++) {
        float[] preds = calculateRegionProbs(j, m_rowNumber);
        m_result.setLocationProbs(j, preds);
        m_result
          .setPercentCompleted((int) (100 * ((float) j / (float) m_panelWidth)));
      }
    } catch (Exception ex) {
  ...
  private float getRandomX(int pix) {
    ...
    return (float) (minPix + m_random.nextDouble() * m_pixWidth);
  }
  ...
  private float getRandomY(int pix) {
    ...
    return (float) (minPix + m_random.nextDouble() * m_pixHeight);
  }
