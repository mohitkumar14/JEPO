...
public class BoundaryPanel extends JPanel {
   ...
   */
  public static final float REMOVE_POINT_RADIUS = 7.0f;
  ...
  protected float m_pixHeight;
  protected float m_pixWidth;
  ...
  protected float m_samplesBase = 2.0f;
  ...
  private final Float m_dummy = new Float(1.0);
  ...
  protected float[][][] m_probabilityCache;
  ...
  public BoundaryPanel(int panelWidth, int panelHeight) {
    ...
    m_probabilityCache = new float[m_panelHeight][m_panelWidth][];
   ...
   */
  public void setGeneratorSamplesBase(float ksb) {
    m_samplesBase = ksb;
  }
   ...
   */
  public float getGeneratorSamplesBase() {
  ...
  public void computeMinMaxAtts() {
    ...
    if (m_trainingData.numInstances() == 0) {
      ...
      for (int i = 0; i < m_trainingData.numInstances(); i++) {
        ...
        float x = inst.value(m_xAttribute);
        float y = inst.value(m_yAttribute);
    ...
    if (m_minX == Float.MAX_VALUE) {
      m_minX = 0;
    }
    if (m_minY == Float.MAX_VALUE) {
      m_minY = 0;
    }
    if (m_maxX == Float.MIN_VALUE) {
      m_maxX = 1;
    }
    if (m_maxY == Float.MIN_VALUE) {
      m_maxY = 1;
    }
   ...
   */
  private float getRandomX(int pix) {

    float minPix = m_minX + (pix * m_pixWidth);

    return minPix + m_random.nextFloat() * m_pixWidth;
  }
   ...
   * @return a value in attribute space
   */
  private float getRandomY(int pix) {

    float minPix = m_minY + (pix * m_pixHeight);

    return minPix + m_random.nextFloat() * m_pixHeight;
  }
  ...
  protected class PlotThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        // generate samples
        m_weightingAttsValues = new float[m_attsToWeightOn.length];
        m_vals = new float[m_trainingData.numAttributes()];
        m_predInst = new DenseInstance(1.0f, m_vals);
    ...
    private float[] calculateRegionProbs(int j, int i) throws Exception {
      float[] sumOfProbsForRegion = new float[m_trainingData.classAttribute()
        .numValues()];
      ...
      for (int u = 0; u < m_numOfSamplesPerRegion; u++) {
        ...
        float[] sumOfProbsForLocation = new float[m_trainingData
        ...
        float[] weights = m_dataGenerator.getWeights();
        float sumOfWeights = Utils.sum(weights);
        ...
        float sumSoFar = 0;
        float criticalMass = 0.99f * sumOfWeights;
        ...
        for (int z = 0; z < m_numOfSamplesPerGenerator; z++) {
          ...
          m_dataGenerator.setWeightingValues(m_weightingAttsValues);
          float[][] values = m_dataGenerator.generateInstances(indices);
      ...
      if ((i < m_panelHeight) && (j < m_panelWidth)) {
        m_probabilityCache[i][j] = new float[sumOfProbsForRegion.length];
   ...
   */
  private int convertToPanelX(float xval) {
    float temp = (xval - m_minX) / m_rangeX;
   ...
   */
  private int convertToPanelY(float yval) {
    float temp = (yval - m_minY) / m_rangeY;
   ...
   */
  private float convertFromPanelX(float pX) {
    pX /= m_panelWidth;
    pX *= m_rangeX;
    return pX + m_minX;
  }
   ...
   */
  private float convertFromPanelY(float pY) {
    pY = m_panelHeight - pY;
    pY /= m_panelHeight;
    pY *= m_rangeY;
   ...
   */
  protected void plotPoint(int x, int y, float[] probs, boolean update) {
   ...
   * Plot a point in our visualization on-screen.
   */
  private void plotPoint(int x, int y, int width, int height, float[] probs,
    boolean update) {
  ...
  public void addTrainingInstanceFromMouseLocation(int mouseX, int mouseY,
    int classAttIndex, float classValue) {
    // convert to coordinates in the training instance space.
    float x = convertFromPanelX(mouseX);
    float y = convertFromPanelY(mouseY);
  ...
  public void removeTrainingInstanceFromMouseLocation(int mouseX, int mouseY) {
    ...
    int bestIndex = -1;
    float bestDistanceBetween = Integer.MAX_VALUE;

    // find the closest point.
    for (int i = 0; i < m_trainingData.numInstances(); i++) {
      Instance current = m_trainingData.instance(i);
      float distanceBetween = (current.value(m_xAttribute) - x)
    ...
    float panelDistance = (convertToPanelX(best.value(m_xAttribute)) - mouseX)
