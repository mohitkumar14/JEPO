...
public class KDDataGenerator implements DataGenerator, Serializable {
  ...
  private float[] m_globalMeansOrModes;
  ...
  private final float m_laplaceConst = 1.0f;
   ...
   */
  private float[] m_weightingValues;

  private static float m_normConst = (float) Math.sqrt(2 * Math.PI);
  ...
  public void buildGenerator(Instances inputInstances) throws Exception {
    ...
    m_globalMeansOrModes = new float[m_instances.numAttributes()];
    ...
    m_kernelParams = new float[m_instances.numInstances()][m_instances
      .numAttributes()];
    computeParams();
  }
  ...
  public float[] getWeights() {
    ...
    float[] weights = new float[m_instances.numInstances()];
   ...
   */
  private float[] computeCumulativeDistribution(float[] dist) {

    float[] cumDist = new float[dist.length];
    float sum = 0;
   ...
   */
  @Override
  public float[][] generateInstances(int[] indices) throws Exception {

    float[][] values = new float[m_instances.numInstances()][];

    for (int k = 0; k < indices.length; k++) {
      values[indices[k]] = new float[m_instances.numAttributes()];
      for (int i = 0; i < m_instances.numAttributes(); i++) {
        if ((!m_weightingDimensions[i]) && (i != m_instances.classIndex())) {
          if (m_instances.attribute(i).isNumeric()) {
            float mean = 0;
            float val = (float) m_random.nextGaussian();
            ...
            // nominal attribute
            float[] dist = new float[m_instances.attribute(i).numValues()];
            ...
            float[] cumDist = computeCumulativeDistribution(dist);
            float randomVal = m_random.nextFloat();
            int instVal = 0;
  ...
  private float normalDens(float x, float mean, float stdDev) {
    ...
    return (float) ((1 / (m_normConst * stdDev))
   ...
   */
  @Override
  public void setWeightingValues(float[] vals) {
  ...
  private float distance(Instance first, Instance second) {
    ...
    float diff, distance = 0;

    for (int i = 0; i < m_instances.numAttributes(); i++) {
      if (i == m_instances.classIndex()) {
        continue;
      }
      float firstVal = m_globalMeansOrModes[i];
      float secondVal = m_globalMeansOrModes[i];
  ...
  private void computeParams() throws Exception {
    ...
    m_Min = new float[m_instances.numAttributes()];
    m_Max = new float[m_instances.numAttributes()];
    for (int i = 0; i < m_instances.numAttributes(); i++) {
      m_Min[i] = m_Max[i] = Float.NaN;
    }
    for (int i = 0; i < m_instances.numInstances(); i++) {
      updateMinMax(m_instances.instance(i));
    }

    float[] distances = new float[m_instances.numInstances()];
    for (int i = 0; i < m_instances.numInstances(); i++) {
      ...
      float bandwidth = distances[sorted[k]];
