...
public class IncrementalClassifierEvaluator extends AbstractEvaluator implements
  ...
  private float[] m_dataPoint = new float[1];
  private boolean m_reset = false;

  private float m_min = Float.MAX_VALUE;
  private float m_max = Float.MIN_VALUE;
  ...
  private LinkedList<float[]> m_windowedPreds;
    ...
    try {
      if (ce.getStatus() == IncrementalClassifierEvent.NEW_BATCH) {
        ...
        if (m_windowSize > 0) {
          ...
          m_windowedPreds = new LinkedList<float[]>();
        ...
        if (inst != null) {
          ...
          float[] dist = ce.getClassifier().distributionForInstance(inst);
          float pred = 0;
          if (!inst.isMissing(inst.classIndex())) {
            ...
            if (m_windowSize > 0) {
              ...
              if (m_instanceCount > m_windowSize) {
                ...
                float[] oldDist = m_windowedPreds.removeLast();
            ...
            if (inst.attribute(inst.classIndex()).isNominal()) {
              if (!inst.isMissing(inst.classIndex())) {
                ...
                if (m_dataPoint.length < 1) {
                  m_dataPoint = new float[1];
                  m_dataLegend.addElement("Confidence");
                }
              }
              float primaryMeasure = 0;
              if (!inst.isMissing(inst.classIndex())) {
                if (m_windowSize > 0) {
                  primaryMeasure = 1.0f - m_windowEval.errorRate();
                } else {
                  primaryMeasure = 1.0f - m_eval.errorRate();
                }
              } else {
