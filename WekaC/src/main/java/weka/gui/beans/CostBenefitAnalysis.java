...
public class CostBenefitAnalysis extends JPanel implements BeanCommon,
  ...
  protected static class AnalysisPanel extends JPanel {
    ...
    protected float m_tpPrevious;
    protected float m_fpPrevious;
    protected float m_tnPrevious;
    protected float m_fnPrevious;
    ...
    protected static class ConfusionCell extends JPanel {
       ...
       */
      public void setCellValue(float cellValue, float max,
        float scaleFactor, int precision) {
        ...
        m_conf_perc.setText(Utils.doubleToString(m_percentage * 100.0f,
          precision) + "%");
    ...
    public AnalysisPanel() {
      ...
      m_thresholdSlider.addChangeListener(new ChangeListener() {
        ...
        public void stateChanged(ChangeEvent e) {
          updateInfoForSliderValue(m_thresholdSlider.getValue() / 100.0f);
        }
      });
      ...
      ActionListener rl = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          float gain = Float.parseFloat(m_gainV.getText());
          gain = -gain;
    ...
    private void findMaxMinCB(boolean max) {
      float maxMin = (max) ? Float.NEGATIVE_INFINITY
      ...
      float valueOfMetric = m_masterPlot.getPlotInstances()
    ...
    private void updateCostBenefit() {
      float value = m_thresholdSlider.getValue() / 100.0f;
    ...
    private void updateCBRandomGainInfo(int index) {
      float requestedPopSize = m_originalPopSize;
      try {
        requestedPopSize = Float.parseFloat(m_totalPopField.getText());
      } catch (NumberFormatException e) {
      }
      float scaleFactor = requestedPopSize / m_originalPopSize;

      float CB = m_costBenefit.getPlotInstances().instance(index).value(1);
      m_costBenefitV.setText(Utils.doubleToString(CB, 2));

      float totalRandomCB = 0.0f;
      Instance first = m_masterPlot.getPlotInstances().instance(0);
      float totalPos = first.value(m_masterPlot.getPlotInstances()
        .attribute(ThresholdCurve.TRUE_POS_NAME).index())
        * scaleFactor;
      float totalNeg = first.value(m_masterPlot.getPlotInstances().attribute(
        ThresholdCurve.FALSE_POS_NAME))
        * scaleFactor;

      float posInSample = (totalPos * (Float.parseFloat(m_percPopLab
        .getText()) / 100.0f));
      float negInSample = (totalNeg * (Float.parseFloat(m_percPopLab
        .getText()) / 100.0f));
      float posOutSample = totalPos - posInSample;
      float negOutSample = totalNeg - negInSample;

      float tpCost = 0.0f;
      try {
        tpCost = Float.parseFloat(m_cost_aa.getText());
      } catch (NumberFormatException n) {
      }
      float fpCost = 0.0f;
      try {
        fpCost = Float.parseFloat(m_cost_ba.getText());
      } catch (NumberFormatException n) {
      }
      float tnCost = 0.0f;
      try {
        tnCost = Float.parseFloat(m_cost_bb.getText());
      } catch (NumberFormatException n) {
      }
      float fnCost = 0.0f;
      try {
        fnCost = Float.parseFloat(m_cost_ab.getText());
      } catch (NumberFormatException n) {
      }
    ...
    private void updateInfoGivenIndex(int index) {
      ...
      m_percPopLab.setText(Utils.doubleToString(
        100.0f * plotInstances.instance(index).value(indexOfSampleSize), 4));
      m_percOfTargetLab.setText(Utils.doubleToString(100.0f * plotInstances
      ...
      float totalInstances = temp.value(tp) + temp.value(fp) + temp.value(tn)
        + temp.value(fn);
      // get the value out of the total pop field (if possible)
      float requestedPopSize = totalInstances;
      try {
        requestedPopSize = Float.parseFloat(m_totalPopField.getText());
      } catch (NumberFormatException e) {
      }
    ...
    private void updateInfoForSliderValue(float value) {
      int indexOfSampleSize = m_masterPlot.getPlotInstances()
        .attribute(ThresholdCurve.SAMPLE_SIZE_NAME).index();
      int indexOfPercOfTarget = m_masterPlot.getPlotInstances()
        .attribute(ThresholdCurve.RECALL_NAME).index();
    ...
    private int findIndexForValue(float value, Instances plotInstances,
      int indexOfMetric) {
      // binary search
      ...
      while (!done) {
        if (upper - lower <= 1) {
          ...
          float comp1 = plotInstances.instance(upper).value(indexOfMetric);
          float comp2 = plotInstances.instance(lower).value(indexOfMetric);
        ...
        float comparisonVal = plotInstances.instance(mid).value(indexOfMetric);
    ...
    private boolean constructCostBenefitData() {
      ...
      for (int i = 0; i < performanceI.numInstances(); i++) {
        ...
        Instance newInst = new DenseInstance(1.0f, vals);
