...
public class StripChart extends JPanel implements ChartListener,
  ...
  private float m_oldMax;
  private float m_oldMin;
   ...
   */
  private LinkedList<float[]> m_dataList = new LinkedList<float[]>();
    ...
    if (m_outputFrame == null) {
      ...
      m_outputFrame.addWindowListener(new java.awt.event.WindowAdapter() {
        ...
        public void windowClosing(java.awt.event.WindowEvent e) {
          ...
          synchronized (m_dataList) {
            m_dataList = new LinkedList<float[]>();
          }
   ...
   */
  protected void updateChart(float[] dataPoint) {
    if (m_previousY[0] == -1) {
    ...
    if (m_yScaleUpdate) {
      ...
      String midVal = numToString((m_oldMax - m_oldMin) / 2.0f);
      if (m_labelMetrics == null) {
