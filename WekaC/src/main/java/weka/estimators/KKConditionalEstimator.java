...
public class KKConditionalEstimator implements ConditionalEstimator {
   ...
   */
  private int findNearestPair(float key, float secondaryKey) {
    ...
    while (low < high) {
      middle = (low + high) / 2;
      float current = m_CondValues[middle];
      if (current == key) {
	float secondary = m_Values[middle];
   ...
   */
  public KKConditionalEstimator(float precision) {

    m_CondValues = new float [50];
   ...
   */
  public void addValue(float data, float given, float weight) {
	...
	float [] newCondValues = new float [m_Values.length*2];
    ...
    float range = m_CondValues[m_NumValues-1] - m_CondValues[0];
   ...
   */
  public Estimator getEstimator(float given) {
   ...
   */
  public float getProbability(float data, float given) {
  ...
  public static void main(String [] argv) {
	...
	seed = Integer.parseInt(argv[0]);
      ...
      KKConditionalEstimator newEst = new KKConditionalEstimator(0.1f);
