...
public class KernelEstimator extends Estimator implements IncrementalEstimator,
  ...
  private float m_SumOfWeights;

  /** The standard deviation */
  private float m_StandardDev;
  ...
  private static float MAX_ERROR = 0.01f;
   ...
   */
  private int findNearestValue(float key) {
   ...
   */
  private float round(float data) {

    return (float) (Math.rint(data / m_Precision) * m_Precision);
  }
   ...
   */
  public KernelEstimator(float precision) {

    m_Values = new float[50];
    m_Weights = new float[50];
    m_NumValues = 0;
    m_SumOfWeights = 0;
  ...
  public void addValue(float data, float weight) {
    ...
    if ((m_NumValues <= insertIndex) || (m_Values[insertIndex] != data)) {
      if (m_NumValues < m_Values.length) {
        ...
        float[] newValues = new float[m_Values.length * 2];
        float[] newWeights = new float[m_Values.length * 2];
        int left = m_NumValues - insertIndex;
    ...
    float range = m_Values[m_NumValues - 1] - m_Values[0];
    if (range > 0) {
      m_StandardDev = (float) Math.max(range / Math.sqrt(m_SumOfWeights),
  ...
  public static void testAggregation() {
    KernelEstimator ke = new KernelEstimator(0.01f);
    KernelEstimator one = new KernelEstimator(0.01f);
    KernelEstimator two = new KernelEstimator(0.01f);
  ...
  public static void main(String[] argv) {
    ...
    try {
      ...
      KernelEstimator newEst = new KernelEstimator(0.01f);
      for (int i = 0; i < argv.length - 3; i += 2) {
        newEst.addValue(Float.valueOf(argv[i]).floatValue(),
            Float.valueOf(argv[i + 1]).floatValue());
      }
      System.out.println(newEst);

      float start = Float.valueOf(argv[argv.length - 2]).floatValue();
      float finish = Float.valueOf(argv[argv.length - 1]).floatValue();
      for (float current = start; current < finish; current += (finish - start) / 50) {
