...
public class PoissonEstimator
   ...
   */
  private float Poisson(float x) {
    
    return (float) Math.exp(-m_Lambda + (x * Math.log(m_Lambda)) - logFac(x));
  }
  ...
  public static void main(String [] argv) {
    ...
    try {
      ...
      for(int i = 0; i < argv.length; i++) {
        float current = Double.valueOf(argv[i]).floatValue();
