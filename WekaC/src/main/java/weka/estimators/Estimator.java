...
public abstract class Estimator implements Cloneable, Serializable,
   ...
   * only
   */
  protected float m_classValueIndex = -1.0f;
  ...
  public void addValues(Instances data, int attrIndex) throws Exception {
    ...
    addValues(data, attrIndex, min, max, 1.0f);
  }
   ...
   * @exception Exception if building of estimator goes wrong
   */
  public void addValues(Instances data, int attrIndex, float min, float max,
    ...
    for (int i = 1; i < numInst; i++) {
      addValue(data.instance(i).value(attrIndex), 1.0f);
    }
  }
  ...
  public void addValues(Instances data, int attrIndex, int classIndex,
    ...
    float factor = getInstancesFromClass(data, attrIndex, classIndex,
      classValue, workData);
   ...
   */
  public void addValues(Instances data, int attrIndex, int classIndex,
    int classValue, float min, float max) throws Exception {
   ...
   */
  private float getInstancesFromClass(Instances data, int attrIndex,
    int classIndex, float classValue, Instances workData) {
    ...
    Float alphaFactor = new Float((float) numClassValue / (float) num);
    return alphaFactor;
  }
   ...
   */
  public abstract float getProbability(float data);
