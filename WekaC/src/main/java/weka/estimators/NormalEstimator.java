...
public class NormalEstimator extends Estimator implements IncrementalEstimator,
   ...
   */
  public NormalEstimator(float precision) {
  ...
  protected void computeParameters() {
    if (m_SumOfWeights > 0) {
      ...
      float stdDev = (float) Math.sqrt(Math.abs(m_SumOfValuesSq - m_Mean
          * m_SumOfValues)
          / m_SumOfWeights);
  ...
  public float getProbability(float data) {
    ...
    data = round(data);
    float zLower = (data - m_Mean - (m_Precision / 2)) / m_StandardDev;
    float zUpper = (data - m_Mean + (m_Precision / 2)) / m_StandardDev;

    float pLower = Statistics.normalProbability(zLower);
    float pUpper = Statistics.normalProbability(zUpper);
  ...
  public static void testAggregation() {
    NormalEstimator ne = new NormalEstimator(0.01f);
    NormalEstimator one = new NormalEstimator(0.01f);
    NormalEstimator two = new NormalEstimator(0.01f);
  ...
  public static void main(String[] argv) {
    ...
    try {
      ...
      NormalEstimator newEst = new NormalEstimator(0.01f);
      for (int i = 0; i < argv.length; i++) {
        float current = Float.valueOf(argv[i]).floatValue();
