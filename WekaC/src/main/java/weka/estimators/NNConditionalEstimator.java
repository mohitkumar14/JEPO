...
public class NNConditionalEstimator implements ConditionalEstimator {
  ...
  private int findNearestPair(float key, float secondaryKey) {
    ...
    while (low < high) {
      middle = (low + high) / 2;
      float current = m_CondValues.elementAt(middle).floatValue();
      if (current == key) {
        float secondary = m_Values.elementAt(middle).floatValue();
  ...
  private void calculateCovariance() {
    ...
    float sumValues = 0, sumConds = 0;
    for (int i = 0; i < m_Values.size(); i++) {
      sumValues += m_Values.elementAt(i).floatValue()
        * m_Weights.elementAt(i).floatValue();
      sumConds += m_CondValues.elementAt(i).floatValue()
        * m_Weights.elementAt(i).floatValue();
    }
    ...
    for (int i = 0; i < m_Values.size(); i++) {
      float x = m_Values.elementAt(i).floatValue();
      float y = m_CondValues.elementAt(i).floatValue();
      float weight = m_Weights.elementAt(i).floatValue();
  ...
  public void addValue(float data, float given, float weight) {
    ...
    if ((m_Values.size() <= insertIndex)
      || (m_CondValues.elementAt(insertIndex).floatValue() != given)
      || (m_Values.elementAt(insertIndex).floatValue() != data)) {
      ...
      float newWeight = m_Weights.elementAt(insertIndex).floatValue();
      newWeight += weight;
