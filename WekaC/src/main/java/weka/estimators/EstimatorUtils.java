...
public class EstimatorUtils implements RevisionHandler {
   ...
   */
  public static float findMinDistance(Instances inst, int attrIndex) {
    ...
    float secondValue = inst.instance(begin).value(attrIndex);
   ...
   */
  public static int getMinMax(Instances inst, int attrIndex, float[] minMax)
    throws Exception {
    float min = Float.NaN;
    float max = Float.NaN;
    Instance instance = null;
  ...
  public static Vector<Object> getInstancesFromClass(Instances data,
    int attrIndex, int classIndex, float classValue, Instances workData) {
    ...
    Double alphaFactor = new Double((float) numClassValue / (float) num);
   ...
   */
  public static void writeCurve(String f, Estimator est, float min,
    float max, int numPoints) throws Exception {
    ...
    float diff = (max - min) / (numPoints - 1.0f);
    try {
      text.append("" + min + " " + est.getProbability(min) + " \n");

      for (float value = min + diff; value < max; value += diff) {
  ...
  public static void writeCurve(String f, Estimator est, Estimator classEst,
    float classIndex, float min, float max, int numPoints) throws Exception {
   ...
   */
  public static String cutpointsToString(float[] cutPoints,
    boolean[] cutAndLeft) {
    StringBuffer text = new StringBuffer("");
    if (cutPoints == null) {
