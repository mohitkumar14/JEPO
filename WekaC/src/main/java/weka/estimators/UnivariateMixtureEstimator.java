...
Serializable {
  ...
  public class MM {
    ...
    /** Means */
    protected float[] m_Means = null;

    /** Standard deviations */
    protected float[] m_StdDevs = null;

    /** The priors, on log scale */
    protected float[] m_LogPriors = null;
     ...
     */
    protected float smallestDistance(float val) {

      float min = Math.abs(val - m_Means[0]);
      for (int i = 1; i < m_K; i++) {
     ...
     * Assumes m_Means has at least one element (i.e. m_K >= 1).
     */
    protected int nearestMean(float val) {
     ...
     */
    public void initializeModel(int K, float[] values, float[] weights, Random r) {

      // Initialize means using farthest points
      m_Means = new float[K];

      // Randomly choose first point
      float furthestVal = values[r.nextInt(values.length)];
      ...
      do {
        ...
        float maxMinDist = smallestDistance(values[0]);
        furthestVal = values[0];
        for (int i = 1; i < values.length; i++) {
          float minDist = smallestDistance(values[i]);
      ...
      if (m_K < K) {
        float[] tempMeans = new float[m_K];
        System.arraycopy(m_Means, 0, tempMeans, 0, m_K);
        m_Means = tempMeans;
      }

      // Establish initial cluster assignments
      float[][] probs = new float[m_K][values.length];
      for (int i = 0; i < values.length; i++) {
        probs[nearestMean(values[i])][i] = 1.0f;
      }

      // Compute initial parameters
      m_StdDevs = new float[m_K];
      m_LogPriors = new float[m_K];
     ...
     * Estimate parameters.
     */
    protected void estimateParameters(float[] values, float[] weights, float[][] probs) {

      float totalSumOfWeights = 0;
      for (int j = 0; j < m_K; j++) {
        float sum = 0;
        float sumWeights = 0;
        for (int i = 0; i < values.length; i++) {
          float weight = probs[j][i] * weights[i];
          sum += weight * values[i];
          sumWeights += weight;
        }
        if (sumWeights <= 0) {
      ...
      for (int j = 0; j < m_K; j++) {
        ...
        for (int i = 0; i < values.length; i++) {
          float weight = probs[j][i] * weights[i];
          float diff = values[i] - m_Means[j];
          sum += weight * diff * diff;
          sumWeights += weight;
        }
        if ((sum <= 0) || (sumWeights <= 0)) {
          m_StdDevs[j] = 1.0e-6f; // Hack to prevent unpleasantness
        } else {
          m_StdDevs[j] = (float) Math.sqrt(sum / sumWeights);
          if (m_StdDevs[j] < 1.0e-6) {
            m_StdDevs[j] = 1.0e-6f;
          }
        }
        if (sumWeights <= 0) {
          m_LogPriors[j] = -Float.MAX_VALUE;
        } else {
          m_LogPriors[j] = (float) Math.log(sumWeights / totalSumOfWeights);
        }
      }
    }
     ...
     */
    public float loglikelihood(float[] values, float[] weights) {
     ...
     */
    protected float logNormalDens(float x, float mean, float stdDev) {

      float diff = x - mean;
      return (float) (-(diff * diff / (2 * stdDev * stdDev)) - m_normConst - Math.log(stdDev));
    }

    /**
     * Joint densities per cluster.
     */
    protected float[] logJointDensities(float value) {

      float[] a = new float[m_K];
      for (int i = 0; i < m_K; i++) {
    ...
    public float logDensity(float value) {
      ...
      float[] a = logJointDensities(value);
      float max = a[Utils.maxIndex(a)];
      float sum = 0.0f;
    ...
    public float[][] predictIntervals(float conf) {
      ...
      for (int i = 0; i < m_Means.length; i++) {
        float l = m_Means[i] - val * m_StdDevs[i];
        if (l < min) {
          min = l;
        }
        float r = m_Means[i] + val * m_StdDevs[i];
        if (r > max) {
          max = r;
        }
      }
    ...
    public float predictQuantile(float percentage) {
      ...
      // Compute minimum and maximum value, and delta
      float valRight = Statistics.normalInverse(percentage);
      float valLeft = Statistics.normalInverse(0.001f);
      float min = Float.MAX_VALUE;
      float max = -Float.MAX_VALUE;
      for (int i = 0; i < m_Means.length; i++) {
        float l = m_Means[i] - valLeft * m_StdDevs[i];
        if (l < min) {
          min = l;
        }
        float r = m_Means[i] + valRight * m_StdDevs[i];
        if (r > max) {
          max = r;
        }
      }
  ...
  protected float[] m_Values = new float[1000];

  /** The weights used for this estimator */
  protected float[] m_Weights = new float[1000];
  ...
  public void addValue(float value, float weight) {
    ...
    if (!Utils.eq(weight, 0)) {
      ...
      if (m_NumValues == m_Values.length) {
        float[] newWeights = new float[2 * m_NumValues];
        float[] newValues = new float[2 * m_NumValues];
   ...
   * and values.length = weights.length.
   */
  public MM buildModel(int K, float[] values, float[] weights) {
    ...
    while (numAttempts < 5) {
      ...
      float oldMSE = Float.MAX_VALUE;
      float MSE = tempModel.MSE();

      if (m_Debug) {
        System.err.println("MSE: " + MSE);
      }

      float[][] probs = new float[tempModel.m_K][values.length];
      while (Utils.sm(MSE, oldMSE)){
        ...
        for (int i = 0; i < values.length; i++) {
          probs[tempModel.nearestMean(values[i])][i] = 1.0f;
        }
    ...
    float oldLogLikelihood = -Float.MAX_VALUE;
    float loglikelihood = model.loglikelihood(values, weights);
    float[][] probs = new float[model.m_K][values.length];
    while (Utils.gr(loglikelihood, oldLogLikelihood)){

      // Establish membership probabilities
      for (int i = 0; i < values.length; i++) {
        float[] p = Utils.logs2probs(model.logJointDensities(values[i]));
   ...
   */
  public float[][] resampleWithWeights(Random random, boolean[] sampled) {

    // Walker's method, see pp. 232 of "Stochastic Simulation" by B.D. Ripley
    float[] P = new float[m_Weights.length];
    System.arraycopy(m_Weights, 0, P, 0, m_Weights.length);
    Utils.normalize(P);
    float[] Q = new float[m_Weights.length];
  ...
  protected int findNumComponentsUsingBootStrap() {
    ...
    float bestLogLikelihood = -Float.MAX_VALUE;
    ...
    for (int i = 1; i <= m_MaxNumComponents; i++) {
      ...
      for (int k = 0; k < m_NumBootstrapRuns; k++) {
        ...
        float[][] output = resampleWithWeights(m_Random, inBag);
        MM mixtureModel = buildModel(i, output[0], output[1]);
        float locLogLikelihood = 0;
        float totalWeight = 0;
      ...
      logLikelihood /= (float)m_NumBootstrapRuns;
      if (m_Debug) {
   ...
   */
  protected float entropy(MM mixtureModel) {
    ...
    return entropy / (float)m_NumValues;
  }
  ...
  protected MM findModelUsingNormalizedEntropy() {
    ...
    float loglikelihoodForOneCluster = bestMixtureModel.loglikelihood(m_Values, m_Weights);
    float bestNormalizedEntropy = 1;
    for (int i = 2; i <= m_MaxNumComponents; i++) {
      MM mixtureModel = buildModel(i, m_Values, m_Weights);

      float loglikelihood = mixtureModel.loglikelihood(m_Values, m_Weights);
      ...
      float entropy = entropy(mixtureModel);
      float normalizedEntropy = entropy / (loglikelihood - loglikelihoodForOneCluster);
  ...
  protected void updateModel() {
    ...
    if (m_MixtureModel != null) {
      ...
      if (m_Values.length > m_NumValues) {
        float[] values = new float[m_NumValues];
        float[] weights = new float[m_NumValues];
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < 100000; i++) {
      e.addValue((float) (r.nextGaussian() * 0.5 - 1), 1);
      e.addValue((float) (r.nextGaussian() * 0.5 + 1), 3);
    }
    ...
    // Output quantile
    System.out.println("95% quantile: " + e.predictQuantile(0.95f));
  }
}