...
public class Normalize extends PotentialClassIgnorer implements
  ...
  protected float[] m_MinArray;

  /** The maximum values for numeric attributes. */
  protected float[] m_MaxArray;

  /** The translation of the output range. */
  protected float m_Translation = 0;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setScale(Float.parseFloat(tmpStr));
    } else {
      setScale(1.0f);
    }
    ...
    if (tmpStr.length() != 0) {
      setTranslation(Float.parseFloat(tmpStr));
    } else {
      setTranslation(0.0f);
    }
  ...
  public boolean batchFinished() throws Exception {
    ...
    if (m_MinArray == null) {
      ...
      m_MinArray = new float[input.numAttributes()];
      m_MaxArray = new float[input.numAttributes()];
      for (int i = 0; i < input.numAttributes(); i++) {
        m_MinArray[i] = Float.NaN;
      }

      for (int j = 0; j < input.numInstances(); j++) {
        float[] value = input.instance(j).toDoubleArray();
        for (int i = 0; i < input.numAttributes(); i++) {
  ...
  protected void convertInstance(Instance instance) throws Exception {
    ...
    if (instance instanceof SparseInstance) {
      ...
      float[] vals = instance.toDoubleArray();
      int ind = 0;
