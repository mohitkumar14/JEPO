...
public class Discretize extends PotentialClassIgnorer implements
  ...
  protected float m_DesiredWeightOfInstancesPerInterval = -1;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (weight.length() != 0) {
      setDesiredWeightOfInstancesPerInterval((new Double(weight)).floatValue());
    } else {
   ...
   */
  public float getDesiredWeightOfInstancesPerInterval() {
   ...
   */
  public void setDesiredWeightOfInstancesPerInterval(float newDesiredNumber) {
  ...
  protected void calculateCutPointsByEqualWidthBinning(int index) {
    ...
    float[] cutPoints = null;
    if ((m_NumBins > 1) && (binWidth > 0)) {
      cutPoints = new float[m_NumBins - 1];
  ...
  protected void calculateCutPointsByEqualFrequencyBinning(int index) {
    ...
    if (getDesiredWeightOfInstancesPerInterval() > 0) {
      ...
      cutPoints = new float[(int) (sumOfWeights / freq)];
    } else {
  ...
  protected void findNumBins(int index) {
    ...
    float min = Float.MAX_VALUE, max = -Float.MAX_VALUE, binWidth = 0, entropy, bestEntropy = Float.MAX_VALUE, currentVal;
    float[] distribution;
    int bestNumBins = 1;
    Instance currentInstance;
    ...
    for (int i = 0; i < m_NumBins; i++) {
      ...
      for (int j = 0; j < getInputFormat().numInstances(); j++) {
        ...
        if (!currentInstance.isMissing(index)) {
          for (int k = 0; k < i + 1; k++) {
            if (currentInstance.value(index) <= (min + (((float) k + 1) * binWidth))) {
    ...
    if ((bestNumBins > 1) && (binWidth > 0)) {
      cutPoints = new float[bestNumBins - 1];
