...
public class NumericCleaner extends SimpleStreamFilter implements WeightedAttributesHandler, WeightedInstancesHandler {
  ...
  protected float m_MinDefault = -Float.MAX_VALUE;

  /** the maximum threshold */
  protected float m_MaxThreshold = Float.MAX_VALUE;

  /** the maximum default replacement value */
  protected float m_MaxDefault = Float.MAX_VALUE;
  ...
  protected float m_CloseToDefault = 0;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setMinThreshold(Float.parseFloat(tmpStr));
    } else {
    ...
    if (tmpStr.length() != 0) {
      setMinDefault(Float.parseFloat(tmpStr));
    } else {
      setMinDefault(-Float.MAX_VALUE);
    }

    tmpStr = Utils.getOption("max", options);
    if (tmpStr.length() != 0) {
      setMaxThreshold(Float.parseFloat(tmpStr));
    } else {
      setMaxThreshold(Float.MAX_VALUE);
    }

    tmpStr = Utils.getOption("max-default", options);
    if (tmpStr.length() != 0) {
      setMaxDefault(Float.parseFloat(tmpStr));
    } else {
      setMaxDefault(Float.MAX_VALUE);
    }
    ...
    if (tmpStr.length() != 0) {
      setCloseToDefault(Float.parseFloat(tmpStr));
    } else {
      setCloseToDefault(0);
    }
  ...
  protected Instance process(Instance instance) throws Exception {
    ...
    float[] result = new float[instance.numAttributes()];

    if (m_Decimals > -1) {
      factor = (float) StrictMath.pow(10, m_Decimals);
    } else {
      factor = 1;
    }
    ...
    for (i = 0; i < instance.numAttributes(); i++) {
      ...
      if (m_Decimals > -1 && !Utils.isMissingValue(result[i])) {
        val = result[i];
        val = (float) (StrictMath.round(val * factor) / factor);
        result[i] = val;
      }
    }
   ...
   */
  public void setMinDefault(float value) {
    m_MinDefault = value;
  }
   ...
   */
  public void setMaxDefault(float value) {
    m_MaxDefault = value;
  }
   ...
   */
  public void setCloseToDefault(float value) {
