...
public class ReplaceMissingValues extends PotentialClassIgnorer implements
  ...
  /** The modes and means */
  private float[] m_ModesAndMeans = null;
  ...
  public boolean batchFinished() {
    ...
    if (m_ModesAndMeans == null) {
      ...
      float sumOfWeights = getInputFormat().sumOfWeights();
      float[][] counts = new float[getInputFormat().numAttributes()][];
      for (int i = 0; i < getInputFormat().numAttributes(); i++) {
        if (getInputFormat().attribute(i).isNominal()) {
          counts[i] = new float[getInputFormat().attribute(i).numValues()];
          if (counts[i].length > 0)
            counts[i][0] = sumOfWeights;
        }
      }
      float[] sums = new float[getInputFormat().numAttributes()];
      for (int i = 0; i < sums.length; i++) {
        sums[i] = sumOfWeights;
      }
      float[] results = new float[getInputFormat().numAttributes()];
      ...
      m_ModesAndMeans = new float[getInputFormat().numAttributes()];
      for (int i = 0; i < getInputFormat().numAttributes(); i++) {
        if (getInputFormat().attribute(i).isNominal()) {
          if (counts[i].length == 0)
            m_ModesAndMeans[i] = Utils.missingValue();
          else
            m_ModesAndMeans[i] = (float) Utils.maxIndex(counts[i]);
  ...
  private void convertInstance(Instance instance) {
    ...
    if (hasMissing) {
      if (instance instanceof SparseInstance) {
        float[] vals = new float[instance.numValues()];
        ...
        float[] vals = new float[getInputFormat().numAttributes()];
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 14796 $");
  }
