...
public class PrincipalComponents extends Filter implements OptionHandler,
   ...
   * data.
   */
  protected float[][] m_Eigenvectors;

  /** Eigenvalues for the corresponding eigenvectors. */
  protected float[] m_Eigenvalues = null;

  /** Sorted eigenvalues. */
  protected int[] m_SortedEigens;

  /** sum of the eigenvalues. */
  protected float m_SumOfEigenValues = 0.0;
   ...
   * best n PC's.
   */
  protected float m_CoverVariance = 0.95f;
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      setVarianceCovered(Float.parseFloat(tmpStr));
    } else {
      setVarianceCovered(0.95f);
    }
   ...
   */
  public void setVarianceCovered(float value) {
  ...
  protected Instances determineOutputFormat(Instances inputFormat)
    ...
    for (i = m_NumAttribs - 1; i >= numAttsLowerBound; i--) {
      ...
      coeff_mags = new float[m_NumAttribs];
      for (j = 0; j < m_NumAttribs; j++) {
        coeff_mags[j] = (float) -Math.abs(m_Eigenvectors[j][m_SortedEigens[i]]);
      }
  ...
  protected Instance convertInstance(Instance instance) throws Exception {
    ...
    newVals = new float[m_OutputNumAtts];
