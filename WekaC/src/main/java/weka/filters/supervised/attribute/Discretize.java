...
public class Discretize extends Filter implements SupervisedFilter,
  ...
  protected float[][] m_CutPoints = null;
   ...
   */
  public float[] getCutPoints(int attributeIndex) {
  ...
  public String getBinRangesString(int attributeIndex) {
    ...
    float[] cutPoints = m_CutPoints[attributeIndex];
   ...
   */
  private static String binRangeString(float[] cutPoints, int j, int precision) {
  ...
  protected void calculateCutPoints() {
    ...
    m_CutPoints = new float[getInputFormat().numAttributes()][];
   ...
   */
  private boolean KononenkosMDL(float[] priorCounts, float[][] bestCounts,
    float numInstances, int numCutPoints) {

    float distPrior, instPrior, distAfter = 0, sum, instAfter = 0;
    float before, after;
    int numClassesTotal;

    // Number of classes occuring in the set
    numClassesTotal = 0;
    for (float priorCount : priorCounts) {
      if (priorCount > 0) {
        numClassesTotal++;
      }
    }
    ...
    for (float[] bestCount : bestCounts) {
   ...
   */
  private boolean FayyadAndIranisMDL(float[] priorCounts,
    float[][] bestCounts, float numInstances, int numCutPoints) {

    float priorEntropy, entropy, gain;
    float entropyLeft, entropyRight, delta;
    ...
    delta = Utils.log2((float) (Math.pow(3, numClassesTotal) - 2))
   ...
   * @return
   */
  private float[] cutPointsForSubset(Instances instances, int attIndex,
    int first, int lastPlusOne) {

    float[][] counts, bestCounts;
    float[] priorCounts, left, right, cutPoints;
    float currentCutPoint = -Float.MAX_VALUE, bestCutPoint = -1, currentEntropy, bestEntropy, priorEntropy, gain;
    ...
    counts = new float[2][instances.numClasses()];
    ...
    // Save prior counts
    priorCounts = new float[instances.numClasses()];
    ...
    // Find best entropy.
    bestCounts = new float[2][instances.numClasses()];
    for (int i = first; i < (lastPlusOne - 1); i++) {
      ...
      if (instances.instance(i).value(attIndex) < instances.instance(i + 1)
        ...
        currentCutPoint = ((instances.instance(i).value(attIndex) + instances
          .instance(i + 1).value(attIndex)) / 2.0f);
          ...
          if (cutPoints == null) {
            ...
            for (int j = 0, n = cutPoints.length; j < n; ++j) {
              ...
              if (m_UseBinNumbers) {
                ...
                float[] binaryCutPoint = { cutPoints[j] };
  ...
  protected void convertInstance(Instance instance) {
    ...
    for (int i = 0; i < getInputFormat().numAttributes(); i++) {
      if (m_DiscretizeCols.isInRange(i)
        && getInputFormat().attribute(i).isNumeric()) {
        int j;
        float currentVal = instance.value(i);
