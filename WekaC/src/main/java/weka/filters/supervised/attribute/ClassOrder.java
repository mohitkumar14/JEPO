...
public class ClassOrder extends Filter implements SupervisedFilter,
   ...
   * effect
   */
  private float[] m_ClassCounts = null;
  ...
  public boolean setInputFormat(Instances instanceInfo) throws Exception {
    ...
    m_ClassCounts = new float[numClasses];
    return false;
  }
  ...
  public boolean batchFinished() throws Exception {
    ...
    if (m_Converter == null) {
      ...
      float[] randomizedCounts = new float[m_ClassCounts.length];
      ...
      if (m_ClassOrder == RANDOM) {
        ...
        // Change class counts
        float[] tmp2 = new float[m_ClassCounts.length];
   ...
   * 
   * @param classDist the given class distribution
   * @return the distribution converted back
   */
  public float[] distributionsByOriginalIndex(float[] classDist) {

    float[] after = new float[m_Converter.length];
    for (int i = 0; i < m_Converter.length; i++) {
      after[i] = classDist[m_Converter[i]];
    }
