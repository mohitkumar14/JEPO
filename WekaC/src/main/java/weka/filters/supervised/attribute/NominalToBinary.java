...
public class NominalToBinary extends Filter implements SupervisedFilter,
  ...
  private void computeAverageClassValues() {
    ...
    float totalCounts, sum;
    Instance instance;
    float[] counts;

    float[][] avgClassValues = new float[getInputFormat().numAttributes()][0];
    m_Indices = new int[getInputFormat().numAttributes()][0];
    for (int j = 0; j < getInputFormat().numAttributes(); j++) {
      Attribute att = getInputFormat().attribute(j);
      if (att.isNominal()) {
        avgClassValues[j] = new float[att.numValues()];
        counts = new float[att.numValues()];
        for (int i = 0; i < getInputFormat().numInstances(); i++) {
