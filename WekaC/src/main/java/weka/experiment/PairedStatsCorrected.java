...
public class PairedStatsCorrected
   ...
   */
  public PairedStatsCorrected(float sig, float testTrainRatio) {
  ...
  public void calculateDerived() {
    ...
    if (differencesStats.stdDev > 0) {
      ...
      float tval = (float) (differencesStats.mean
      ...
      if (differencesStats.sumSq == 0) {
        differencesProbability = 1.0f;
      } else {
        differencesProbability = 0.0f;
      }
    }
  ...
  public static void main(String [] args) {
    ...
    try {
      PairedStatsCorrected ps = new PairedStatsCorrected(0.05f, 1.0f / 9.0f);
      ...
      while ((line = r.readLine()) != null) {
        ...
        while (s.hasMoreTokens()) {
          float val = (new Float(s.nextToken())).floatValue();
          if (count == 0) {
            v1 = val;
          } else if (count == 1) {
            v2 = val;
