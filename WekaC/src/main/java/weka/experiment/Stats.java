...
implements Serializable, RevisionHandler {
  ...
  public float sumSq = 0;

  /** The std deviation of values at the last calculateDerived() call */    
  public float stdDev = Float.NaN;

  /** The mean of values, or Float.NaN if no values seen */
   ...
   * @param weight the weight of the observed value
   */
  public void subtract(float value, float weight) {
    ...
    try {
      ...
      while ((line = r.readLine()) != null) {
        ...
        while (s.hasMoreTokens()) {
          float val = (float) (new Float(s.nextToken())).doubleValue();
          if (count == 0) {
            v1 = val;
          } else {
