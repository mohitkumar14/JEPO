...
public class PairedTTester implements OptionHandler, Tester, RevisionHandler {
  ...
  protected float m_SignificanceLevel = 0.05f;
  ...
  protected class Dataset implements RevisionHandler, Serializable {
    ...
    public void sort(int runColumn) {
      ...
      float[] runNums = new float[m_Dataset.size()];
  ...
  public PairedStats calculateStatistics(Instance datasetSpecifier,
    ...
    for (int k = 0; k < dataset1.size(); k++) {
      ...
      float value1 = current1.value(comparisonColumn);
      float value2 = current2.value(comparisonColumn);
  ...
  public String multiResultsetFull(int baseResultset, int comparisonColumn)
    ...
    float[] sortValues = new float[getNumDatasets()];

    // determine max field width
    for (int i = 0; i < getNumDatasets(); i++) {
      sortValues[i] = Float.POSITIVE_INFINITY; // sorts skipped cols to end
      ...
      for (int j = 0; j < getNumResultsets(); j++) {
        ...
        try {
          PairedStats pairedStats = calculateStatistics(
            ...
            comparisonColumn);
          if (!Float.isInfinite(pairedStats.yStats.mean)
            && !Float.isNaN(pairedStats.yStats.mean)) {
            float width = (float) ((Math.log(Math.abs(pairedStats.yStats.mean)) / Math
              .log(10)) + 1);
          ...
          if (m_ShowStdDevs && !Float.isInfinite(pairedStats.yStats.stdDev)
            && !Float.isNaN(pairedStats.yStats.stdDev)) {
            float width = (float) ((Math.log(Math.abs(pairedStats.yStats.stdDev)) / Math
              .log(10)) + 1);
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (sigStr.length() != 0) {
      setSignificanceLevel((new Float(sigStr)).floatValue());
    } else {
      setSignificanceLevel(0.05f);
    }
   ...
   */
  @Override
  public void setSignificanceLevel(float newSignificanceLevel) {
