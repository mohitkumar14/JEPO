...
public abstract class ResultMatrix implements Serializable, RevisionHandler,
  ...
  /** the values. */
  protected float[][] m_Mean = null;

  /** the standard deviation. */
  protected float[][] m_StdDev = null;

  /** the counts for the different datasets. */
  protected float[] m_Counts = null;
   ...
   */
  public void setCount(int index, float count) {
    if ((index >= 0) && (index < getRowCount())) {
   ...
   */
  public void setMean(int col, int row, float value) {
   ...
   * @return the mean
   */
  public float getMean(int col, int row) {
    if ((col >= 0) && (col < getColCount()) && (row >= 0)
      && (row < getRowCount())) {
   ...
   * @return the average
   */
  public float getAverage(int col) {
    int i;
    float avg;
    int count;
   ...
   */
  public void setStdDev(int col, int row, float value) {
    if ((col >= 0) && (col < getColCount()) && (row >= 0)
      && (row < getRowCount())) {
   ...
   */
  public float getStdDev(int col, int row) {
    if ((col >= 0) && (col < getColCount()) && (row >= 0)
      && (row < getRowCount())) {
   ...
   */
  protected String floatToString(float d, int prec) {
    String result;
    int currentPrec;
    int i;
  ...
  protected String[][] toArray() {
    ...
    for (ii = 0; ii < getRowCount(); ii++) {
      ...
      for (nn = 0; nn < getColCount(); nn++) {
        ...
        if (!valueExists) {
          ...
          result[y][x] = floatToString(getMean(n, i), getMeanPrec());
        }
        x++;
        ...
        if (getShowStdDev()) {
          if (!valueExists) {
            ...
            result[y][x] = floatToString(getStdDev(n, i), getStdDevPrec());
          }
          x++;
        }
    ...
    if (getShowAverage()) {
      ...
      for (ii = 0; ii < getColCount(); ii++) {
        ...
        // mean-average
        result[y][x] = floatToString(getAverage(i), getMeanPrec());
        x++;
