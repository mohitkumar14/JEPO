...
public class DefaultAssociationRule extends AssociationRule 
  ...
  public static enum METRIC_TYPE {
    CONFIDENCE("conf") {
      float compute(int premiseSupport, int consequenceSupport, 
    ...
    LIFT("lift") {
        ...
        return confidence / (consequenceSupport /
            totalTransactions);
      }
    },
    LEVERAGE("lev") {
    	...
    	float coverageForItemSet = totalSupport /
          totalTransactions;
    	float expectedCoverageIfIndependent = 
          (premiseSupport / totalTransactions) *
          (consequenceSupport / totalTransactions);
    ...
    CONVICTION("conv") {
    	...
    	float num = 
          premiseSupport * (totalTransactions - consequenceSupport) /
          totalTransactions;
    	float denom = premiseSupport - totalSupport + 1;
        return num / denom;
      }
    };
    ...
    abstract float compute(int premiseSupport, int consequenceSupport, 
        int totalSupport, int totalTransactions);
    ...
    public String toStringMetric(int premiseSupport, int consequenceSupport,
        int totalSupport, int totalTransactions) {
      return new StringBuilder(m_stringVal).append(":(").append(Utils.doubleToString(compute(premiseSupport, consequenceSupport,
          totalSupport, totalTransactions), 2)).append( ")").toString();
    }
    
    public String toXML(int premiseSupport, int consequenceSupport,
        int totalSupport, int totalTransactions) {
      String result = new StringBuilder("<CRITERE name=\"").append(m_stringVal).append("\" value=\" ").append(
   ...
   */
  public float getPrimaryMetricValue() {
   ...
   */
  public float getNamedMetricValue(String metricName) throws Exception {
  ...
  public String toString() {
    ...
    result.append(m_premise.toString()).append(": ").append(m_premiseSupport) 
    	.append(" ==> ").append(m_consequence.toString()).append(": ").append(m_totalSupport )
     .append("   ");
    for (DefaultAssociationRule.METRIC_TYPE m : METRIC_TYPE.values()) {
      if (m.equals(m_metricType)) {
        result.append("<").append(
            m.toStringMetric(m_premiseSupport, m_consequenceSupport, 
                m_totalSupport, m_totalTransactions)).append("> ");
      } else {
        result.append("").append( 
            m.toStringMetric(m_premiseSupport, m_consequenceSupport, 
                m_totalSupport, m_totalTransactions)).append(" ");
