...
public class Apriori extends AbstractAssociator implements OptionHandler,
  ...
  /** The minimum support. */
  protected float m_minSupport;

  /** The upper bound on the support */
  protected float m_upperBoundMinSupport;

  /** The lower bound for the minimum support. */
  protected float m_lowerBoundMinSupport;
  ...
  protected float m_significanceLevel;
  ...
  public String globalInfo() {
    return new StringBuilder("Class implementing an Apriori-type algorithm. Iteratively reduces ")
      .append("the minimum support until it finds the required number of rules with ")
      .append("the given minimum confidence.\n")
      .append("The algorithm has an option to mine class association rules. It is ")
      .append("adapted as explained in the second reference.\n\n")
  ...
  public void resetOptions() {
    ...
    m_minMetric = 0.90f;
    m_numRules = 10;
    m_lowerBoundMinSupport = 0.1f;
    m_upperBoundMinSupport = 1.0f;
    ...
    for (int i = 0; i < instances.numAttributes(); i++) {
      ...
      if (as.missingCount == numInstances) {
        if (first) {
          ...
          deleteString.append(",").append(i + 1);
        }
        removeCount++;
      }
    }
  ...
  public void buildAssociations(Instances instances) throws Exception {
    ...
    float lowerBoundMinSupportToUse;
    if(m_lowerBoundMinSupport* instances.numInstances() < 1.0)
    	lowerBoundMinSupportToUse =  1 / instances.numInstances();
    else
    	lowerBoundMinSupportToUse =  m_lowerBoundMinSupport;
    ...
    do {
      ...
      supports = new float[m_allTheRules[2].size()];
      for (int i = 0; i < (j + 1); i++) {
        supports[j - i] = ((float) ((ItemSet) m_allTheRules[1].get(j - i))
          .support()) * (-1);
      }
      ...
      // }
      confidences = new float[sortedRuleSet[2].size()];
      int sortType = 2 + m_metricType;

      for (int i = 0; i < sortedRuleSet[2].size(); i++) {
        confidences[i] = ((Float) sortedRuleSet[sortType].get(i))
          .floatValue();
      }
      ...
      necSupport = (float) Math.rint(m_minSupport * m_instances.numInstances());
  ...
  public Enumeration<Option> listOptions() {
    ...
    String string1 = new StringBuilder("\tThe required number of rules. (default = ").append(m_numRules).append(")").toString()
    		, string2 = new StringBuilder("\tThe minimum confidence of a rule. (default = ").append(m_minMetric).append(")").toString()
    		, string3 = "\tThe delta by which the minimum support is decreased in\n"
    		, string4 = new StringBuilder("\teach iteration. (default = ").append(m_delta).append(")").toString()
    		, string5 = new StringBuilder("\tThe lower bound for the minimum support. (default = ").append(m_lowerBoundMinSupport).append(")").toString()
    		...
    		, stringType = new StringBuilder("\tThe metric type by which to rank rules. (default = ").append("confidence)").toString()
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (minConfidenceString.length() != 0) {
      m_minMetric = (new Float(minConfidenceString)).floatValue();
    }
    if (deltaString.length() != 0) {
      m_delta = (new Float(deltaString)).floatValue();
    }
    if (maxSupportString.length() != 0) {
      setUpperBoundMinSupport((new Float(maxSupportString)).floatValue());
    }
    if (minSupportString.length() != 0) {
      m_lowerBoundMinSupport = (new Float(minSupportString)).floatValue();
    }
    if (significanceLevelString.length() != 0) {
      m_significanceLevel = (new Float(significanceLevelString)).floatValue();
    }
  ...
  public String[] getOptions() {
    ...
    options[current++] = "-N";
    options[current++] = String.valueOf(m_numRules);
    options[current++] = "-T";
    options[current++] = String.valueOf( m_metricType);
    options[current++] = "-C";
    options[current++] = String.valueOf(m_minMetric);
    options[current++] = "-D";
    options[current++] = String.valueOf( m_delta);
    options[current++] = "-U";
    options[current++] = String.valueOf(m_upperBoundMinSupport);
    options[current++] = "-M";
    options[current++] = String.valueOf(m_lowerBoundMinSupport);
    options[current++] = "-S";
    options[current++] = String.valueOf(m_significanceLevel);
    if (m_car) {
      options[current++] = "-A";
    }
    if (m_verbose) {
    ...
    options[current++] = "-c";
    options[current++] = String.valueOf(m_classIndex);
  ...
  public String toString() {
    ...
    text.append("\nApriori\n=======\n\n");
    text.append("Minimum support: ").append(Utils.doubleToString(m_minSupport, 2)).append(" (")
    .append((int) (m_minSupport * m_instances.numInstances() + 0.5)).append(" instances)\n");
    ...
    text.append("Number of cycles performed: ").append(m_cycles).append('\n');
    text.append("\nGenerated sets of large itemsets:\n");
    if (!m_car) {
      for (int i = 0; i < m_Ls.size(); i++) {
        text.append("\nSize of set of large itemsets L(").append(i + 1).append("): ")
        .append((m_Ls.get(i)).size()).append('\n');
        if (m_outputItemSets) {
          text.append("\nLarge Itemsets L(").append(i + 1).append("):\n");
          for (int j = 0; j < (m_Ls.get(i)).size(); j++) {
            text.append(((AprioriItemSet) (m_Ls.get(i)).get(j))).append(m_instances).append("\n");
          }
        }
      }
      ...
      for (int i = 0; i < m_allTheRules[0].size(); i++) {
        ...
        String n = Utils.doubleToString((float) i + 1,
        ...
        String confidence = Utils.doubleToString(
          ((Float) m_allTheRules[2].get(i)).floatValue(), 2);
        String lift = Utils.doubleToString(
          ((Float) m_allTheRules[3].get(i)).floatValue(), 2);
        String leverage = Utils.doubleToString(
          ((Float) m_allTheRules[4].get(i)).floatValue(), 2);
        String conviction = Utils.doubleToString(
          ((Float) m_allTheRules[5].get(i)).floatValue(), 2);

        int leverageT = (int) (((Float) m_allTheRules[4].get(i)).doubleValue() * m_instances
          .numInstances());
      ...
      for (int i = 0; i < m_Ls.size(); i++) {
        ...
        if (m_outputItemSets) {
          text.append("\nLarge Itemsets L(").append(i + 1).append("):\n");
          for (int j = 0; j < (m_Ls.get(i)).size(); j++) {
            text.append(((ItemSet) (m_Ls.get(i)).get(j)).toString(m_instances)).append("\n");
            text.append(((LabeledItemSet) (m_Ls.get(i)).get(j)).m_classLabel).append("  ");
            text.append(((LabeledItemSet) (m_Ls.get(i)).get(j)).support()).append("\n");
          }
        }
      }
      text.append("\nBest rules found:\n\n");
   ...
   */
  public float getUpperBoundMinSupport() {
   ...
   */
  public void setUpperBoundMinSupport(float v) {
   ...
   */
  public float getLowerBoundMinSupport() {
   ...
   */
  public void setLowerBoundMinSupport(float v) {
  ...
  public String metricTypeTipText() {
    return new StringBuilder("Set the type of metric by which to rank rules. Confidence is ")
      .append("the proportion of the examples covered by the premise that are also ")
      .append("covered by the consequence (Class association rules can only be mined using confidence). Lift is confidence divided by the ")
      .append("proportion of all examples that are covered by the consequence. This ")
      .append("is a measure of the importance of the association that is independent ")
      .append("of support. Leverage is the proportion of additional examples covered ")
      .append("by both the premise and consequence above those expected if the ")
      .append("premise and consequence were independent of each other. The total ")
      .append("number of examples that this represents is presented in brackets ")
      .append("following the leverage. Conviction is ")
      .append("another measure of departure from independence. Conviction is given ")
      .append("by P(premise)P(!consequence) / P(premise, !consequence).").toString();
  }
   ...
   */
  public void setMinMetric(float v) {
   ...
   */
  public void setSignificanceLevel(float v) {
  ...
  private void findLargeCarItemSets() throws Exception {
    ...
    // minimum support
    float nextMinSupport = m_minSupport * m_instances.numInstances();
    float nextMaxSupport = m_upperBoundMinSupport * m_instances.numInstances();
