...
public class AprioriItemSet extends ItemSet implements Serializable,
   ...
   */
  public static float confidenceForRule(AprioriItemSet premise,
    AprioriItemSet consequence) {

    return  consequence.m_counter /  premise.m_counter;
  }
   ...
   */
  public float liftForRule(AprioriItemSet premise, AprioriItemSet consequence,
    int consequenceCount) {
    float confidence = confidenceForRule(premise, consequence);
   ...
   */
  public float leverageForRule(AprioriItemSet premise,
    AprioriItemSet consequence, int premiseCount, int consequenceCount) {
	  float coverageForItemSet =  consequence.m_counter/  m_totalTransactions;
	  float expectedCoverageIfIndependent = ( premiseCount / m_totalTransactions)* ( consequenceCount /  m_totalTransactions);
	  float lev = coverageForItemSet - expectedCoverageIfIndependent;
    return lev;
  }
   ...
   */
  public float convictionForRule(AprioriItemSet premise,
    AprioriItemSet consequence, int premiseCount, int consequenceCount) {
	  float num = premiseCount * (m_totalTransactions - consequenceCount) / m_totalTransactions;
	  float denom = ((premiseCount - consequence.m_counter) + 1);
   ...
   */
  public ArrayList<Object>[] generateRules(float minConfidence,
    ...
    for (int i = 0; i < m_items.length; i++) {
      if (m_items[i] != -1) {
        ...
        float tempLift = liftForRule(premise, consequence,consequenceUnconditionedCounter);
        float tempLev = leverageForRule(premise, consequence,premise.m_counter, consequenceUnconditionedCounter);
        float tempConv = convictionForRule(premise, consequence,premise.m_counter, consequenceUnconditionedCounter);
  ...
  public final ArrayList<Object>[] generateRulesBruteForce(double minMetric,
    ...
    int numItemsInSet, int numTransactions, float significanceLevel)
    throws Exception {
    ...
    float[][] contingencyTable = new float[2][2];
    float metric, chiSquared = 0;
      ...
      if (numItemsInPremise < numItemsInSet) {
        ...
        if (metricType == 0) {
          ...
          if ((!(metric < minMetric))
        	...
        	float tempConf = confidenceForRule(premise, consequence);
        	float tempLift = liftForRule(premise, consequence,consequenceUnconditionedCounter);
        	float tempLev = leverageForRule(premise, consequence,premise.m_counter, consequenceUnconditionedCounter);
        	float tempConv = convictionForRule(premise, consequence,premise.m_counter, consequenceUnconditionedCounter);
          switch (metricType) {
  ...
  private final ArrayList<Object>[] moreComplexRules(ArrayList<Object>[] rules,
    int numItemsInSet, int numItemsInConsequence, float minConfidence,
    ...
    if (numItemsInSet > numItemsInConsequence + 1) {
      ...
      while (enu.hasMoreElements()) {
        ...
        float tempLift = liftForRule(newPremise, current,consequenceUnconditionedCounter);
        float tempLev = leverageForRule(newPremise, current,newPremise.m_counter, consequenceUnconditionedCounter);
        float tempConv = convictionForRule(newPremise, current,newPremise.m_counter, consequenceUnconditionedCounter);
