...
public class NumericItem extends Item implements Serializable {
   ...
   */
  public NumericItem(Attribute att, float splitPoint, Comparison comp) throws Exception {
    super(att);
  ...
  public String toString(boolean freq) {
    ...
    result.append(m_attribute.name()).append(" ");
    switch (m_comparison) {
    case EQUAL:
      result.append("=");
      break;
    ...
    result.append(" ").append(Utils.doubleToString(m_splitPoint, 4));
    if (freq) {
      result.append(":").append(m_frequency);
    }
