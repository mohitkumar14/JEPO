...
public class FPGrowth extends AbstractAssociator implements
  ...
  protected static class FrequentItemSets implements Serializable {
    ...
    public String toString(int numSets) {
      ...
      result.append("").append(m_sets.size()).append("frequent item sets found");
      if (numSets > 0) {
    ...
    public void graphFPTree(StringBuffer text) {
      if (m_children != null) {
        ...
        for (FPTreeNode n : kids) {
          ...
          text.append(" [label=\"");
          text.append(n.getItem().toString()).append(" (").append(n.getProjectedCount(0)).append(")\\n");
          text.append("\"]\n");
          n.graphFPTree(text);
          text.append("N").append(m_ID).append("->").append("N").append(n.m_ID).append("\n");
        }
      }
    }
  }
  ...
  public static List<AssociationRule> generateRulesBruteForce(
    ...
    DefaultAssociationRule.METRIC_TYPE metricToUse, float metricThreshold,
  ...
  protected float m_upperBoundMinSupport = 1.0f;

  /** The lower bound on minimum support */
  protected float m_lowerBoundMinSupport = 0.1f;

  /** The amount by which to decrease the support in each iteration */
  protected float m_delta = 0.05f;
  ...
  protected float m_metricThreshold = 0.9f;
  ...
  public String globalInfo() {
    return new StringBuilder("Class implementing the FP-growth algorithm for finding")
      .append(" large item sets without candidate generation. Iteratively")
      .append(" reduces the minimum support until it finds the required")
      .append(" number of rules with the given minimum metric.")
  ...
  public String positiveIndexTipText() {
    return new StringBuilder("Set the index of binary valued attributes that is to be considered")
      .append(" the positive index. Has no effect for sparse data (in this case")
      .append(" the first index (i.e. non-zero values) is always treated as ")
      .append(" positive. Also has no effect for unary valued attributes (i.e.")
      .append(" when using the Weka Apriori-style format for market basket data,")
      .append(" which uses missing value \"?\" to indicate absence of an item.").toString();
  }
  ...
  public String metricTypeTipText() {
    return new StringBuilder("Set the type of metric by which to rank rules. Confidence is ")
      ...
      .append("another measure of departure from independence.").toString();
  }
  ...
  public Enumeration<Option> listOptions() {
    ...
    String string1 = new StringBuilder("\tThe required number of rules. (default = ").append(m_numRulesToFind).append(")").toString();
    String string2 = new StringBuilder("\tThe minimum metric score of a rule. (default = ")
    		.append(m_metricThreshold).append(")").toString();
    String string3 = "\tThe metric by which to rank rules. (default = confidence)";
    String string4 = new StringBuilder("\tThe lower bound for the minimum support as a fraction or number of instances. (default = ")
    		.append(m_lowerBoundMinSupport).append(")").toString();
    String string5 = "\tUpper bound for minimum support as a fraction or number of instances. (default = 1.0)";
    String string6 = new StringBuilder("\tThe delta by which the minimum support is decreased in\n\teach iteration as a fraction or number of instances. (default = ")
    		.append(m_delta).append(")").toString();
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (minMetricString.length() != 0) {
      setMinMetric(Float.parseFloat(minMetricString));
    }

    if (deltaString.length() != 0) {
      setDelta(Float.parseFloat(deltaString));
    }

    if (lowerBoundSupportString.length() != 0) {
      setLowerBoundMinSupport(Float.parseFloat(lowerBoundSupportString));
    }

    if (upperBoundSupportString.length() != 0) {
      setUpperBoundMinSupport(Float.parseFloat(upperBoundSupportString));
    }
  ...
  public String[] getOptions() {
    ...
    options.add("-P");
    options.add(String.valueOf(getPositiveIndex()));
    options.add("-I");
    options.add(String.valueOf(getMaxNumberOfItems()));
    options.add("-N");
    options.add(String.valueOf(getNumRulesToFind()));
    options.add("-T");
    options.add(String.valueOf(getMetricType().getSelectedTag().getID()));
    options.add("-C");
    options.add(String.valueOf(getMinMetric()));
    options.add("-D");
    options.add(String.valueOf(getDelta()));
    options.add("-U");
    options.add(String.valueOf(getUpperBoundMinSupport()));
    options.add("-M");
    options.add(String.valueOf(getLowerBoundMinSupport()));
  ...
  private void buildAssociations(Object source) throws Exception {
    ...
    if(m_upperBoundMinSupport > 1)
    	upperBoundMinSuppAsInstances = (int) m_upperBoundMinSupport;
    else
   		upperBoundMinSuppAsInstances = (int) Math.ceil(m_upperBoundMinSupport * m_numInstances);

    int lowerBoundMinSuppAsInstances;
    if(m_lowerBoundMinSupport > 1)
    	lowerBoundMinSuppAsInstances = (int) m_lowerBoundMinSupport;
    else
    	lowerBoundMinSuppAsInstances =(int) Math.ceil(m_lowerBoundMinSupport * m_numInstances);

    float lowerBoundMinSuppAsFraction;
    if(m_lowerBoundMinSupport > 1)
    	lowerBoundMinSuppAsFraction = m_lowerBoundMinSupport/ m_numInstances;
    else
    	lowerBoundMinSuppAsFraction= m_lowerBoundMinSupport;

    float deltaAsFraction;
    if(m_delta > 1)
    	deltaAsFraction = m_delta / m_numInstances;
    else
    	deltaAsFraction =m_delta;

    // double currentSupport = upperBoundMinSuppAsFraction;
    float currentSupport = 1.0f;
  ...
  public String toString() {
    ...
    if (m_rules.size() == 0) {
      ...
      result.append("FPGrowth found ").append(m_rules.size()).append(" rules");
      if (!m_findAllRulesForSupportLevel) {
        result.append(" (displaying top ").append(numRules).append(")");
      }
    ...
    int count = 0;
    for (AssociationRule r : m_rules) {
      result.append(Utils.doubleToString((float) count + 1,
        (int) (Math.log(numRules) / Math.log(10) + 1), 0)).append(". ");
