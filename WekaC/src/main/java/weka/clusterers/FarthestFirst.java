...
public class FarthestFirst extends RandomizableClusterer implements
  ...
  protected void updateMinDistance(float[] minDistance, boolean[] selected,
    Instances data, Instance center) {
    for (int i = 0; i < selected.length; i++) {
      if (!selected[i]) {
        float d = distance(center, data.instance(i));
        if (d < minDistance[i]) {
          minDistance[i] = d;
        }
      }
  ...
  protected int farthestAway(float[] minDistance, boolean[] selected) {
  ...
  protected void initMinMax(Instances data) {
    m_Min = new float[data.numAttributes()];
    m_Max = new float[data.numAttributes()];
  ...
  protected int clusterProcessedInstance(Instance instance) {
    ...
    for (int i = 0; i < m_NumClusters; i++) {
      float dist = distance(instance, m_ClusterCentroids.instance(i));
      if (dist < minDist) {
        minDist = dist;
        bestCluster = i;
      }
   ...
   */
  protected float distance(Instance first, Instance second) {
    ...
    return (float) Math.sqrt(distance / m_instances.numAttributes());
  }
  ...
  protected float norm(float x, int i) {
    ...
    if (Float.isNaN(m_Min[i]) || Utils.eq(m_Max[i], m_Min[i])) {
      return 0;
    } else {
