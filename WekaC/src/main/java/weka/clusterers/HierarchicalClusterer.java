...
public class HierarchicalClusterer extends AbstractClusterer implements
   ...
   **/
  class Tuple {
    public Tuple(float d, int i, int j, int nSize1, int nSize2) {
      m_fDist = d;
      m_iCluster1 = i;
      m_iCluster2 = j;
  ...
  class Node implements Serializable {
    ...
    float m_fLeftLength = 0;
    float m_fRightLength = 0;
    float m_fHeight = 0;
    ...
    void setHeight(float fHeight1, float fHeight2) {
      m_fHeight = fHeight1;
      if (m_left == null) {
    ...
    void setLength(float fLength1, float fLength2) {
  ...
  void neighborJoining(int nClusters, Vector<Integer>[] nClusterID,
    ...
    float[][] fDist = new float[nClusters][nClusters];
    ...
    float[] fSeparationSums = new float[n];
    float[] fSeparations = new float[n];
    ...
    while (nClusters > 2) {
      ...
      if (m_Debug) {
        for (int i = 0; i < n; i++) {
          if (nClusterID[i].size() > 0) {
            float[] fRow = fDist[i];
            float fSep1 = fSeparations[i];
            for (int j = 0; j < n; j++) {
              if (nClusterID[j].size() > 0 && i != j) {
                float fSep2 = fSeparations[j];
                float fVal = fRow[j] - fSep1 - fSep2;
      ...
      // record distance
      float fMinDistance = fDist[iMin1][iMin2];
      nClusters--;
      float fSep1 = fSeparations[iMin1];
      float fSep2 = fSeparations[iMin2];
      float fDist1 = (0.5f * fMinDistance) + (0.5f * (fSep1 - fSep2));
      float fDist2 = (0.5f* fMinDistance) + (0.5f * (fSep2 - fSep1));
      if (nClusters > 2) {
        // update separations & distance
        float fNewSeparationSum = 0;
        float fMutualDistance = fDist[iMin1][iMin2];
        float[] fRow1 = fDist[iMin1];
        float[] fRow2 = fDist[iMin2];
        for (int i = 0; i < n; i++) {
          if (i == iMin1 || i == iMin2 || nClusterID[i].size() == 0) {
            ...
            float fVal1 = fRow1[i];
            float fVal2 = fRow2[i];
            float fDistance = (fVal1 + fVal2 - fMutualDistance) / 2.0f;
        ...
        for (int j = i + 1; j < n; j++) {
          if (nClusterID[j].size() > 0) {
            ...
            if (nClusterID[i].size() == 1) {
              ...
              merge(i, j, fDist1 / 2.0f, fDist1 / 2.0f, nClusterID, clusterNodes);
            }
            break;
          }
        }
  ...
  void doLinkClustering(int nClusters, Vector<Integer>[] nClusterID,
    ...
    float[][] fDistance0 = new float[nClusters][nClusters];
    float[][] fClusterDistance = null;
    if (m_Debug) {
      fClusterDistance = new float[nClusters][nClusters];
    }
    for (int i = 0; i < nClusters; i++) {
      fDistance0[i][i] = 0;
      for (int j = i + 1; j < nClusters; j++) {
    ...
    while (nClusters > m_nNumClusters) {
        ...
        for (int i = 0; i < nInstances; i++) {
          if (nClusterID[i].size() > 0) {
            for (int j = i + 1; j < nInstances; j++) {
              if (nClusterID[j].size() > 0) {
                float fDist = fClusterDistance[i][j];
      ...
      for (int i = 0; i < nInstances; i++) {
        if (i != iMin1 && nClusterID[i].size() != 0) {
          ...
          float fDistance = getDistance(fDistance0, nClusterID[i1],
            nClusterID[i2]);
          if (m_Debug) {
  ...
  void merge(int iMin1, int iMin2, float fDist1, float fDist2,
  ...
  float getDistance0(Vector<Integer> cluster1, Vector<Integer> cluster2) {
    float fBestDist = Float.MAX_VALUE;
    switch (m_nLinkType) {
    case SINGLE:
    ...
    case WARD: {
      ...
      float ESS1 = calcESS(cluster1);
      float ESS2 = calcESS(cluster2);
      Vector<Integer> merged = new Vector<Integer>();
      merged.addAll(cluster1);
      merged.addAll(cluster2);
      float ESS = calcESS(merged);
   ...
   */
  float getDistance(float[][] fDistance, Vector<Integer> cluster1,
    ...
    case CENTROID:
      ...
      float[] fValues1 = new float[m_instances.numAttributes()];
      for (int i = 0; i < cluster1.size(); i++) {
      ...
      float[] fValues2 = new float[m_instances.numAttributes()];
      for (int i = 0; i < cluster2.size(); i++) {
  ...
  float calcESS(Vector<Integer> cluster) {
    float[] fValues1 = new float[m_instances.numAttributes()];
  ...
  public int clusterInstance(Instance instance) throws Exception {
    ...
    for (int i = 0; i < m_instances.numInstances(); i++) {
      float fDist = m_DistanceFunction.distance(instance,
  ...
  public float[] distributionForInstance(Instance instance) throws Exception {
    ...
    float[] p = new float[numberOfClusters()];
    p[clusterInstance(instance)] = 1.0f;
    return p;
  }
