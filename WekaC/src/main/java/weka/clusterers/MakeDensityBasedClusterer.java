...
public class MakeDensityBasedClusterer extends AbstractDensityBasedClusterer
  ...
  public void buildClusterer(Instances data) throws Exception {
    ...
    m_model = new DiscreteEstimator[m_wrappedClusterer.numberOfClusters()][data
      .numAttributes()];
    m_modelNormal = new float[m_wrappedClusterer.numberOfClusters()][data
      .numAttributes()][2];
    float[][] weights = new float[m_wrappedClusterer.numberOfClusters()][data
      .numAttributes()];
    m_priors = new float[m_wrappedClusterer.numberOfClusters()];
    for (int i = 0; i < m_wrappedClusterer.numberOfClusters(); i++) {
      m_priors[i] = 1.0f; // laplace correction
    ...
    for (int i = 0; i < data.numInstances(); i++) {
      ...
      for (int j = 0; j < data.numAttributes(); j++) {
        if (!inst.isMissing(j)) {
          if (data.attribute(j).isNumeric()) {
            float diff = m_modelNormal[clusterIndex[i]][j][0] - inst.value(j);
  ...
  public float[] logDensityPerClusterForInstance(Instance inst)
    ...
    int i, j;
    float logprob;
    float[] wghts = new float[m_wrappedClusterer.numberOfClusters()];
  ...
  private static float m_normConst = (float) (0.5f * Math.log(2 * Math.PI));
