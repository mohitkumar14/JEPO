...
public class ClusterEvaluation implements Serializable, RevisionHandler {
   ...
   */
  public float[] getClusterAssignments() {
  ...
  public void evaluateClusterer(Instances test, String testFileName,
      ...
      if (m_Clusterer instanceof BatchPredictor
        ...
        try {
          if (m_Clusterer instanceof DensityBasedClusterer) {
            ...
            clusterAssignments.add((float) cnum);
          } else {
            cnum = m_Clusterer.clusterInstance(inst);
            clusterAssignments.add((float) cnum);
          }
        } catch (Exception e) {
          clusterAssignments.add(-1.0f);
    ...
    float sum = Utils.sum(instanceStats);
    loglk /= sum;
    m_logL = loglk;
    m_clusterAssignments = new float[clusterAssignments.size()];
    ...
    for (i = 0; i < cc; i++) {
      if (instanceStats[i] > 0) {
        m_clusteringResults.append(Utils.doubleToString(i, clustFieldWidth, 0)
          ...
          + Utils.doubleToString((instanceStats[i] / sum * 100.0f), 3, 0)
          + "%)\n");
      }
    }
  ...
  private void evaluateClustersWithRespectToClass(Instances inst,
    ...
    float[] best = new float[m_numClusters + 1];
    float[] current = new float[m_numClusters + 1];
    DataSource source = null;
    ...
    best[m_numClusters] = Float.MAX_VALUE;
    ...
    m_clusteringResults.append("\nIncorrectly clustered instances :\t"
      + best[m_numClusters]
      + "\t"
      + (Utils.doubleToString((best[m_numClusters] / numInstances * 100.0f), 8,
        4)) + " %\n");
  ...
  public static void mapClasses(int numClusters, int lev, int[][] counts,
    int[] clusterTotals, float[] current, float[] best, int error) {
    // leaf
    if (lev == numClusters) {
   ...
   */
  public static float crossValidateModel(DensityBasedClusterer clusterer,
  ...
  public boolean equals(Object obj) {
    ...
    if (Float.isNaN(m_logL) != Float.isNaN(cmp.m_logL)) {
      return false;
    }
    if (!Float.isNaN(m_logL)) {
