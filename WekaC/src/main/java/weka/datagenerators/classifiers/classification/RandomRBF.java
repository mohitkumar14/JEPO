...
public class RandomRBF extends ClassificationGenerator {
   ...
   */
  protected int chooseRandomIndexBasedOnProportions(float[] proportionArray,
    Random random) {
  ...
  public Instances defineDataFormat() throws Exception {
    ...
    m_centroids = new float[getNumCentroids()][getNumAttributes()];
    m_centroidClasses = new int[getNumCentroids()];
    m_centroidWeights = new float[getNumCentroids()];
    m_centroidStdDevs = new float[getNumCentroids()];

    for (i = 0; i < getNumCentroids(); i++) {
      for (j = 0; j < getNumAttributes(); j++) {
        m_centroids[i][j] = rand.nextFloat();
      }
      m_centroidClasses[i] = rand.nextInt(getNumClasses());
      m_centroidWeights[i] = rand.nextFloat();
      m_centroidStdDevs[i] = rand.nextFloat();
    }
  ...
  public Instance generateExample() throws Exception {
    ...
    // generate attributes
    atts = new float[getNumAttributes() + 1];
    for (i = 0; i < getNumAttributes(); i++) {
      atts[i] = (rand.nextFloat() * 2.0f) - 1.0f;
    }
    ...
    desiredMag = (float) (rand.nextGaussian() * m_centroidStdDevs[centroid]);
