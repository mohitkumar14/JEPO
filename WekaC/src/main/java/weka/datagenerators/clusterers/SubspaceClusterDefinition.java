...
public class SubspaceClusterDefinition extends ClusterDefinition {
  ...
  /** min or mean */
  protected float[] m_valueA;

  /** max or stddev */
  protected float[] m_valueB;
  ...
  public void setValuesList(String fromToList) throws Exception {
    m_valueA = new float[m_numClusterAttributes];
    m_valueB = new float[m_numClusterAttributes];
    ...
    for (int i = 0; i < m_numClusterAttributes; i++) {
      ...
      if (getParent() != null) {
        ...
        if (parent.isNominal(m_attrIndices[i])) {
          ...
          float rest = (float) (m_valueA[i] - Math.rint(m_valueA[i]));
          if (rest != 0.0) {
            throw new Exception(" Ranges for nominal must be integer");
          }
          rest = (float) (m_valueB[i] - Math.rint(m_valueB[i]));
          if (rest != 0.0) {
            throw new Exception("Ranges for nominal must be integer");
          }
          if (m_valueA[i] < 0.0) {
   ...
   * @throws Exception if values are not correct in number or value
   */
  public void setValuesList(String fromToList, float[] first, float[] second,
    ...
    while (tok.hasMoreTokens()) {
      first[index] = Float.parseFloat(tok.nextToken());
      second[index] = Float.parseFloat(tok.nextToken());
      index++;
    }
  }
